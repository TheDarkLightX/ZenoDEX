...
fn distinct_fee_owner_accepts_the_exact_i128_min_sender_delta() {
    // Arrange: amount + fee is exactly 2^127, whose negative is i128::MIN.
    // Constructing the positive sum in i128 before negation incorrectly rejects
    // this representable final effect.
    let max_i128_atoms = i128::MAX as u128;
    let debit_atoms = 1_u128 << 127;
    let mut pre_state = state(true, 1);
    pre_state.balances = vec![EconomicAmountV1 {
        owner: "alice".to_owned(),
        asset: "USD".to_owned(),
        custody_domain: ACCOUNT_CUSTODY_DOMAIN_V1.to_owned(),
        amount_atoms: debit_atoms,
    }];
    pre_state.supplies[0].amount_atoms = debit_atoms;
    let mut transfer_command = command();
    transfer_command.amount_atoms = max_i128_atoms;
    transfer_command.max_fee_atoms = 1;

    // Act.
    let result = transition_asset_transfer_v1(&context(), &pre_state, &transfer_command)
        .expect("typed transfer must evaluate");
...
fn sender_owned_fee_is_aggregated_before_debit_width_validation() {
    // Arrange: amount + fee exceeds i128::MAX, while self-allocation cancels
    // the fee in the sender's final account delta. Every final effect remains
    // representable and the sender pays exactly the transfer amount.
    let max_i128_atoms = i128::MAX as u128;
    let mut pre_state = state_with_fee_owner(true, max_i128_atoms, "alice");
    pre_state.balances = vec![EconomicAmountV1 {
        owner: "alice".to_owned(),
        asset: "USD".to_owned(),
        custody_domain: ACCOUNT_CUSTODY_DOMAIN_V1.to_owned(),
        amount_atoms: max_i128_atoms,
    }];
    pre_state.supplies[0].amount_atoms = max_i128_atoms;
    let mut transfer_command = command();
    transfer_command.amount_atoms = max_i128_atoms;
    transfer_command.max_fee_atoms = max_i128_atoms;
...
#[test]
fn final_effect_widths_reject_each_neighbor_above_the_supported_boundary() {
    let max_i128_atoms = i128::MAX as u128;

    // Arrange: the distinct-owner debit magnitude is i128::MIN magnitude + 1.
    let mut distinct_state = state(true, 2);
    distinct_state.balances = vec![EconomicAmountV1 {
        owner: "alice".to_owned(),
        asset: "USD".to_owned(),
        custody_domain: ACCOUNT_CUSTODY_DOMAIN_V1.to_owned(),
        amount_atoms: (1_u128 << 127) + 1,
    }];
    distinct_state.supplies[0].amount_atoms = (1_u128 << 127) + 1;
    let mut distinct_command = command();
    distinct_command.amount_atoms = max_i128_atoms;
    distinct_command.max_fee_atoms = 2;

    // Act and assert.
    assert_eq!(
        reject_code(&context(), &distinct_state, &distinct_command),
        AssetTransferRejectCodeV1::EFFECT_DELTA_OVERFLOW
    );

    // Arrange: self-allocation cancels the account fee delta, while the
    // separately reported fee-allocation effect is still one atom too wide.
    let too_wide_fee = 1_u128 << 127;
    let mut sender_owned_state = state_with_fee_owner(true, too_wide_fee, "alice");
    sender_owned_state.balances = vec![EconomicAmountV1 {
        owner: "alice".to_owned(),
        asset: "USD".to_owned(),
        custody_domain: ACCOUNT_CUSTODY_DOMAIN_V1.to_owned(),
        amount_atoms: 1,
    }];
    sender_owned_state.supplies[0].amount_atoms = 1;
    let mut sender_owned_command = command();
    sender_owned_command.amount_atoms = 1;
    sender_owned_command.max_fee_atoms = too_wide_fee;

    // Act and assert.
    assert_eq!(
        reject_code(&context(), &sender_owned_state, &sender_owned_command),
        AssetTransferRejectCodeV1::EFFECT_DELTA_OVERFLOW
    );

    // Arrange: recipient ownership aggregates amount + fee into a positive
    // account effect one atom above i128::MAX.
    let mut recipient_owned_state = state_with_fee_owner(true, 1, "bob");
    recipient_owned_state.balances = vec![EconomicAmountV1 {
        owner: "alice".to_owned(),
        asset: "USD".to_owned(),
        custody_domain: ACCOUNT_CUSTODY_DOMAIN_V1.to_owned(),
        amount_atoms: 1_u128 << 127,
    }];
    recipient_owned_state.supplies[0].amount_atoms = 1_u128 << 127;
    let mut recipient_owned_command = command();
    recipient_owned_command.amount_atoms = max_i128_atoms;
    recipient_owned_command.max_fee_atoms = 1;

    // Act and assert.
    assert_eq!(
        reject_code(&context(), &recipient_owned_state, &recipient_owned_command,),
        AssetTransferRejectCodeV1::EFFECT_DELTA_OVERFLOW
    );
}
