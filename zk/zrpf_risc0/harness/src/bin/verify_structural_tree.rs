...
fn parse_options(args: impl IntoIterator<Item = String>) -> Result<Options, String> {
    ...
    if args.len() != 7 || args.iter().any(String::is_empty) {
...
fn usage() -> &'static str {
    "usage: verify_structural_tree <leaf0.receipt.json> <leaf1.receipt.json> <leaf2.receipt.json> <leaf3.receipt.json> <l1-left.receipt.json> <l1-right.receipt.json> <l2-root.receipt.json>"
}
...
mod tests {
    ...
    use super::{parse_options, read_bounded_regular_file, MAX_RECEIPT_READ_BYTES_U64};
