use zenodex_global_settlement_abi_v1::{
    canonical_bytes_v1, RootV1, ZDEXAMMPurchaseJournalV1, ZDEXAmountBucketV1, ZDEXBurnRejectCodeV1,
...
use zenodex_zdex_hyperdeflation_burn_risc0_shared::{
    canonical_zdex_hyperdeflation_burn_guest_input_bytes_v1,
    prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1, prepare_zdex_hyperdeflation_burn_v1,
    ZDEXHyperdeflationBurnGuestErrorV1, ZDEXHyperdeflationBurnGuestInputV1,
...
fn root_hex(value: &str) -> RootV1 {
    RootV1::parse(value, "ZDEX burn guest golden root", false).unwrap()
}

fn guest_input(source_atoms: u128) -> ZDEXHyperdeflationBurnGuestInputV1 {
...
#[test]
fn exact_core_and_refinement_commit_only_the_canonical_burn_journal() {
    // Arrange
    let input = guest_input(100);
    let input_bytes = canonical_zdex_hyperdeflation_burn_guest_input_bytes_v1(&input).unwrap();

    // Act
    let prepared = prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&input_bytes).unwrap();

    // Assert
    assert_eq!(prepared.input, input);
    assert_eq!(prepared.projection.journal().burned_zdex_atoms, 100);
    assert_eq!(
        prepared
            .projection
            .journal()
    ...
    assert_eq!(
        ...
        canonical_bytes_v1(prepared.projection.journal()).unwrap()
    );
}

#[test]
fn economic_rejection_and_partial_route_drain_produce_no_guest_journal() {
    // Arrange
    let mut over_capacity = guest_input(100);
    over_capacity.route_context.purchased_zdex_atoms = 101;
    over_capacity.command.purchased_zdex_atoms = 101;
    let partial_drain = guest_input(150);

    // Act / Assert
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_v1(over_capacity),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Rejected(
            ZDEXBurnRejectCodeV1::PURCHASE_EXCEEDS_BURN_CAPACITY
        ))
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_v1(partial_drain),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Refinement)
    ));
}
...
fn empty_oversized_unknown_schema_and_noncanonical_inputs_fail_closed() {
    // Arrange
    let canonical =
        canonical_zdex_hyperdeflation_burn_guest_input_bytes_v1(&guest_input(100)).unwrap();
    ...
    // Act / Assert
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&[]),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::EmptyInput)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(b"{"),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Decode)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&at_limit),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Decode)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&oversized),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::InputTooLarge)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&unknown),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Decode)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&wrong_schema),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::Schema)
    ));
    assert!(matches!(
        prepare_zdex_hyperdeflation_burn_from_canonical_bytes_v1(&trailing),
        Err(ZDEXHyperdeflationBurnGuestErrorV1::NonCanonicalInput)
    ));
}
