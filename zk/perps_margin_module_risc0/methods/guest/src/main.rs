...
use zenodex_perps_margin_module_risc0_shared::{
    ...
    MAX_PERPS_MARGIN_MODULE_GUEST_INPUT_BYTES_U32_V1,
};
...
pub fn main() {
    ...
    if input_len == 0 || input_len > MAX_PERPS_MARGIN_MODULE_GUEST_INPUT_BYTES_U32_V1 {
        abort("perps margin module guest input length unsupported");
    }
    let input_len = match usize::try_from(input_len) {
        Ok(value) => value,
        Err(_) => abort("perps margin module guest input length conversion failed"),
    };
    let mut input_bytes = vec![0_u8; input_len];
    env::read_slice(&mut input_bytes);
    let prepared = match prepare_perps_margin_module_from_canonical_bytes_v1(&input_bytes) {
        Ok(value) => value,
