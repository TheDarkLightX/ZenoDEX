...
fn write_placeholder(methods_rs: &std::path::Path) {
    let stub = r#"
// Test-only placeholder. The real-proof test rejects both values.
pub const ZENODEX_ROUTE_STRUCTURAL_TEST_LEAF_ELF: &[u8] = &[];
pub const ZENODEX_ROUTE_STRUCTURAL_TEST_LEAF_ID: [u32; 8] = [0; 8];
"#;
