...
use zenodex_zdex_fee_allocation_risc0_host::{
    prove_zdex_fee_allocation_succinct_v1, zdex_fee_allocation_image_root_v1,
};
...
use zenodex_zdex_tokenomics_lane_coordinator_risc0_host::{
    ...
    verify_child_fee_allocation_receipt_v1, verify_zdex_tokenomics_lane_coordinator_receipt_v1,
...
#[test]
#[ignore = "generates real fee-allocation and recursive coordinator Succinct receipts"]
fn real_recursive_fee_coordinator_proves_the_exact_complete_lane_journal() {
    // Arrange
    assert!(!ZENODEX_ZDEX_TOKENOMICS_LANE_COORDINATOR_GUEST_ELF.is_empty());
    assert_ne!(ZENODEX_ZDEX_TOKENOMICS_LANE_COORDINATOR_GUEST_ID, [0; 8]);
    let child_image = zdex_fee_allocation_image_root_v1().unwrap();
    let fixture = fee_fixture(child_image.clone());
    let prepared =
        prepare_zdex_tokenomics_fee_lane_coordinator_v1(fixture.coordinator_input.clone()).unwrap();
    let child_receipt = prove_zdex_fee_allocation_succinct_v1(&fixture.child_input).unwrap();
    let started = Instant::now();

    // Act
    let receipt = prove_zdex_tokenomics_fee_lane_coordinator_succinct_v1(
        &fixture.coordinator_input,
        &child_receipt,
    )
    .unwrap();
    ...
    assert!(matches!(&child_receipt.inner, InnerReceipt::Succinct(_)));
    assert!(matches!(&receipt.inner, InnerReceipt::Succinct(_)));
    assert_eq!(child_receipt.journal.bytes, prepared.child_journal_bytes);
    assert_eq!(receipt.journal.bytes, prepared.lane_journal_bytes);
    PinnedZDEXTokenomicsLaneCoordinatorReceiptVerifierV1
        .verify_succinct_receipt(&receipt_bytes, &image_root, &prepared.lane_journal_bytes)
        .unwrap();

    let mut wrong_child_journal = prepared.child_journal_bytes.clone();
    wrong_child_journal[0] ^= 1;
    assert!(matches!(
        verify_child_fee_allocation_receipt_v1(&child_receipt, &child_image, &wrong_child_journal,),
        Err(ZDEXTokenomicsLaneCoordinatorHostErrorV1::ChildReceiptJournal)
    ));
    assert!(matches!(
    ...
    assert!(build_zdex_tokenomics_fee_lane_coordinator_executor_env_v1(
        &fixture.coordinator_input,
        &child_receipt,
    )
    .is_ok());

    println!("ZDEX tokenomics fee coordinator image root: {image_root}");
    println!("ZDEX tokenomics fee coordinator real proof elapsed: {elapsed:?}");
}
