...
mod jmt_storage_update_plan_v1 {
    ...
    pub const MAX_JMT_STORAGE_NEW_NODES_V1: usize =
        MAX_SPARSE_MERKLE_BATCH_ENTRIES_V1 * (JMT_NIBBLE_PATH_MAX_NIBBLES_V1 as usize + 1);
    ...
    impl ValidatedJmtStorageUpdatePlanV1 {
        ...
        pub fn validate_self_consistency(&self) -> Result<(), JmtStorageUpdatePlanErrorV1> {
            ...
            let expected_new_nodes = materialize_new_nodes(&derived.post, self.target_version)?;
            validate_supplied_new_nodes(&self.new_nodes, &expected_new_nodes, self.target_version)?;
    ...
    pub fn encode_jmt_storage_update_plan_v1(
        ...
        plan.validate_self_consistency()?;
        let bytes =
            postcard::to_allocvec(plan).map_err(|_| JmtStorageUpdatePlanErrorV1::PostcardEncode)?;
    ...
    pub fn decode_exact_jmt_storage_update_plan_v1(
        ...
        let (plan, remainder) = postcard::take_from_bytes::<ValidatedJmtStorageUpdatePlanV1>(bytes)
    ...
    fn derive_witness_boundary_nodes(
        ...
        nodes
            ...
            .map_err(|_| JmtStorageUpdatePlanErrorV1::AllocationFailed("boundary_nodes"))?;
        ...
        nodes.push((
            JmtNibblePathV1::from_key_prefix(key_bytes, JMT_NIBBLE_PATH_MAX_NIBBLES_V1)?,
            current,
        ));
        ...
        for depth in (0..SPARSE_MERKLE_TREE_DEPTH_V1).rev() {
            ...
            if depth % 4 == 0 {
                let nibble_count = u8::try_from(depth / 4).map_err(|_| {
                    JmtStorageUpdatePlanErrorV1::ArithmeticOverflow("nibble_boundary")
                })?;
                nodes.push((
        ...
        if current != expected_root {
            return Err(JmtStorageUpdatePlanErrorV1::BoundaryRootMismatch(phase));
        }
    ...
    fn materialize_new_nodes(
        ...
        let mut nodes = Vec::new();
        nodes
            .try_reserve_exact(post.len())
            .map_err(|_| JmtStorageUpdatePlanErrorV1::AllocationFailed("derived_new_nodes"))?;
    ...
    fn validate_supplied_new_nodes(
        ...
        for (offset, pair) in actual.windows(2).enumerate() {
            ...
            match compare_node_keys(pair[0].node_key(), pair[1].node_key()) {
                Ordering::Equal => {
                    return Err(JmtStorageUpdatePlanErrorV1::DuplicateNewNodeKey { index });
                }
                Ordering::Greater => {
                    return Err(JmtStorageUpdatePlanErrorV1::NonCanonicalNewNodeOrder { index });
                }
                Ordering::Less => {}
            }
        }
        ...
        for (index, (actual_node, expected_node)) in actual.iter().zip(expected).enumerate() {
            if actual_node != expected_node {
                return Err(JmtStorageUpdatePlanErrorV1::NewNodeMismatch { index });
            }
        }
        Ok(())
    }
    ...
    fn validate_stale_nodes(
        ...
        for (index, stale) in nodes.iter().enumerate() {
            ...
            if actual != target_version {
                return Err(JmtStorageUpdatePlanErrorV1::StaleSinceVersionMismatch {
                    index,
                    actual,
                    expected: target_version,
                });
        ...
        for (offset, pair) in nodes.windows(2).enumerate() {
            ...
            {
                Ordering::Equal => {
                    return Err(JmtStorageUpdatePlanErrorV1::DuplicateStalePath { index });
                }
                Ordering::Greater => {
                    return Err(JmtStorageUpdatePlanErrorV1::NonCanonicalStaleNodeOrder { index });
                }
                Ordering::Less => {}
            }
        }
        for (index, stale) in nodes.iter().enumerate() {
            ...
            let expected = base_nodes
                .get(&path)
                .ok_or(JmtStorageUpdatePlanErrorV1::UntouchedStalePath { index })?;
            if stale.expected_node_hash() != *expected {
                return Err(JmtStorageUpdatePlanErrorV1::StaleNodeHashMismatch { index });
            }
        }
        Ok(())
    }
        ...
        impl<'de> Visitor<'de> for NewNodeVisitor {
                ...
                A: SeqAccess<'de>,
            {
                ...
                nodes.try_reserve_exact(declared).map_err(|_| {
                    de::Error::custom(JmtStorageUpdatePlanErrorV1::AllocationFailed("new_nodes"))
                })?;
    ...
    fn deserialize_stale_nodes<'de, D>(
        deserializer: D,
    ) -> Result<Vec<JmtStaleNodeIndexV1>, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for StaleNodeVisitor {
                ...
                A: SeqAccess<'de>,
            {
                ...
                let mut nodes = Vec::new();
                nodes.try_reserve_exact(declared).map_err(|_| {
                    de::Error::custom(JmtStorageUpdatePlanErrorV1::AllocationFailed("stale_nodes"))
                })?;
                for index in 0..declared {
                    nodes.push(
                        sequence
