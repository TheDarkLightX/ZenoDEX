...
claim families; Wave 3 records six perps timing, keeper, and cascade obligations; Wave 4
records six verification-market obligations across proof mining and
permissionless-hosting bounties. Each row names the claim, the evidence, and the
surface a separately reviewed change would touch. Replay for the strategic
deviation waves:

```bash
python3 -m pytest experiments/mechanism_design_math_v1/wave2_sealed_bid/ -q
python3 -m pytest experiments/mechanism_design_math_v1/wave3_perp_timing/ -q
...
|---|---|---|
| `O-SS-01` | claim "truthful floor reporting is dominant under greedy priority" FALSIFIED for the tested greedy_ab witness: Alice raises executable `min_out` from 0 to 9000, flips from second to first, and output rises from 9706 to 9900 | decide whether declared surplus should buy priority; candidate surfaces are the greedy AB priority key and any user-facing guidance that claims min_out is only protection |
...
| `O-SS-04` | claim "ties are harmless" FALSIFIED: identical 10000 exact-in swaps have tie value 194 output units because lower `intent_id` executes first | use a post-admission tie salt, canonical batch-local ordering, or make tie-value disclosure explicit; surface: spot ordering tie-break rules |
| `O-SS-05` | claim "settlement is continuous in batch size" FALSIFIED: one rejected 1-unit dust intent across `_MAX_SWAP_ORDERING_BRUTE_FORCE_N` silently switches `optimal_ab_bounded` to `limit_price` ordering, flipping a victim from fill 47619 to REJECT and dropping the settled (A, B) from (50000, 619) to (10000, 100); the production 12-to-13 crossing is bound branch-by-branch because the 12-intent brute-force side would enumerate 12! orderings | make the regime switch explicit instead of silent: reject or split batches above the bound, record the ordering mode actually used in the settlement receipt, or admission-price dust intents so cardinality is costly to inflate; surface: `_order_swaps_optimal_ab_bounded` fallback policy in `src/core/batch_clearing.py` |
| `O-SS-06` | claim "CoW netting is neutral for LPs" FALSIFIED in the self-counterpair witness: netting pays zero fees and no reserve movement while pool execution pays fee 2 and worse outputs | decide whether self-netting is allowed, fee-adjusted, or restricted to independent counterparties; surface: `cow_pair_netting_v1` admission/matching rules |
...
| `O-PT-01` | funding timing is not neutral in the single-account gate: immediate funding is zero when flat, and `set_position` can enter after `funding_last_applied_epoch == now_epoch` | decide whether funding accrues by time-in-position, by epoch snapshot, or by global event receipt; surface: `src/core/perp_v2/guards.py` and the funding apply gate |
| `O-PT-02` | PRICE_PUBLISHED blocks voluntary `set_position`; in the tested non-liquidatable state, allowed funding and settlement steps keep `position_base` unchanged | keep this as regression evidence for any phase-gate rewrite; broaden with action-sequence model checking before changing `EpochPhase` transitions |
| `O-PT-03` | bounded keeper-race cost surface shows priority-gas all-pay traces can dissipate nearly the full reward | prefer deterministic keeper assignment, commit-reveal keeper lotteries, or protocol keeper backstops over raw priority-gas races; surface: liquidation execution policy |
| `O-PT-04` | under an explicit one-sided book-impact model, real per-account perp transitions admit bounded price-mediated cascades: K=0 has an exact one-unit threshold, a 4-account chain reaches K=3, and depth weakly decreases over the swept book-depth grid; the production core has no multi-account book coupling in this branch | make any future cross-account perps runtime expose the price-impact depth and liquidation headroom assumptions as checked parameters; keep insurance as a separate residual-risk mechanism because the inspected kernel receives liquidation penalties but does not automatically spend insurance to absorb cascade losses |
| `O-PT-05` | `fraction_bps` is a value-transfer lever: auto minimum 181 bps transfers penalty 9 while full liquidation is also legal and transfers 500 in the witness | prefer the auto minimum as the default or cap caller-specified fractions to the minimum plus a reviewed safety margin; surface: partial liquidation caller API |
