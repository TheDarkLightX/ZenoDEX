# Mechanism Design Math Crosswalk

This file binds the charter obligations in
`docs/ZENODEX_MECHANISM_DESIGN_AND_MATH.md` to their evidence IDs and current
artifact status.

| Obligation | Hypothesis ID(s) | Domain | Artifact / status |
|---|---|---|---|
| `O-SS-01` | `H-MD-SS-001` | spot settlement | `wave1_spot_settlement/test_spot_strategy_deviations.py`; supported |
| `O-SS-02` | `H-MD-SS-002` | spot settlement | `wave1_spot_settlement/test_spot_fee_arithmetic.py`; supported |
| `O-SS-03` | `H-MD-SS-003` | spot settlement | queued |
| `O-SS-04` | `H-MD-SS-004` | spot settlement | `wave1_spot_settlement/test_spot_strategy_deviations.py`; supported |
| `O-SS-05` | `H-MD-SS-005`, `H-MD-SS-006` | spot settlement | `wave1_spot_settlement/test_spot_cardinality_cliff.py`; supported |
| `O-SS-06` | `H-MD-SS-007` | spot settlement | `wave1_spot_settlement/test_spot_strategy_deviations.py`; supported |
| `O-SS-07` | `H-MD-SS-008` | spot settlement | `wave1_spot_settlement/test_spot_fee_arithmetic.py`; supported |
| `O-SB-01` | `H-MD-SB-001` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-SB-02` | `H-MD-SB-002` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-SB-03` | `H-MD-SB-003` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-SB-04` | `H-MD-SB-004` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-SB-05` | `H-MD-SB-005` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-SB-06` | `H-MD-SB-006` | sealed bid | `wave2_sealed_bid/test_sealed_bid_deviations.py`; supported |
| `O-PT-01` | `H-MD-PT-001`, `H-MD-PT-005` | perps timing | `wave3_perp_timing/test_perp_timing_and_keeper_deviations.py`; supported |
| `O-PT-02` | `H-MD-PT-002` | perps timing | `wave3_perp_timing/test_perp_timing_and_keeper_deviations.py`; supported |
| `O-PT-03` | `H-MD-PT-003` | perps timing | `wave3_perp_timing/test_perp_timing_and_keeper_deviations.py`; supported |
| `O-PT-04` | `H-MD-PT-004` | perps timing | queued |
| `O-PT-05` | `H-MD-PT-006` | perps timing | `wave3_perp_timing/test_perp_timing_and_keeper_deviations.py`; supported |
| `O-VM-01` | `H-MD-VM-001` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
| `O-VM-02` | `H-MD-VM-002` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
| `O-VM-03` | `H-MD-VM-003` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
| `O-VM-04` | `H-MD-VM-004` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
| `O-VM-05` | `H-MD-VM-005` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
| `O-VM-06` | `H-MD-VM-006` | verification markets | `wave4_verification_markets/test_verification_market_deviations.py`; supported |
