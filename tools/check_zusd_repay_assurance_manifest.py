...
def _git_stdout(*args: str) -> str:
    ...
    try:
        ...
        raise ManifestError("git is required for zUSD repay manifest checks") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the pinned zUSD repay assurance manifest.")
    ...
    _require(esso_head == toolchain["esso_code_hash"], "ESSO code hash drifted from zUSD repay manifest")
    _require(esso_tree == toolchain["esso_tree_sha256"], "ESSO tree drifted from zUSD repay manifest")
    ...
    tests = manifest.get("adapter_regression_tests")
    _require(isinstance(tests, list) and tests, "adapter_regression_tests must be a non-empty list")
    for rel in tests:
