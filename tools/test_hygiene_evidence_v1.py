...
def _validate_boundaries(raw: object, *, context: str, required: bool) -> None:
    ...
    if required:
        require(
            bool(rows),
            f"{context.rsplit('.', 1)[0]}: boundary evidence requires dimensions",
        )
...
def _validate_mutations(
    ...
    *,
    ...
    if required:
        require(
            bool(rows),
            f"{context.rsplit('.', 1)[0]}: mutation evidence requires named mutants",
        )
    for index, item in enumerate(rows):
        item_context = f"{context}[{index}]"
        row = object_value(item, context=item_context)
        exact_fields(
            row, frozenset({"description", "killed_by"}), context=item_context
        )
...
def load_packet(path: Path, contract: ContractV1) -> PacketV1:
    ...
    require(
        families <= contract.allowed_families,
        f"{context}: unsupported evidence family",
    )
