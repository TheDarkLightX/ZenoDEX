...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/quote_receipt_totals_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.quote_receipt_totals_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/quote_receipt_totals_gate_v1"
mkdir -p "$VERIFY_ROOT"
...
"$PY" -m pytest -q tests/core/test_quote_receipt_totals_gate.py tests/core/test_quote_receipts.py tests/kernels/test_quote_receipt_totals_gate_v1_native_adapter.py
"$PY" "$ROOT_DIR/tools/check_quote_receipt_totals_gate_assurance_manifest.py"
