...
from src.fire.kernel.kernel_settlement_receipt_v1 import verify_fire_kernel_settlement_receipt_file  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fail-closed checker for FIRE kernel_settlement_receipt.json files.")
    parser.add_argument("--receipt-file", type=Path, required=True, help="Path to kernel_settlement_receipt.json")
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Optional path to kernel_eval_receipt.json; its sha256 will be used as an expected pin",
    )
    parser.add_argument("--expected-receipt-sha256", help="Optional expected SHA-256 of kernel_settlement_receipt.json")
    ...
    parser.add_argument(
        "--expected-kernel-eval-receipt-sha256",
        help="Optional expected SHA-256 of kernel_eval_receipt.json",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.replay_input_file is not None and expected_replay_input_sha256 is None:
        expected_replay_input_sha256 = _sha256_file(args.replay_input_file.resolve())
    ...
    ok, err, verification = verify_fire_kernel_settlement_receipt_file(
        args.receipt_file,
        ...
        expected_replay_input_sha256=expected_replay_input_sha256,
    ...
    if not ok or verification is None:
        payload = {
            "schema": "zenodex/fire-kernel-settlement-receipt-check-report/v1",
            "ok": False,
            "receipt_path": str(args.receipt_file.resolve()),
            "error": err or "kernel_settlement_receipt_verification_failed",
        }
