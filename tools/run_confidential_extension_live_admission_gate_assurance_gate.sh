...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/confidential_extension_live_admission_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.confidential_extension_live_admission_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/confidential_extension_live_admission_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== confidential-live-admission-gate: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== confidential-live-admission-gate: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== confidential-live-admission-gate: verify-shell =="
...
echo "== confidential-live-admission-gate: verify-multi =="
...
echo "== confidential-live-admission-gate: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_confidential_extension_live_admission_gate_v1_native_adapter.py

echo "== confidential-live-admission-gate: manifest check =="
"$PY" "$ROOT_DIR/tools/check_confidential_extension_live_admission_gate_assurance_manifest.py"
