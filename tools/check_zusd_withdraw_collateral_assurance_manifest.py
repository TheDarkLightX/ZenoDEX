...
DEFAULT_MANIFEST = REPO_ROOT / "tools" / "zusd_withdraw_collateral_assurance_manifest.json"
...
def _git_stdout(*args: str) -> str:
    ...
    try:
        ...
        raise ManifestError("git is required for zUSD withdraw-collateral manifest checks") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check the pinned zUSD withdraw-collateral assurance manifest.")
    parser.add_argument("--manifest", default=str(DEFAULT_MANIFEST), help="Path to zUSD withdraw-collateral assurance manifest JSON")
    ...
    _require(
        int(manifest.get("manifest_version", 0)) == 1,
        "unsupported zUSD withdraw-collateral assurance manifest version",
    )
    ...
    _require(esso_head == toolchain["esso_code_hash"], "ESSO code hash drifted from zUSD withdraw-collateral manifest")
    _require(esso_tree == toolchain["esso_tree_sha256"], "ESSO tree drifted from zUSD withdraw-collateral manifest")
