...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/dex_intent_auth_shape_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.dex_intent_auth_shape_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/dex_intent_auth_shape_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== dex-intent-auth-shape: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== dex-intent-auth-shape: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== dex-intent-auth-shape: verify-shell =="
...
echo "== dex-intent-auth-shape: verify-multi =="
...
echo "== dex-intent-auth-shape: regression net =="
"$PY" -m pytest -q \
...
echo "== dex-intent-auth-shape: manifest check =="
"$PY" "$ROOT_DIR/tools/check_dex_intent_auth_shape_assurance_manifest.py"
