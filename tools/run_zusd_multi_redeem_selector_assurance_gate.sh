...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_multi_redeem_selector_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_multi_redeem_selector_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_multi_redeem_selector_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-multi-redeem-selector: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-multi-redeem-selector: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-multi-redeem-selector: verify-shell =="
...
echo "== zusd-multi-redeem-selector: verify-multi =="
...
echo "== zusd-multi-redeem-selector: regression net =="
"$PY" -m pytest -q \
  tests/core/test_zusd_multi_redeem_selector.py \
  tests/kernels/test_zusd_multi_redeem_selector_v1_native_adapter.py

echo "== zusd-multi-redeem-selector: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_multi_redeem_selector_assurance_manifest.py"
