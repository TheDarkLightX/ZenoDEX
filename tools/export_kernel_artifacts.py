...
This repo uses an optional spec toolchain (ESSO: Evolutionary State Space Optimizer)
to validate/verify kernel specs and export standalone reference implementations.
...
ESSO_ACRONYM = "Evolutionary State Space Optimizer"
...
def _ensure_esso() -> None:
    if not ESSO_ROOT.exists():
        raise SystemExit(f"ESSO not found at {ESSO_ROOT}. Clone/update external/ESSO.")
...
def _patch_esso_header(python_out_dir: Path) -> None:
    generated_by_new = (
        f"Generated from the YAML kernel spec by the repo's optional spec toolchain "
        f"(ESSO: {ESSO_ACRONYM})."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for ex in exports:
        ...
        if not args.no_python:
            ...
            _patch_esso_header(ex.python_out_dir)
