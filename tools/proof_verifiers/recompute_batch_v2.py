#!/usr/bin/env python3
"""
Reference proof verifier: "recompute batch" certificate (v2, compressed witness).

Like v1, but allows embedding the witness (pre-state snapshot + ops) in a
compressed form inside the proof object to reduce proof payload size.

Proof schema (inside operations['3']['proof']):
  {
    "scheme": "recompute_batch_v2",
    "pre_state_commitment": "0x..",
    "batch_commitment": "0x..",
    # Witness (choose one form per field):
    "pre_state_snapshot": {...} OR "pre_state_snapshot_zlib_b64": "....",
    "operations": {"2":[...], "3": {...}} OR "operations_zlib_b64": "...."
  }
"""
...
MAX_DECOMPRESSED_SNAPSHOT_BYTES = 4_000_000
...
def _b64decode(value: str, *, name: str, max_bytes: int = 1_000_000) -> bytes:
...
def _settlement_commitment_dict(op3_obj: Any) -> Dict[str, Any]:
    out: Dict[str, Any] = {k: v for k, v in dict(op3_obj).items() if k not in ("batch_ref", "events")}
    fills = out.get("fills")
...
def _load_witness(proof: Mapping[str, Any]) -> Tuple[Mapping[str, Any], Mapping[str, Any]]:
    # Snapshot can be embedded as object or zlib(base64(canonical_json_bytes(snapshot))).
...
def _verify(payload: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        op3 = create_settlement_operation(claimed_settlement).get("3")
        if not isinstance(op3, dict):
            raise TypeError("settlement operation must be an object")
        settlement_commit_obj = _settlement_commitment_dict(op3)
