#!/usr/bin/env python3
"""
Reference proof verifier: "recompute batch" certificate (v3, projected pre-state).

v3 switches the pre-state commitment from the full global `state_root` to a
*support root* computed only over the batch's read-set. This allows the proof
witness to carry only a projected snapshot of the state while still binding the
certificate to the relevant pre-state.

Proof schema (inside operations['3']['proof']):
  {
    "scheme": "recompute_batch_v3",
