...
#   4. re-check P with the packet checker (replay) and the builder round trip;
#   5. push and tag.
# Every Lean-bearing step takes the shared lock. Any red step stops the chain before P exists.
#
...
echo "builder check exit $?"
git push -q origin "$BRANCH" && echo pushed
git tag "$TAG" "$P" && echo "tagged $TAG"
