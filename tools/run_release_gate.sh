...
#   0 = every in-scope surface clear                          -> continue
#   1 = blocked claim (a regression after reviewed closure)    -> FAIL CLOSED
#   2+ = structural/infrastructure error (missing/corrupt registry, import, etc.)
#                                                              -> FAIL CLOSED
# REVIEW [A- -> A]: exit 1 used to be advisory while the matrix was still under
# promotion. Now production_security_claim=true is the reviewed baseline, so the
# release gate must fail if any in-scope surface regresses to an uncleared row.
echo "== release: CBC production-claim status =="
cbc_status=0
"$PY" "$ROOT_DIR/tools/gate_cbc_matrix_closure.py" || cbc_status=$?
case "$cbc_status" in
  0) ;;
  1) echo "error: CBC production claim regressed to false — see the matrix above" >&2
     exit 1 ;;
