#!/usr/bin/env python3
"""Replay the sampled n=8 AB child-frontier bidirectional transition Tau certificate."""
...
SPEC_ID = "ab_child_frontier_bidirectional_transition_n8_sample_scope_certificate_v1"
...
REPORT_MD = (
    ...
    / "docs"
    / "research"
    / "ZENODEX_AB_CHILD_FRONTIER_BIDIRECTIONAL_TRANSITION_N8_SAMPLE_TAU_CERTIFICATE_20260629.md"
)
...
EXPECTED_SOURCE_REPORT_HASH = (
    "de633b40c90942b466750a24b76b6379a5a3322d54925c1801a2f5dbd8b0fd24"
)
...
EXPECTED_LINKED_CHILD_COVERAGE_COUNT = 88
EXPECTED_LINKED_CANONICAL_MEMBERSHIP_COUNT = 88
...
def _fact_bundle(report: Mapping[str, Any]) -> dict[str, int]:
    ...
    linked_witness = search.get("linked_witness_summary", {})
    linked_merkle = search.get("linked_canonical_merkle_summary", {})
    ...
    sampled_n8_zero_min_scope_ok = _contains_all(
        non_claims_text,
        (
            "bounded to the deterministic sampled n=8 corpus",
    ...
    transition_counts_complete = (
        ...
        and int(search.get("expected_transition_count", -1))
        == EXPECTED_TRANSITION_ROW_COUNT
        and int(search.get("covered_transition_count", -1))
        == EXPECTED_TRANSITION_ROW_COUNT
    ...
    linked_child_coverage_ok = (
        isinstance(linked_witness, Mapping)
        and bool(linked_witness.get("available")) is True
        and bool(linked_witness.get("ok")) is True
        and linked_witness.get("schema")
        == "zenodex.ab_reserve_state_child_frontier_witness_compression_n8_sample_report.v1"
        and int(linked_witness.get("case_count", -1)) == EXPECTED_CASE_COUNT
        and int(linked_witness.get("valid_case_count", -1)) == EXPECTED_CASE_COUNT
        and int(linked_witness.get("sampled_child_mask_count", -1))
        == EXPECTED_SAMPLED_CHILD_MASK_COUNT
        and int(linked_witness.get("witness_count", -1))
        == EXPECTED_LINKED_CHILD_COVERAGE_COUNT
        and int(linked_witness.get("predecessor_transition_count", -1))
        == EXPECTED_TRANSITION_ROW_COUNT
        and int(linked_witness.get("negative_control_accept_count", -1)) == 0
        and int(search.get("linked_child_coverage_witness_count", -1))
        == EXPECTED_LINKED_CHILD_COVERAGE_COUNT
    )
    linked_canonical_membership_ok = (
        isinstance(linked_merkle, Mapping)
        and bool(linked_merkle.get("available")) is True
        and bool(linked_merkle.get("ok")) is True
        and linked_merkle.get("schema")
        == "zenodex.ab_reserve_state_child_frontier_canonical_merkle_n8_sample_report.v1"
        and int(linked_merkle.get("case_count", -1)) == EXPECTED_CASE_COUNT
        and int(linked_merkle.get("valid_case_count", -1)) == EXPECTED_CASE_COUNT
        and int(linked_merkle.get("sampled_child_mask_count", -1))
        == EXPECTED_SAMPLED_CHILD_MASK_COUNT
        and int(linked_merkle.get("frontier_root_count", -1))
        == EXPECTED_SAMPLED_CHILD_MASK_COUNT
        and int(linked_merkle.get("sampled_child_state_count", -1))
        == EXPECTED_LINKED_CANONICAL_MEMBERSHIP_COUNT
        and int(linked_merkle.get("membership_count", -1))
        == EXPECTED_LINKED_CANONICAL_MEMBERSHIP_COUNT
        and int(linked_merkle.get("negative_control_accept_count", -1)) == 0
        and int(search.get("linked_canonical_membership_count", -1))
        == EXPECTED_LINKED_CANONICAL_MEMBERSHIP_COUNT
    )
    transition_digest_pinned = (
        search.get("transition_rows_digest") == EXPECTED_TRANSITION_ROWS_DIGEST
    )
    linked_witness_digest_pinned = (
        isinstance(linked_witness, Mapping)
        and linked_witness.get("witness_rows_digest") == EXPECTED_LINKED_WITNESS_DIGEST
    )
    linked_merkle_digest_pinned = (
        isinstance(linked_merkle, Mapping)
        and linked_merkle.get("membership_rows_digest") == EXPECTED_LINKED_MERKLE_DIGEST
    )
    ...
    source_report_hash_pinned = _sha256(SOURCE_REPORT) == EXPECTED_SOURCE_REPORT_HASH
    corpus_nonvacuous = (
        ...
        and int(search.get("linked_canonical_membership_count", 0)) > 0
    )
    return {
        ...
        "linked_canonical_membership_ok": int(linked_canonical_membership_ok),
        "transition_digest_pinned": int(transition_digest_pinned),
        "linked_witness_digest_pinned": int(linked_witness_digest_pinned),
        "linked_merkle_digest_pinned": int(linked_merkle_digest_pinned),
        ...
        "source_report_hash_pinned": int(source_report_hash_pinned),
    }
...
def _tau_cases(facts: Mapping[str, int]) -> tuple[TauCase, ...]:
    pass_step = {
        ...
        "i7": int(facts["linked_canonical_membership_ok"]),
        "i8": int(facts["transition_digest_pinned"]),
        "i9": int(facts["linked_witness_digest_pinned"]),
        "i10": int(facts["linked_merkle_digest_pinned"]),
        "i11": int(facts["deterministic_replay_ok"]),
        "i12": int(facts["negative_controls_reject"]),
        "i13": int(facts["authority_boundary_ok"]),
        "i14": int(facts["no_authority_effect"]),
        "i15": int(facts["corpus_nonvacuous"]),
        "i16": int(facts["source_report_hash_pinned"]),
    }
    inactive = dict(pass_step)
    inactive["i1"] = 0
    return (
        TauCase(
            "bidirectional_transition_n8_sample_certificate_pass",
            pass_step,
            {
                "o1": 1,
                "o2": 1,
            ...
            "All scoped host facts admit the sampled n=8 bidirectional transition certificate.",
        ),
        TauCase(
            "missing_source_report_reject",
            {**pass_step, "i2": 0},
            {"o1": 0, "o7": 0},
            "The source bidirectional transition report must be present, valid, and successful.",
        ),
        TauCase(
            "wrong_scope_reject",
            {**pass_step, "i3": 0},
        ...
        TauCase(
            ...
            {**pass_step, "i4": 0},
            {"o2": 0, "o7": 0},
            "The 268 transition rows must exactly cover all expected sampled transitions.",
        ),
        TauCase(
            "generated_child_count_reject",
            {**pass_step, "i5": 0},
            {"o2": 0, "o7": 0},
            "The transition rows must bind the 88 generated child states.",
        ),
        TauCase(
            "linked_child_coverage_reject",
            {**pass_step, "i6": 0},
            {"o3": 0, "o7": 0},
            "The child coverage direction must remain linked to the predecessor-witness report.",
        ),
        TauCase(
            "linked_canonical_membership_reject",
            {**pass_step, "i7": 0},
            {"o3": 0, "o7": 0},
            "Every generated child state must remain linked to canonical-Merkle membership.",
        ),
        TauCase(
        ...
        TauCase(
            ...
            {**pass_step, "i9": 0},
            {"o5": 0, "o7": 0},
            "The linked predecessor-witness digest must remain pinned.",
        ),
        TauCase(
            "linked_merkle_digest_reject",
            {**pass_step, "i10": 0},
            {"o5": 0, "o7": 0},
            "The linked canonical-membership digest must remain pinned.",
        ),
        TauCase(
            "nondeterministic_replay_reject",
            {**pass_step, "i11": 0},
            {"o4": 0, "o7": 0},
            "The sampled n=8 bidirectional checker replay must remain deterministic.",
        ),
        TauCase(
        ...
        TauCase(
            ...
            {**pass_step, "i15": 0},
            {"o1": 0, "o7": 0},
            "The certificate must bind a nonempty sampled transition corpus.",
        ),
        TauCase(
            "source_hash_reject",
            {**pass_step, "i16": 0},
            {"o1": 0, "o7": 0, "o9": 0},
            "The source report hash must match the pinned source packet.",
        ),
        TauCase(
            "inactive_safe",
            inactive,
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "transition_corpus": {
            ...
            "linked_witness_rows_digest": linked_witness.get("witness_rows_digest")
            if isinstance(linked_witness, Mapping)
            else None,
            "linked_merkle_membership_rows_digest": linked_merkle.get(
                "membership_rows_digest"
            )
            if isinstance(linked_merkle, Mapping)
            else None,
        ...
        "linked_reports": {
            "predecessor_witness": linked_witness
            if isinstance(linked_witness, Mapping)
            else {},
            "canonical_merkle": linked_merkle if isinstance(linked_merkle, Mapping) else {},
        },
        "facts": facts,
        "tau": tau,
        "breakthrough": {
            "name": "AB child-frontier bidirectional transition n8 sample Tau certificate",
            "spec_id": SPEC_ID,
            "tau_cases": len(tau["case_results"]),
            "invalid_accepts": tau["invalid_accepts"],
            "scoped_claims": [
                "the sampled n=8 bidirectional transition source report is present and hash-pinned",
                "268 transition rows exactly cover all expected sampled predecessor transitions",
                "88 generated child states are linked to predecessor-witness evidence",
                "88 generated child states are linked to canonical-Merkle membership evidence",
                "transition, witness, membership, and source-report digests are pinned",
                "11 mutation controls reject with zero accepts",
        ...
        "hypothesis_card": {
            "hypothesis_id": "H-AB-N8-BIDIR-TRANSITION-TAU-20260629",
            "status": "supported_bounded",
            "mechanism_change": "Add a versioned Tau scope certificate over sampled n=8 bidirectional transition closure evidence.",
            "null_hypothesis": "A Tau envelope gives no additional falsifiable boundary beyond the sampled n=8 Python transition checker.",
            "support_recipe": "Host checks the source report, linked reports, and pinned digests; Tau rejects every missing-fact negative case.",
            "falsification_recipe": "Clear each required fact bit, mutate source or linked digest pins, or remove the no-authority rail and require Tau rejection.",
...
def _write_markdown(report: Mapping[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Transition Pins",
            "",
            f"- Cases: `{report['transition_corpus']['case_count']}`",
            f"- Sampled child masks: `{report['transition_corpus']['sampled_child_mask_count']}`",
            ...
            f"- Linked predecessor witnesses: `{report['transition_corpus']['linked_child_coverage_witness_count']}`",
            f"- Linked canonical memberships: `{report['transition_corpus']['linked_canonical_membership_count']}`",
            f"- Source report hash: `{report['source_report']['sha256']}`",
            f"- Transition digest: `{report['transition_corpus']['transition_rows_digest']}`",
            f"- Linked witness digest: `{report['transition_corpus']['linked_witness_rows_digest']}`",
            f"- Linked Merkle membership digest: `{report['transition_corpus']['linked_merkle_membership_rows_digest']}`",
