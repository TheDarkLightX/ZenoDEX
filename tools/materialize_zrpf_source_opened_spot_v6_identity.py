#!/usr/bin/env python3
"""CLI for authority-neutral ZRPF Spot V6 identity materialization."""
...
from tools.zrpf_v6_identity_materialization import (
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    for name in ("check", "apply"):
        ...
        command.add_argument("--run-source-snapshot", type=Path, required=True)
        if name == "apply":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    request = MaterializationRequest(
        ...
        run_snapshot_root=args.run_source_snapshot,
    )
    try:
    ...
    except (ExecutionError, MaterializationError, OSError, planner.RebuildPlanError) as exc:
