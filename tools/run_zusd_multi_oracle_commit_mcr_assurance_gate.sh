...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_multi_oracle_commit_mcr_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_multi_oracle_commit_mcr_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_multi_oracle_commit_mcr_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-multi-oracle-commit-mcr: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-multi-oracle-commit-mcr: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-multi-oracle-commit-mcr: verify-shell =="
...
echo "== zusd-multi-oracle-commit-mcr: verify-multi =="
...
echo "== zusd-multi-oracle-commit-mcr: adapter/core regression net =="
"$PY" -m pytest -q \
...
echo "== zusd-multi-oracle-commit-mcr: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_multi_oracle_commit_mcr_assurance_manifest.py"
