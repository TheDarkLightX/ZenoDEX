...
DISASTER_SEARCH_EXPANSION_AXES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "what_if": "zUSD oracle recovery accepts a request whose committed single-vault oracle state and recovery-mode region disagree about risk state.",
        ...
        "mutation_families": (
            ...
            "single-vault redemption with stale MCR/oracle snapshot",
        ),
        "bounded_harness_ideas": (
            "pair oracle-recovery lifecycle tests with region partitions and single-vault oracle admission BVA",
        ...
        "commands": (
            (
                ...
                "tests/core/test_zusd_oracle_ingress_admission.py",
    ...
    },
    {
        ...
        "what_if": "The production zUSD monetary, Tau token, or wallet surfaces accept an action whose oracle or policy root belongs to a neighboring state.",
        "disaster_state_template": "zUSD monetary policy-valid action executes under stale oracle or wallet authority",
        "mutation_families": (
            "wallet command replay with changed token policy context",
            "zUSD monetary action at the committed oracle-policy boundary",
            "single-vault action whose token/Tau witness belongs to a stale vault state",
        ),
        "bounded_harness_ideas": (
            "combine the monetary bridge, Tau token, wallet CLI, and single-vault core tests",
            "reject every token action whose committed authority and oracle policy roots do not match current state",
        ),
        "commands": (
            (
                "pytest",
                "-q",
                "tests/integration/test_zusd_monetary_bridge.py",
    ...
    },
    {
        ...
        "mutation_families": (
            ...
            "wallet admission changes oracle bootstrap semantics compared with engine integration",
        ),
        "bounded_harness_ideas": (
        ...
        "commands": (
            (
                ...
                "tests/integration/test_perps_wallet_api.py",
            ),
        ),
    },
    {
    ...
    },
    {
        ...
        "what_if": "zUSD support-root logic and single-vault Oracle ingress admission agree locally but diverge under support/root or oracle-region drift.",
        "disaster_state_template": "support-root-valid zUSD transition admits a stale Oracle state",
        "mutation_families": (
            "support root changes after Oracle admission witness construction",
            "single-vault Oracle ingress accepts a state rejected by its native adapter",
            "zUSD support-root proof omits a risky liquidation boundary case",
        ),
        "bounded_harness_ideas": (
            "compose support-root, single-vault Oracle ingress, and liquidation partition checks",
            "reject zUSD promotion unless support, Oracle ingress, and liquidation roots agree",
        ),
        "commands": (
            (
                "pytest",
                ...
                "tests/core/test_zusd_oracle_ingress_admission.py",
                "tests/core/test_zusd_liquidation_partition.py",
            ),
        ),
    },
    {
