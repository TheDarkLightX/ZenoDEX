...
class GitObjectStoreV1:
    ...
    def commit_parents_v1(self, commit: str) -> tuple[str, ...]:
        _returncode, raw, _stderr = self.run_v1("rev-list", "--parents", "-n", "1", commit)
        value = _git_single_ascii_v1(raw, "GIT_PARENTS")
        fields = value.split(" ")
        if not fields or fields[0] != commit or any(not is_git_oid_v1(item) for item in fields):
            _input_reject("GIT_PARENTS", commit, "invalid parent record")
