...
from src.fire.compiler.compile_receipt_v1 import (  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fail-closed checker for FIRE compile receipts.")
    parser.add_argument("--receipt-file", type=Path, required=True, help="Path to compile_receipt.json")
    ...
    parser.add_argument("--expected-receipt-sha256", help="Optional expected SHA-256 of compile_receipt.json")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ok, err, verification = verify_fire_compile_receipt_file(
        args.receipt_file,
    ...
    if not ok or verification is None:
        payload = {
            ...
            "error": err or "compile_receipt_verification_failed",
        }
