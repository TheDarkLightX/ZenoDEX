...
"""State-feedback wrapper for api_server boundary concolic exploration."""
...
from tools import api_server_boundary_concolic as legacy
...
from tools.stateful_semantics import api_boundary_semantic_state, sequence_action_summary


def _trace(parser: legacy.ParserFn, payload: object, trace_files: Sequence[Path]) -> tuple[str, str, int, tuple[str, ...]]:
...
def _legacy_mutations(target: legacy.ParserTarget) -> tuple[Mutation, ...]:
...
def explore_target(
    ...
    *,
    ...
    semantic_state_fn = api_boundary_semantic_state(target.name)
    return explore_bounded_frontier(
        harness_id=f"api_server_boundary_concolic:{name}",
        seed=target.valid_seed,
        mutations=_legacy_mutations(target),
        trace_fn=lambda payload: _trace(target.parser, payload, target.trace_files),
...
def _reports_json(reports: Sequence[ExplorationTargetReport]) -> dict[str, Any]:
    return {
        "schema": "zenodex/api-server-boundary-concolic-stateful/v1",
