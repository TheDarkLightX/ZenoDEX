...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_risky_ops_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_risky_ops_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_risky_ops_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-risky-ops: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-risky-ops: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-risky-ops: verify-shell =="
...
echo "== zusd-risky-ops: verify-multi =="
...
echo "== zusd-risky-ops: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_zusd_risky_ops_gate_v1_native_adapter.py

echo "== zusd-risky-ops: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_risky_ops_assurance_manifest.py"
