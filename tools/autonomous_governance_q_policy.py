...
def _cmd_sample(args: argparse.Namespace) -> int:
    if args.next and not args.surface:
        raise SystemExit("--next requires --surface")
    ...
    if args.ebrm:
        ...
        bundle = _sample_surface_bundle(next_policy=args.next)
    else:
...
def _cmd_live_session_file_context(args: argparse.Namespace) -> int:
    try:
        ...
        if head.get("ok") is not True:
            ...
            head_pin = head.get("head_pin", {})
            head_pin_hash = (
                dict(head_pin).get("pin_hash", "")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sample.add_argument(
        "--ebrm", action="store_true", help="sample a frozen EBRM policy-step bundle"
    )
    sample.add_argument(
        "--next",
        action="store_true",
    ...
    trajectory = sub.add_parser(
        "trajectory",
        help="run a multi-step autonomous governance trajectory",
    )
    ...
    continue_trajectory.add_argument(
        "bundle",
        help="path to JSON with policy, previous_receipt, steps, and expected_policy_hash",
    )
    ...
    admit_trajectory.add_argument(
        "bundle",
        help="path to JSON with policy, trajectory_receipt, expected_policy_hash, and optional state anchors",
    )
    ...
    verify_session_store.add_argument("bundle", help="path to JSON with policy and store")
    ...
    session_store_head.add_argument("bundle", help="path to JSON store object or {store}")
    ...
    admit_session_file_continuation.add_argument(
        "bundle",
        help="path to JSON with path, policy, trajectory_receipt, and optional expected_store_hash",
    )
    ...
    verify_session_store_file.add_argument("bundle", help="path to JSON with path and policy")
    ...
    session_store_file_head.add_argument("bundle", help="path to JSON with path")
    ...
    live_session_file_context.add_argument(
        "bundle",
        help="path to JSON with path, trajectory_receipt, expected_policy_hash, and committed_surface_state",
    )
    ...
    admit_live_session_file_update.add_argument(
        "bundle",
        help="path to JSON with path, policy, trajectory_receipt, committed_surface_state, expected_policy_hash, expected_store_hash, and expected_live_context_hash",
    )
