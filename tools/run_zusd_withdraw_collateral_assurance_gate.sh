...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_withdraw_collateral_apply_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_withdraw_collateral_apply_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_withdraw_collateral_apply_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-withdraw-collateral: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-withdraw-collateral: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-withdraw-collateral: verify-shell =="
...
echo "== zusd-withdraw-collateral: verify-multi =="
...
echo "== zusd-withdraw-collateral: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_zusd_withdraw_collateral_apply_v1_native_adapter.py

echo "== zusd-withdraw-collateral: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_withdraw_collateral_assurance_manifest.py"
