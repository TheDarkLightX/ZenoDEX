...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_submission_auth_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_submission_auth_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_submission_auth_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-submission-auth: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-submission-auth: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-submission-auth: verify-shell =="
...
echo "== perp-submission-auth: verify-multi =="
...
echo "== perp-submission-auth: regression net =="
"$PY" -m pytest -q \
...
echo "== perp-submission-auth: manifest check =="
"$PY" "$ROOT_DIR/tools/check_perp_submission_auth_assurance_manifest.py"
