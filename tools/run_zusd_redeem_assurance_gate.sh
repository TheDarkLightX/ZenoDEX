...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_redeem_fee_collateral_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_redeem_fee_collateral_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_redeem_fee_collateral_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-redeem: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-redeem: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-redeem: verify-shell =="
...
echo "== zusd-redeem: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_zusd_redeem_fee_collateral_v1_native_adapter.py

echo "== zusd-redeem: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_redeem_assurance_manifest.py"
