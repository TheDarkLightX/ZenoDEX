...
"""State-feedback wrapper for state-boundary concolic exploration."""
...
from tools import state_boundary_concolic as legacy
...
from tools.stateful_semantics import sequence_action_summary, state_boundary_semantic_state
...
def explore_target(
    ...
    *,
    ...
    semantic_state_fn = state_boundary_semantic_state(target.name)
    return explore_bounded_frontier(
        harness_id=f"state_boundary_concolic:{name}",
...
def _reports_json(reports: Sequence[ExplorationTargetReport]) -> dict[str, Any]:
    return {
        "schema": "zenodex/state-boundary-concolic-stateful/v1",
