...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/quote_receipt_hop_structure_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.quote_receipt_hop_structure_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/quote_receipt_hop_structure_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== quote-receipt-hop-structure-gate: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== quote-receipt-hop-structure-gate: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== quote-receipt-hop-structure-gate: verify-shell =="
...
echo "== quote-receipt-hop-structure-gate: verify-multi =="
...
echo "== quote-receipt-hop-structure-gate: regression net =="
"$PY" -m pytest -q \
...
echo "== quote-receipt-hop-structure-gate: manifest check =="
"$PY" "$ROOT_DIR/tools/check_quote_receipt_hop_structure_gate_assurance_manifest.py"
