...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/confidential_extension_receipt_precheck_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.confidential_extension_receipt_precheck_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/confidential_extension_receipt_precheck_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== confidential-extension-receipt-precheck-gate: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== confidential-extension-receipt-precheck-gate: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== confidential-extension-receipt-precheck-gate: verify-shell =="
...
echo "== confidential-extension-receipt-precheck-gate: verify-multi =="
...
echo "== confidential-extension-receipt-precheck-gate: regression net =="
"$PY" -m pytest -q \
...
echo "== confidential-extension-receipt-precheck-gate: manifest check =="
"$PY" "$ROOT_DIR/tools/check_confidential_extension_receipt_precheck_gate_assurance_manifest.py"
