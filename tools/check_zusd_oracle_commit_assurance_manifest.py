...
DEFAULT_MANIFEST = REPO_ROOT / "tools" / "zusd_oracle_commit_assurance_manifest.json"
...
def _git_stdout(*args: str) -> str:
    ...
    try:
        ...
        raise ManifestError("git is required for zUSD oracle-commit manifest checks") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check the pinned zUSD oracle-commit assurance manifest.")
    parser.add_argument("--manifest", default=str(DEFAULT_MANIFEST), help="Path to zUSD oracle-commit assurance manifest JSON")
    args = parser.parse_args(argv)

    manifest_path = Path(args.manifest).resolve()
    manifest = _as_dict(_load_json(manifest_path), ctx=str(manifest_path))

    _require(int(manifest.get("manifest_version", 0)) == 1, "unsupported zUSD oracle-commit assurance manifest version")
    ...
    _require(esso_head == toolchain["esso_code_hash"], "ESSO code hash drifted from zUSD oracle-commit manifest")
    _require(esso_tree == toolchain["esso_tree_sha256"], "ESSO tree drifted from zUSD oracle-commit manifest")
