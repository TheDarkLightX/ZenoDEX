...
def _validate_pin(
    ...
    require(
        absolute.resolve(strict=True) == repo_root.resolve() / pin.path,
        f"{packet.evidence_id}: pinned {label} path uses a symlink: {pin.path}",
    )
    require(
...
def _parse_git_name_status(output: bytes) -> tuple[ChangedPathV1, ...]:
    """Parse exact NUL-delimited Git paths without quote-path ambiguity."""

    fields = output.split(b"\0")
    require(fields[-1] == b"", "malformed NUL-delimited Git diff")
    tokens = fields[:-1]
    ...
    while index < len(tokens):
        try:
            ...
            raise TestHygieneError("non-ASCII Git change status") from exc
        index += 1
        if status.startswith(("R", "C")):
            require(index + 1 < len(tokens), "malformed Git rename row")
            old_path = tokens[index].decode("utf-8", errors="surrogateescape")
            new_path = tokens[index + 1].decode("utf-8", errors="surrogateescape")
            changes.append(ChangedPathV1(status="D", path=old_path))
            changes.append(ChangedPathV1(status="A", path=new_path))
            index += 2
            continue
        normalized_status = status[:1]
        require(
            normalized_status in ALLOWED_STATUSES and index < len(tokens),
            f"unsupported Git diff status: {status}",
        )
        path = tokens[index].decode("utf-8", errors="surrogateescape")
        changes.append(ChangedPathV1(status=normalized_status, path=path))
        index += 1
