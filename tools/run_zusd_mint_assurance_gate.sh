...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_mint_borrow_fee_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_mint_borrow_fee_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_mint_borrow_fee_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-mint: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-mint: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-mint: verify-shell =="
...
echo "== zusd-mint: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_zusd_mint_borrow_fee_v1_native_adapter.py

echo "== zusd-mint: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_mint_assurance_manifest.py"
