...
class StateCountProfile:
    ...
    residual_quote_counts: list[int] = field(default_factory=list)
    # Resource bounds (new)
...
def profile_staircase(
    ...
    for i in range(k):
        ...
        residual_quote_count = 0
        for spent in combined:
            residual = D - int(spent)
            if residual > 0 and residual >= 1:
                residual_quote_count += 1
        profile.residual_quote_counts.append(residual_quote_count)
        total_residual_quotes += residual_quote_count
...
def format_profile(p: StateCountProfile) -> str:
    ...
    lines.append(f"  residual_quote_counts (per interior): {p.residual_quote_counts}")
...
# Default cases stay bounded so the script is safe as a smoke/profile command.
TEST_CASES = [
...
def main():
    ...
    parser.add_argument(
        "--include-large",
        action="store_true",
        help="include large-domain profiling cases; may be slow",
    )
    args = parser.parse_args()
    cases = TEST_CASES + (LARGE_TEST_CASES if args.include_large else [])
    profiles = []
    for name, pools, D, max_legs in cases:
        print(f"\n{'='*60}")
        print(f"Case: {name}")
        print(f"{'='*60}")
        ...
        profiles.append({"name": name, "profile": {
            ...
            "residual_quote_counts": p.residual_quote_counts,
            "max_residual_quotes_seen": max(p.residual_quote_counts) if p.residual_quote_counts else 0,
    ...
    for entry in profiles:
        p = entry["profile"]
        print(f"{entry['name']:<25} {p['max_table_states']:>10} {p['max_prefix_states']:>8} {p['max_combine_pairs']:>12} {p['combine_pairs']:>10} {p['max_residual_quotes']:>8} {p['max_residual_quotes_seen']:>8}")
