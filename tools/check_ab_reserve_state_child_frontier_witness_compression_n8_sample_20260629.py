#!/usr/bin/env python3
"""Check sampled n=8 compressed predecessor witnesses for child frontiers.

This research-only checker extends the n=7 one-witness proof-object shape to
the deterministic n=8 sample: one predecessor witness per sampled child
quotient state.
"""
...
from tools.check_ab_reserve_state_child_frontier_witness_compression_20260629 import (  # noqa: E402
    _find_predecessor_witness,
    _packet_summary_from_rows,
    _state_from_json,
    _witness_rows_digest,
)
...
REPORT_MD = (
    ...
    / "docs"
    / "research"
    / "ZENODEX_AB_RESERVE_STATE_CHILD_FRONTIER_WITNESS_COMPRESSION_N8_SAMPLE_20260629.md"
)

PACKET_SCHEMA = (
    "zenodex.ab_reserve_state_child_frontier_witness_compression_n8_sample_packet.v1"
)
REPORT_SCHEMA = (
...
SCOPE = "n8_sampled_zero_min_child_frontier_predecessor_witnesses"
...
def _lean_contract() -> dict[str, str]:
    return {
        **_n8_frontier_lean_contract(),
        "host_witness_shape": (
            "one predecessor ReserveState.afterStep witness per sampled child quotient state"
        ),
    }
...
def _build_sampled_witness_rows(
    case: Any,
    *,
    ...
    for child_mask_id in _sampled_child_mask_ids(len(case.intents)):
        for child_state in _sorted_states(_state_set(full_dp[child_mask_id])):
...
def _sampled_packet_summary_from_rows(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    base = _packet_summary_from_rows(rows)
    return {
        "sampled_child_mask_count": base["child_mask_count"],
        "witness_count": base["witness_count"],
        "unique_child_witness_count": base["unique_child_witness_count"],
        "duplicate_witness_count": base["duplicate_witness_count"],
        "witness_rows_digest": base["witness_rows_digest"],
    }


def _verify_sampled_witness_rows(
    case: Any,
    *,
    full_dp: list[list[_HostRecord]],
    ...
    expected_child_keys = {
        (child_mask_id, state)
        for child_mask_id, states in sampled_frontier.items()
        for state in states
    }
    rows = list(packet.get("witness_rows", []) if packet is not None else [])
    seen_child_keys: set[tuple[int, Any]] = set()
    ...
    for index, row in enumerate(rows):
        ...
        if child_mask_id not in sampled_masks:
            row_reasons.append("witness_child_mask_not_sampled")
        ...
        if child_mask_id in sampled_frontier:
            child_states = sampled_frontier[child_mask_id]
            if child_state not in child_states:
                row_reasons.append("witness_child_state_not_in_sampled_child_frontier")
        ...
        if 0 <= step_bit_index < n:
            ...
            if row.get("step_order_short") != _short((intent.intent_id,)):
                row_reasons.append("witness_step_order_short_mismatch")
    ...
    if missing_child_keys:
        reasons.append("missing_sampled_child_state_witness")
        child_mask_id, _state = sorted(
            missing_child_keys,
            key=lambda item: (
                item[0],
        ...
        first_failure = _new_failure(
            ...
            case_id=case.case_id,
            mask_id=child_mask_id,
            reason="missing_sampled_child_state_witness",
        )
    if extra_child_keys:
        reasons.append("extra_sampled_child_state_witness")
    ...
    summary = _sampled_packet_summary_from_rows(rows)
    summary.update(
        {
            "expected_sampled_child_state_count": len(expected_child_keys),
            "covered_sampled_child_state_count": len(seen_child_keys & expected_child_keys),
            "missing_sampled_child_state_witness_count": len(missing_child_keys),
            "extra_sampled_child_state_witness_count": len(extra_child_keys),
...
def build_case_packet(
    ...
    *,
    ...
    witness_rows = _build_sampled_witness_rows(case, full_dp=full_dp)
    ...
    summary = _sampled_packet_summary_from_rows(witness_rows)
    summary.update(
        {
            "expected_sampled_child_state_count": summary["witness_count"],
            "covered_sampled_child_state_count": summary["witness_count"],
...
def verify_case(case: Any) -> dict[str, Any]:
    ...
    verification = _verify_sampled_witness_rows(case, full_dp=full_dp, packet=packet)
    return verification | {
...
def _negative_controls(cases: list[Any]) -> list[dict[str, Any]]:
    ...
    missing_witness["witness_summary"] = _sampled_packet_summary_from_rows(
    ...
    bad_parent["witness_summary"] = _sampled_packet_summary_from_rows(
        bad_parent["witness_rows"]
    )
    controls.append(
        (
    ...
    bad_child["witness_summary"] = _sampled_packet_summary_from_rows(
        bad_child["witness_rows"]
    )
    controls.append(
        (
    ...
    bad_step["witness_summary"] = _sampled_packet_summary_from_rows(
        bad_step["witness_rows"]
    )
    controls.append(
        (
    ...
    duplicate["witness_summary"] = _sampled_packet_summary_from_rows(
        duplicate["witness_rows"]
    )
    controls.append(
        (
    ...
    for mutation_id, packet, expected_reason in controls:
        verification = _verify_sampled_witness_rows(
            case,
...
def run_search() -> dict[str, Any]:
    ...
    return {
        ...
        "sampled_child_mask_count": sum(
            int(row["sampled_child_mask_count"]) for row in rows
        ),
        ...
        "missing_sampled_child_state_witness_count": sum(
            int(row["missing_sampled_child_state_witness_count"]) for row in rows
        ),
        "extra_sampled_child_state_witness_count": sum(
            int(row["extra_sampled_child_state_witness_count"]) for row in rows
        ),
...
def build_report() -> dict[str, Any]:
    ...
    ok = bool(
        ...
        and search["expected_sampled_child_state_count"] == search["witness_count"]
        and search["expected_sampled_child_state_count"]
        == search["covered_sampled_child_state_count"]
        and search["missing_sampled_child_state_witness_count"] == 0
        and search["extra_sampled_child_state_witness_count"] == 0
    ...
    return {
        ...
        "ok": ok,
        "summary": (
            "A bounded deterministic n=8 sample supports a compressed child-frontier "
            "proof-object shape: one predecessor witness covers each sampled child "
            "quotient state."
        ),
        "authority_boundary": (
...
def _write_markdown(report: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Expected sampled child states: `{search['expected_sampled_child_state_count']}`",
        f"- Witness rows: `{search['witness_count']}`",
        f"- Covered sampled child states: `{search['covered_sampled_child_state_count']}`",
        f"- Missing sampled witnesses: `{search['missing_sampled_child_state_witness_count']}`",
        f"- Extra sampled witnesses: `{search['extra_sampled_child_state_witness_count']}`",
        f"- Invalid witness count: `{search['invalid_witness_count']}`",
        f"- Duplicate witness count: `{search['duplicate_witness_count']}`",
        f"- Baseline sampled predecessor transitions: `{search['predecessor_transition_count']}`",
