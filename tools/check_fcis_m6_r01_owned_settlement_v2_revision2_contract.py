#!/usr/bin/env python3
"""Check the review-only M6-R01 OwnedSettlementV2 Revision 2 contract."""
...
DEFAULT_CONTRACT = (
    REPO_ROOT
    / "docs/research/"
    "FCIS_M6_R01_OWNED_SETTLEMENT_V2_ATDD_MATRIX_REVISION_2_20260731.json"
)
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from tools.fcis_m6_r01_owned_settlement_v2_revision2_contract_policy import (  # noqa: E402
    CASE_FIELDS,
...
def _validate_decision_lists(decision: dict[str, object]) -> list[str]:
    ...
    for label, actual, expected in (
        ("OUTER_FIELD_REGISTRY", decision.get("outer_field_registry"), OUTER_FIELDS),
        ("INNER_CLAIM_FIELDS", decision.get("inner_claim_fields"), INNER_FIELDS),
        (
            "FORBIDDEN_INNER_CLAIM_FIELDS",
            decision.get("forbidden_inner_claim_fields"),
            FORBIDDEN_INNER_FIELDS,
        ),
        (
            "CONTROLLED_BATCH_FIELDS",
            decision.get("controlled_batch_fields"),
            CONTROLLED_BATCH_FIELDS,
        ),
        (
            "FORBIDDEN_CONTROLLED_BATCH_FIELDS",
            decision.get("forbidden_controlled_batch_fields"),
            FORBIDDEN_CONTROLLED_BATCH_FIELDS,
        ),
        (
            "STATE_BINDING_LAWS",
            decision.get("state_binding_laws"),
            STATE_BINDING_LAWS,
        ),
    ):
        if _exact_string_list(actual) != expected:
            errors.append(label)
...
def _validate_decision(value: object) -> list[str]:
    ...
    if decision.get("downstream_claim_source") != DOWNSTREAM_CLAIM_SOURCE:
        errors.append("DOWNSTREAM_CLAIM_SOURCE")
...
def _validate_graph(value: object) -> list[str]:
    ...
    if set(nodes) != REQUIRED_NODES:
        errors.append(
            _field_error("DEPENDENCY_NODE_SET", set(nodes), REQUIRED_NODES)
        )
...
def validate_contract(
    ...
    *,
    ...
    """Return stable errors for one already decoded Revision 2 contract."""
