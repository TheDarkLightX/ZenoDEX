...
Pubkeys and asset ids canonicalize like ``canonical_hex_fixed_allow_0x``: raw
hex, ``0x`` / ``0X`` prefixes, mixed case, and surrounding whitespace collapse
to lowercase ``0x`` form.
...
def smoke_tx_sequence() -> list[dict]:
    ...
    return [
        ...
        _credit(f"  {_B[2:].upper()}  ", "0X" + _Y[2:].upper(), 500),  # accept/canonicalize
