#!/usr/bin/env python3
"""Check the review-only M6-R01 OwnedSettlementV2 Revision 3 contract."""
...
DEFAULT_CONTRACT = (
    REPO_ROOT
    / "docs/research/"
    "FCIS_M6_R01_OWNED_SETTLEMENT_V2_ATDD_MATRIX_REVISION_3_20260731.json"
)
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from tools.fcis_m6_r01_owned_settlement_v2_revision3_contract_policy import (  # noqa: E402
    CASE_FIELDS,
...
_DECISION_LIST_SPECS = {
    "outer_field_registry": ("OUTER_FIELD_REGISTRY", OUTER_FIELDS),
    "replay_projection_fields": ("REPLAY_PROJECTION_FIELDS", REPLAY_PROJECTION_FIELDS),
    "replay_projection_forbidden_fields": (
        "REPLAY_PROJECTION_FORBIDDEN_FIELDS",
        REPLAY_PROJECTION_FORBIDDEN_FIELDS,
    ),
    "inner_claim_fields": ("INNER_CLAIM_FIELDS", INNER_FIELDS),
    "forbidden_inner_claim_fields": ("FORBIDDEN_INNER_CLAIM_FIELDS", FORBIDDEN_INNER_FIELDS),
    "controlled_occurrence_fields": ("CONTROLLED_OCCURRENCE_FIELDS", CONTROLLED_OCCURRENCE_FIELDS),
    "controlled_batch_fields": ("CONTROLLED_BATCH_FIELDS", CONTROLLED_BATCH_FIELDS),
    "forbidden_controlled_batch_fields": (
        "FORBIDDEN_CONTROLLED_BATCH_FIELDS",
        FORBIDDEN_CONTROLLED_BATCH_FIELDS,
    ),
    "state_binding_laws": ("STATE_BINDING_LAWS", STATE_BINDING_LAWS),
    "noninterference_laws": ("NONINTERFERENCE_LAWS", NONINTERFERENCE_LAWS),
    "occurrence_pairing_laws": ("OCCURRENCE_PAIRING_LAWS", OCCURRENCE_PAIRING_LAWS),
    "recomputed_claim_predecessors": (
        "RECOMPUTED_CLAIM_PREDECESSORS",
        RECOMPUTED_CLAIM_PREDECESSORS,
    ),
...
_DECISION_SCALARS = {
    "inner_claim_type": "ProvisionalProtocolFeeOccurrenceClaimV2",
    "replay_projection_type": "ExactSettlementReplayProjectionV2",
    "controlled_occurrence_type": "ControlledProvisionalProtocolFeeOccurrenceV2",
    "controlled_batch_type": "StateBoundProvisionalProtocolFeeWitnessBatchV2",
    "root_strategy": "full_settlement_root_plus_claim_erased_replay_and_paired_occurrences",
    "downstream_occurrence_source": DOWNSTREAM_OCCURRENCE_SOURCE,
}
...
def _validate_sources(value: object, *, repo_root: Path) -> list[str]:
    if type(value) is not dict:
    ...
    path_pairs = (
        ...
        ("provisional_fee_replay_path", "provisional_fee_replay_sha256"),
        (
            "provisional_fee_replay_values_path",
            "provisional_fee_replay_values_sha256",
        ),
        ("revision_2_contract_path", "revision_2_contract_sha256"),
        ("revision_2_matrix_path", "revision_2_matrix_sha256"),
        ("state_binding_path", "state_binding_sha256"),
    )
    for path_field, digest_field in path_pairs:
...
def _validate_decision_lists(decision: dict[str, object]) -> list[str]:
    errors = [
        label
        for field, (label, expected) in _DECISION_LIST_SPECS.items()
        if _exact_string_list(decision.get(field)) != expected
    ]
    projection_fields = _exact_string_list(decision.get("replay_projection_fields")) or []
    projection_overlap = sorted(
        set(projection_fields) & set(REPLAY_PROJECTION_FORBIDDEN_FIELDS)
    )
    if projection_overlap:
        errors.append(
            f"REPLAY_PROJECTION_FORBIDDEN_FIELD:{','.join(projection_overlap)}"
        )
    inner_fields = _exact_string_list(decision.get("inner_claim_fields")) or []
    inner_overlap = sorted(set(inner_fields) & set(FORBIDDEN_INNER_FIELDS))
    if inner_overlap:
        errors.append(f"INNER_CLAIM_FORBIDDEN_FIELD:{','.join(inner_overlap)}")
    batch_fields = _exact_string_list(decision.get("controlled_batch_fields")) or []
    batch_overlap = sorted(
        set(batch_fields) & set(FORBIDDEN_CONTROLLED_BATCH_FIELDS)
    )
    if batch_overlap:
        errors.append(
            f"CONTROLLED_BATCH_DUPLICATED_FIELD:{','.join(batch_overlap)}"
        )
    return errors
...
def _validate_decision(value: object) -> list[str]:
    ...
    expected_fields = set(_DECISION_LIST_SPECS) | set(_DECISION_SCALARS)
    if set(decision) != expected_fields:
        return [_field_error("DECISION_FIELDS", set(decision), expected_fields)]
    errors = [
        f"DECISION_VALUE:{field}"
        for field, expected in _DECISION_SCALARS.items()
        if decision.get(field) != expected
    ]
    errors.extend(_validate_decision_lists(decision))
    return errors
...
def _validate_exact_predecessors(edges: set[tuple[str, str]]) -> list[str]:
    errors: list[str] = []
    for node, expected in EXACT_PREDECESSORS.items():
        actual = {source for source, target in edges if target == node}
        if actual != expected:
            errors.append(
                _field_error(
                    f"DEPENDENCY_PREDECESSORS:{node}",
                    actual,
                    expected,
                )
            )
...
def _validate_graph(value: object) -> list[str]:
    ...
    expected_fields = {"edges", "nodes", "topological_order"}
    if set(graph) != expected_fields:
        return [
            _field_error("DEPENDENCY_GRAPH_FIELDS", set(graph), expected_fields)
        ]
    errors: list[str] = []
    ...
    if set(nodes) != REQUIRED_NODES:
        errors.append(_field_error("DEPENDENCY_NODE_SET", set(nodes), REQUIRED_NODES))
    ...
    forbidden = sorted(edges & FORBIDDEN_EDGES)
    if forbidden:
        errors.append(
            "DEPENDENCY_FORBIDDEN_EDGE:"
            + ",".join(f"{source}->{target}" for source, target in forbidden)
        )
    errors.extend(_validate_exact_predecessors(edges))
    if order:
...
def validate_contract(
    ...
    *,
    ...
    """Return stable errors for one already decoded Revision 3 contract."""
    ...
    if _exact_string_list(contract.get("no_authority_outputs")) != NO_AUTHORITY_OUTPUTS:
        errors.append("NO_AUTHORITY_OUTPUTS")
    ...
    if source_roles != SOURCE_ROLES:
        ...
        if type(source_roles) is dict:
            if source_roles.get("submitted_claims") != (
                "equality targets only and never replay inputs"
            ):
                errors.append("SUBMITTED_CLAIM_AUTHORITY")
            if source_roles.get("submitted_roots") != "equality targets only":
                errors.append("SUBMITTED_ROOT_AUTHORITY")
    if contract.get("claim_cardinality_policy") != CLAIM_CARDINALITY_POLICY:
        errors.append("CLAIM_CARDINALITY_POLICY")
    if contract.get("currentness_policy") != CURRENTNESS_POLICY:
        errors.append("CURRENTNESS_POLICY")
    if _exact_string_list(contract.get("nonclaims")) != NONCLAIMS:
...
def _report(path: Path) -> tuple[int, dict[str, object]]:
    ...
    report: dict[str, object] = {
        "acceptance_case_count": (
            len(acceptance_cases) if type(acceptance_cases) is list else 0
        ),
        "errors": errors,
