...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/confidential_request_use_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.confidential_request_use_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/confidential_request_use_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== confidential-request-use-gate: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== confidential-request-use-gate: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== confidential-request-use-gate: verify-shell =="
...
echo "== confidential-request-use-gate: verify-multi =="
...
echo "== confidential-request-use-gate: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_confidential_request_use_gate_v1_native_adapter.py

echo "== confidential-request-use-gate: manifest check =="
"$PY" "$ROOT_DIR/tools/check_confidential_request_use_gate_assurance_manifest.py"
