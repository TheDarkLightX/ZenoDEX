...
RELEASE_CHECK_PREDECESSOR_STAGE_IDS = (
    "identity_rebuild",
...
RELEASE_CHECK_INPUT_ROLES = (*RELEASE_CHECK_ARTIFACT_ROLES, "release_plan_expectation")
_RELEASE_CHECK_ARTIFACT_ARGUMENTS = tuple(
    argument
    for role in RELEASE_CHECK_ARTIFACT_ROLES
    for argument in (f"--artifact-{role.replace('_', '-')}", f"@{role}")
)
...
ARTIFACT_SPECS = (
    ...
    ArtifactSpec(
        "release_plan_expectation",
        "inputs/release_plan_expectation.json",
        "expectation",
        "external_operator",
        64 * 1024,
    ),
...
TASK_SPECS = (
    ...
    TaskSpec(
        ...
        (
            ...
            "--execution-packet",
            "@execution_packet_file",
            "--release-plan-expectation",
            "@release_plan_expectation",
            *_RELEASE_CHECK_ARTIFACT_ARGUMENTS,
            "--release-plan-out",
            "@release_plan",
        ...
        (
            "the exact release execution packet and every declared artifact ID are recomputed",
            "the ordered unique predecessor marker digest list is committed without a Return V5 cycle",
            "worker marker-record validation remains a separate prerequisite of command execution",
            "all five exact one-bit mutation relations are derived from the returned receipt pairs",
            "worker build V7 and mutation reports bind the exact returned program receipt and mutation bytes",
            "the deterministic release closure matches an external exact plan expectation",
            "proof release settlement ledger and production authority remain false",
        ),
        "light",
        execution_adapter_status="implemented",
    ),
