...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_submission_auth_field_selector_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_submission_auth_field_selector_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_submission_auth_field_selector_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-submission-auth-field-selector: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-submission-auth-field-selector: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-submission-auth-field-selector: verify-shell =="
...
echo "== perp-submission-auth-field-selector: verify-multi =="
...
echo "== perp-submission-auth-field-selector: regression net =="
"$PY" -m pytest -q \
