...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--upba-coverage-profile", type=Path, required=True)
    parser.add_argument(
        "--upba-source-kind",
    ...
    parser.add_argument("--autotrader-coverage-profile", type=Path, required=True)
    parser.add_argument(
    ...
    print(operator_json_dumps(bundle))
    return 0
...
def build_production_evidence_bundle(
    *,
    ...
    upba_coverage_profile: dict[str, Any],
    ...
    autotrader_coverage_profile: dict[str, Any],
    ...
    upba_real_replay = build_upba_real_replay_report(
        ...
        coverage_profile=upba_coverage_profile,
    )
    autotrader_real_shadow = build_autotrader_real_shadow_report(
        ...
        coverage_profile=autotrader_coverage_profile,
    )
    ...
    return {
        ...
        "builder": {
            ...
            "coverage_profile_tool": "tools/check_zenoenergy_replay_coverage_profile.py",
        },
        "source_paths": source_paths or {},
        ...
        "coverage_profile_checks": {
            "upba": coverage_profile_summary(
                validate_replay_coverage_profile(
                    real_report=upba_real_replay,
                    profile=upba_coverage_profile,
                )
            ),
            "autotrader": coverage_profile_summary(
                validate_replay_coverage_profile(
                    real_report=autotrader_real_shadow,
                    profile=autotrader_coverage_profile,
                )
            ),
        },
        "reports": {
        ...
        "negative_knowledge": [
            ...
            "A passing coverage profile is a breadth guard, not a representativeness proof.",
...
def _build_from_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source_paths = {
        ...
        "upba_coverage_profile": _display_path(args.upba_coverage_profile),
        ...
        "autotrader_coverage_profile": _display_path(args.autotrader_coverage_profile),
    }
    return build_production_evidence_bundle(
        ...
        upba_coverage_profile=_load_json(args.upba_coverage_profile),
        ...
        autotrader_coverage_profile=_load_json(args.autotrader_coverage_profile),
...
def _markdown_report(bundle: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Coverage Profile Checks", ""])
    for name, summary in bundle["coverage_profile_checks"].items():
        lines.append(
            f"- {name}: ok={str(summary['ok']).lower()}, "
            f"profile_type={summary['profile_type']}, "
            f"source_report_count={summary['source_report_count']}"
        )
