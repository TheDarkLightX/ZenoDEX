...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/intent_nonce_sequence_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.intent_nonce_sequence_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/intent_nonce_sequence_gate_v1"
mkdir -p "$VERIFY_ROOT"
...
"$PY" -m pytest -q tests/state/test_intent_nonce_sequence_gate.py tests/state/test_nonces.py tests/tau/test_replay_semantic_lane.py tests/kernels/test_intent_nonce_sequence_gate_v1_native_adapter.py
"$PY" "$ROOT_DIR/tools/check_intent_nonce_sequence_gate_assurance_manifest.py"
