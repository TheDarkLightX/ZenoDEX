...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_liquidation_eligibility_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_liquidation_eligibility_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_liquidation_eligibility_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-liquidation-eligibility: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-liquidation-eligibility: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-liquidation-eligibility: verify-shell =="
...
echo "== perp-liquidation-eligibility: verify-multi =="
...
echo "== perp-liquidation-eligibility: regression net =="
"$PY" -m pytest -q \
