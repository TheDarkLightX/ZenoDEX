...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_tau_ingress_stream_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_tau_ingress_stream_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_tau_ingress_stream_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-tau-ingress-stream: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-tau-ingress-stream: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-tau-ingress-stream: verify-shell =="
...
echo "== perp-tau-ingress-stream: verify-multi =="
...
echo "== perp-tau-ingress-stream: regression net =="
"$PY" -m pytest -q \
