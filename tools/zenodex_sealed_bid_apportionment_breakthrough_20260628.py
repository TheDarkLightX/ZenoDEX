...
def build_certificate(*, case_id: str, units_for_sale: int, bids: tuple[RevealedSealedBid, ...]) -> dict[str, Any]:
    ...
    apportionment_fills = _apportionment_fills(
    ...
    return {
        ...
        "prefix_rows": prefix_rows,
        "marginal_rows": rows,
        "settlement_fills": apportionment_fills,
        "runtime_reference_fills": [
            {
        ...
        "total_filled": sum(int(fill["filled_quantity"]) for fill in apportionment_fills),
        "runtime_reference_total_filled": int(settlement.total_filled),
...
def _apportioned_fill_total_for_bidder(
    *,
    case_id: str,
    units_for_sale: int,
    ...
    return sum(
        int(fill["filled_quantity"])
        for fill in certificate["settlement_fills"]
        if str(fill["bidder_id"]) == str(bidder_id)
    )
...
def split_bid_witness() -> dict[str, Any]:
    ...
    base = build_certificate(case_id="split_bid_base", units_for_sale=2, bids=base_bids)
    split = build_certificate(case_id="split_bid_split", units_for_sale=2, bids=split_bids)
    base_alice = _apportioned_fill_total_for_bidder(
        case_id="split_bid_base",
        units_for_sale=2,
        bids=base_bids,
        bidder_id="alice",
    )
    split_alice = _apportioned_fill_total_for_bidder(
        case_id="split_bid_split",
        units_for_sale=2,
        bids=split_bids,
        bidder_id="alice",
    )
    ...
    return {
        ...
        "base_fills": [
            ...
            for fill in base["settlement_fills"]
        ],
        "split_fills": [
            {"bidder_id": fill["bidder_id"], "commitment": fill["commitment"], "filled_quantity": fill["filled_quantity"]}
            for fill in split["settlement_fills"]
        ],
