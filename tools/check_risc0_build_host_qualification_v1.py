...
V1 has no descendant replay mode. The checked root must have HEAD at E, with E
directly adding the artifact to implementation commit C and C directly adding
the implementation to fixed parent P. The live artifact must exactly equal E's
blob. An external, immutable launcher must pin the complete executable closure
before it can make a stronger bootstrap claim; this Python process cannot
authenticate its own startup.
"""
...
def _live_artifact_bytes_v1(store: GitObjectStoreV1, entry: GitTreeEntryV1) -> bytes:
    """Bind the live regular artifact to E's blob without claiming a clean tree."""
...
def _replay_exact_e_v1(
    ...
    *,
    ...
    return cast(str, result["status"]), {
        ...
        "git_executable_path": str(store.git_executable),
        "implementation_commit": implementation_commit,
...
def check_risc0_build_host_qualification_v1(
    *,
    ...
    """Fail closed unless HEAD and the live artifact are exactly source-bound E."""
...
def main(
    ...
    *,
    ...
    report = check_risc0_build_host_qualification_v1(
        root=args.root,
        expected_parent=expected_parent,
    )
