...
"""State-feedback explorer for duplicate-signature and replay-like signed-operations envelopes."""
...
from tools import operations_grammar_fuzz as legacy
...
from tools.stateful_semantics import operations_signature_semantic_state, sequence_action_summary


TARGET = legacy.TARGET_INDEX["signed_intents"]
SEED_DERIVATION = "SignedOps->OneEntry ; Entry->Envelope2(IntentDict(valid), Signature(valid))"
MUTATED_DERIVATIONS: tuple[tuple[str, str], ...] = (
    ("duplicate_signature_same", "SignedOps->OneEntry ; Entry->DuplicateSignatureSame"),
    ("duplicate_signature_same_with_dead_tail", "SignedOps->OneEntry ; Entry->DuplicateSignatureSameWithDeadTail"),
    ("duplicate_signature_differs", "SignedOps->OneEntry ; Entry->DuplicateSignatureDiffers"),
)
...
def explore_target(
    name: str = "signature_reuse_sequence",
    *,
    max_depth: int = 1,
    max_frontier: int = 64,
    ...
    if name != "signature_reuse_sequence":
    ...
    return explore_bounded_frontier(
        harness_id="operations_signature_sequence:signature_reuse_sequence",
        seed=_seed_payload(),
        mutations=MUTATIONS,
        trace_fn=_trace,
        ...
        target_id=target_id,
        semantic_state_fn=operations_signature_semantic_state,
        action_summary_fn=lambda prev_payload, next_payload, mutation_name: sequence_action_summary(
            operations_signature_semantic_state,
            prev_payload,
            next_payload,
            mutation_name,
        ),
...
def _reports_json(reports: Sequence[ExplorationTargetReport]) -> dict[str, Any]:
    return {
        "schema": "zenodex/operations-signature-sequence-grammar-fuzz/v1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--target", default="signature_reuse_sequence", choices=("signature_reuse_sequence",))
