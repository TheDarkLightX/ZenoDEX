...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/zusd_oracle_commit_apply_v1.yaml"
ADAPTER_SPEC="src.kernels.python.zusd_oracle_commit_apply_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/zusd_oracle_commit_apply_v1"
mkdir -p "$VERIFY_ROOT"

echo "== zusd-oracle-commit: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== zusd-oracle-commit: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== zusd-oracle-commit: verify-shell =="
...
echo "== zusd-oracle-commit: verify-multi =="
...
echo "== zusd-oracle-commit: adapter regression net =="
"$PY" -m pytest -q tests/kernels/test_zusd_oracle_commit_apply_v1_native_adapter.py

echo "== zusd-oracle-commit: manifest check =="
"$PY" "$ROOT_DIR/tools/check_zusd_oracle_commit_assurance_manifest.py"
