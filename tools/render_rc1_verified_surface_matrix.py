...
def render_matrix_text(
    *,
    ...
    lines = [
        "---",
        f"title: {active_label}_VERIFIED_SURFACE_MATRIX",
        "type: note",
        "permalink: autonomous-tau-dex-review/docs/rc1-verified-surface-matrix",
        "---",
        "",
        f"# {active_label} Candidate Verified Surface Matrix",
        "",
        ...
        "",
        f"This matrix defines the exact conservative {active_label} candidate claim boundary for ZenoDEX.",
        "",
        "```text",
    ...
    lines[-2] = f"## Excluded Claims That Must Stay Out Of {active_label}"
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            "",
            f"These checks are intentionally narrower than the full repo. They exist to keep the {active_label} claim specific and auditable.",
            "",
        ]
    )
    return "\n".join(lines)
