...
def _check_workflows(errors: list[str]) -> None:
    ...
    # path appeared only in a comment. Parse active workflow structure instead:
    ...
    for event in ("pull_request", "push"):
        ...
        if missing:
            errors.append(
                f"runtime-shadow on.{event}.paths missing settlement-supply formal-spec filters: {missing}"
            )

    for snippet in EXPECTED_WORKFLOW_TOKENS:
        if snippet == "docs/assurance/settlement_supply_formal_spec_contract.json":
            if not any(
                ...
                errors.append(f"runtime-shadow path filters missing settlement-supply contract: {snippet}")
            continue
        if not (
            ...
            errors.append(
                f"workflow is missing active settlement-supply formal-spec gate token: {snippet}"
            )
