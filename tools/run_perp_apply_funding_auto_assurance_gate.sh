...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_apply_funding_auto_gate_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_apply_funding_auto_gate_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_apply_funding_auto_gate_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-apply-funding-auto: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-apply-funding-auto: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-apply-funding-auto: verify-shell =="
...
echo "== perp-apply-funding-auto: verify-multi =="
...
echo "== perp-apply-funding-auto: regression net =="
"$PY" -m pytest -q \
  ...
  -k 'apply_funding_auto'

echo "== perp-apply-funding-auto: manifest check =="
"$PY" "$ROOT_DIR/tools/check_perp_apply_funding_auto_assurance_manifest.py"
