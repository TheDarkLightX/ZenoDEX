...
MODEL_PATH="$ROOT_DIR/src/kernels/dex/perp_market_version_prefix_guard_v1.yaml"
ADAPTER_SPEC="src.kernels.python.perp_market_version_prefix_guard_v1_native_adapter:make_adapter"
VERIFY_ROOT="$ROOT_DIR/internal/esso_verify/perp_market_version_prefix_guard_v1"
mkdir -p "$VERIFY_ROOT"

echo "== perp-market-version-prefix: validate =="
"$PY" -m ESSO validate "$MODEL_PATH" >"$VERIFY_ROOT/validate.json"

echo "== perp-market-version-prefix: shell-lint =="
"$PY" -m ESSO shell-lint \
  "$MODEL_PATH" \
  --adapter "$ADAPTER_SPEC" \
  --output "$VERIFY_ROOT/shell_lint.json"

echo "== perp-market-version-prefix: verify-shell =="
...
echo "== perp-market-version-prefix: verify-multi =="
...
echo "== perp-market-version-prefix: regression net =="
"$PY" -m pytest -q \
...
echo "== perp-market-version-prefix: manifest check =="
"$PY" "$ROOT_DIR/tools/check_perp_market_version_prefix_assurance_manifest.py"
