...
def render_runtime_path_text(*, root: Path = REPO_ROOT, manifest: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        "```",
        "",
        f"Standard reading: the conservative {active_label} runtime claim is only about a narrow HTTP subset, one pinned spot admission/signing path, and the narrow zUSD wallet transport path.",
        "",
