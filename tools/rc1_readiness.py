...
try:
    from tools.check_tau_supported_runtime_subset import supported_runtime_subset_status
except ModuleNotFoundError:  # pragma: no cover - script execution path
    from check_tau_supported_runtime_subset import supported_runtime_subset_status
...
def _release_labels(manifest: dict[str, Any]) -> tuple[str, str]:
    ...
    if not isinstance(historical, str) or not historical.strip():
        raise RC1Error("historical_release_label must be a non-empty string")
    if not isinstance(active, str) or not active.strip():
        raise RC1Error("active_candidate_label must be a non-empty string")
...
def _route_presence(root: Path, rel_path: str, routes: Sequence[str]) -> dict[str, bool]:
    source_paths = [root / rel_path]
    dispatch_root = root / "src" / "integration"
    if dispatch_root.exists():
        source_paths.extend(sorted(dispatch_root.glob("dex_dispatch*.py")))
        source_paths.append(dispatch_root / "api_server_dex_dispatch.py")
    ...
    # Review note, grade A-: RC readiness previously scanned only
    # api_server.py, which produced a false missing-route blocker after
    # /api/dex handlers moved into route-family registry modules. The check
    # remains source-based and conservative, but it now follows those modules.
    return {route: any(route in text for text in texts) for route in routes}
...
def _build_status_payload(
    *,
    ...
    historical_label, active_label = _release_labels(manifest)
    ...
    tau_runtime = (
        supported_runtime_subset_status(root=root)
        if require_runtime_path_check
        else {"ok": True, "path": "tools/check_tau_supported_runtime_subset.py", "error": None}
    )
    matrix = (
        rc1_matrix_status(
            root=root,
    ...
    checks = {
        ...
        "tau_supported_runtime_subset_ok": bool(tau_runtime.get("ok")),
    ...
    return {
        ...
        "historical_release_label": historical_label,
        ...
        "assurance": {
            ...
            "tau_supported_runtime_subset_ok": bool(tau_runtime.get("ok")),
            "tau_supported_runtime_subset_error": tau_runtime.get("error"),
            "tau_supported_runtime_subset_path": tau_runtime.get("path"),
            "verified_surface_matrix_ok": bool(matrix.get("ok")),
...
def _print_status(payload: dict[str, Any]) -> None:
    ...
    print(f"ZenoDex {active_label} Readiness")
    print(f"historical baseline: {historical_label}")
    print(f"manifest: {payload['manifest_path']}")
    print(f"overall: {'READY' if payload['overall_ok'] else 'BLOCKED'}")
    print(f"dirty tree: {payload['dirty_count']} paths")
    print(f"{active_label.lower()} scoped dirty paths: {payload['rc1_scoped_dirty_count']}")
    print()
    print("Checks")
    ...
    tau_runtime_path = payload["assurance"].get("tau_supported_runtime_subset_path")
    tau_runtime_error = payload["assurance"].get("tau_supported_runtime_subset_error")
    if tau_runtime_path:
        print()
        print("Tau supported runtime subset")
        if tau_runtime_error:
            print(f"  [BLOCK] {tau_runtime_path}: {tau_runtime_error}")
        else:
            state = "OK" if payload["checks"]["tau_supported_runtime_subset_ok"] else "STALE"
            print(f"  [{state}] {tau_runtime_path}")
    ...
    if scoped_dirty_paths:
        print()
        print(f"{active_label} scoped dirty paths")
        limit = 20
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check the current conservative release-candidate boundary.")
    parser.add_argument("--format", choices=("text", "json"), default="text")
    parser.add_argument("--check", action="store_true", help="exit nonzero when the active candidate is not yet ready")
