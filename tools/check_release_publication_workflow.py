...
def _top_level_permissions_are_read_only(text: str) -> bool:
    ...
    for idx, raw_line in enumerate(lines):
        ...
        return "contents: read" in block and all(not item.endswith(": write") for item in block)
    return False
...
def check_release_publication_workflow(path: Path = DEFAULT_WORKFLOW) -> dict[str, Any]:
    ...
    if not top_permissions_ok:
        errors.append("release publication workflow must keep top-level permissions at contents: read")
    ...
    for check_id, (job, required) in job_checks.items():
        ...
        if not ok:
            errors.append(f"release publication workflow job permission check failed: {check_id}")

    npm_manual_only = "if: ${{ github.event_name == 'workflow_dispatch' && inputs.publish_npm }}" in npm_job
