...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--secret-scan-report",
        type=Path,
        help="Optional output from tools/check_zenoenergy_replay_secret_scan.py.",
    )
    ...
    try:
        ...
        manifest = build_replay_source_manifest(
            ...
            secret_scan_report=_load_json(args.secret_scan_report)
            if args.secret_scan_report is not None
            else None,
...
def build_replay_source_manifest(
    *,
    ...
    secret_scan = _secret_scan_fragment(
        source_report_count=len(source_reports),
        secret_scan_tool=secret_scan_tool,
        secret_scan_ok=secret_scan_ok,
        secret_scan_finding_count=secret_scan_finding_count,
...
def _secret_scan_fragment(
    *,
    source_report_count: int,
    secret_scan_tool: str,
    secret_scan_ok: bool,
    secret_scan_finding_count: int,
    secret_scan_report: dict[str, Any] | None,
) -> dict[str, Any]:
    if secret_scan_report is None:
        return {
