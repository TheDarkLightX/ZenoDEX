...
_PLAN_ALLOWED_VM_STATUSES_V1: Final = frozenset(
    {"GAP", "PARTIAL_REQUIRES_CURRENT_RECONCILIATION"}
)
...
def _snapshot_json_string_v1(
    value: str, path: str | _JsonPathV1, budget: _JsonBudgetV1
) -> str:
    ...
    while stack:
        ...
        if type(task) is _JsonDictCursorV1:
            try:
                ...
                if len(task.values) != task.expected_length or len(task.owned) != task.expected_length:
            ...
            owned_key = _snapshot_json_string_v1(
                key, _JsonPathV1(task.path, "<key>"), budget
            )
        ...
        if type(current) is int:
            if not -MAX_JSON_INTEGER_MAGNITUDE_EXCLUSIVE_V1 < current < MAX_JSON_INTEGER_MAGNITUDE_EXCLUSIVE_V1:
...
def _append_canonical_json_chunk_v1(
    chunks: list[bytes], encoded_size: int, chunk: str
) -> int:
...
def _parse_plan_claim_ceiling_v1(fields: dict[str, object]) -> None:
    ...
    if _expect_str(admission.get("authority_effect"), f"{admission_path}.authority_effect") != "NONE":
    ...
    if _expect_str(release_gate.get("whole_value_movement_claim"), claim_path) != _PLAN_FORBIDDEN_VALUE_CLAIM_V1:
