...
/// Canonicalize a sender: `0x` + 96 hex chars -> lowercase, else `None`.
pub fn canonical_sender(sender: &str) -> Option<String> {
    let body = sender.strip_prefix("0x")?;
...
impl AdmissionReceipt {
    pub fn receipt_hash(&self) -> String {
        ...
        buf.extend(encode_uvarint(self.nonce as u128));
        buf.extend_from_slice(b"PRV");
        buf.extend(encode_uvarint(self.prev_nonce as u128));
        sha256_hex(&buf)
    }
}
...
pub fn admit(
    ...
    let new_state = state.with_last(&canonical, nonce);
    Ok(AdmitAccepted {
...
mod tests {
    ...
    fn sequential_accepts_then_duplicate_and_gap_reject() {
        ...
        for n in 1..=4u64 {
            ...
            assert_eq!(acc.receipt.nonce, n);
            assert_eq!(acc.receipt.prev_nonce, n - 1);
            state = acc.state;
        }
        assert_eq!(
            admit(&state, &a, 4),
        ...
        assert_eq!(admit(&state, &a, 3), Err(ReplayRejectedReason::StaleNonce));
        assert_eq!(admit(&state, &a, 5).unwrap().receipt.nonce, 5);
        assert_eq!(admit(&state, &a, 7), Err(ReplayRejectedReason::NonceGap));
    }
    ...
    fn senders_are_independent_unit() {
        ...
        let mut state = ReplayGuardState::default();
        state = admit(&state, &a, 1).unwrap().state;
        state = admit(&state, &a, 2).unwrap().state;
        assert!(admit(&state, &b, 1).is_ok());
        assert_eq!(admit(&state, &b, 3), Err(ReplayRejectedReason::NonceGap));
    }
            ...
            for nonce in ops {
                ...
                match admit(&state, &s, nonce) {
                    Ok(acc) => {
                        ...
                        prop_assert_eq!(acc.receipt.prev_nonce, before);
                        state = acc.state;
                        expected_next += 1;
                    }
                    Err(_) => {
