...
/// Exact-out overdelivery-gap policy (matches the authority default).
pub const MAX_OVERDELIVERY_GAP_BPS: u128 = 200;
...
pub const REJ_OVERDELIVERY_GAP: &str = "overdelivery_gap_exceeded";
...
pub fn swap_exact_out(
    ...
    // Overdelivery-gap policy (authority order: checked *before* slippage). The
    // fee is recomputed from gross_in (ceil), exactly as the v8 kernel does, so
    // net_in_actual / amount_out_quote / gap match the Python authority bit-for-bit.
    let fee_total = ceil_div(gross_in * pool.fee_bps, BPS_DENOM);
