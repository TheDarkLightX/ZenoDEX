...
fn as_u16(v: &serde_json::Value) -> u16 {
    let n = v.as_u64().expect("fixture arg must be a non-negative integer");
...
fn dispatch(surface: &str, a: &[serde_json::Value]) -> bool {
    match surface {
        "fee" => gate::fee_revision_ok(
            as_bool(&a[0]), as_bool(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
            as_u16(&a[4]), as_u16(&a[5]),
        ),
        "router_split" => gate::router_split_revision_ok(
            as_bool(&a[0]), as_bool(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
            as_u16(&a[4]), as_u16(&a[5]), as_u16(&a[6]), as_u16(&a[7]),
        ),
        ...
        "action" => gate::action_bound_ok(
            as_bool(&a[0]), as_bool(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
            as_u16(&a[4]), as_u16(&a[5]), as_u16(&a[6]), as_u16(&a[7]),
            as_u16(&a[8]), as_u16(&a[9]),
        ),
        "drift" => gate::drift_budget_ok(
            as_u16(&a[0]), as_u16(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
        ),
        "cooldown" => gate::cooldown_ok(as_u16(&a[0]), as_u16(&a[1]), as_u16(&a[2])),
        "charter" => gate::charter_ok(
            as_bool(&a[0]), as_u16(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
        ),
        "epoch_budget" => gate::epoch_budget_ok(
            as_u16(&a[0]), as_u16(&a[1]), as_u16(&a[2]), as_u16(&a[3]),
        ),
...
fn rust_matches_shared_boundary_table() {
    ...
    assert!(cases.len() >= 39, "fixture unexpectedly small: {}", cases.len());
...
fn params_digest_matches_python_golden_vectors() {
    ...
    for (i, vec) in vectors.iter().enumerate() {
        ...
        assert_eq!(got, expected, "digest vector {i}: cross-language encoder drift");
    }
}
