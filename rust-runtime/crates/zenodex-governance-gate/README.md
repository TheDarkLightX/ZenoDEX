...
- The **epoch machine** (`epoch` module): the full transition core mirroring
  `gov_epoch.py` — propose → timelock/veto window → all-or-nothing apply in the
  ...
  `tests/tau_specs/governance/fixtures/gov_epoch_scenarios.json` — 53
  transitions GENERATED by running the scenario scripts through
  `gov_epoch.py` itself (all 17 receipt codes, all three surface-gate
  labels), byte-pinned by `test_gov_parity.py`. Every receipt field and
  every state observable must match per step.

Python's hostile-object raise-paths are out of fixture scope (unrepresentable
here); the two remaining value-domain raises map to `EpochError`.
`gov_epoch.py` remains the AUTHORITY machine; this module is its shadow.
