...
/// funding never touches (`entry_price_e8`) and clears `liquidated_this_step` for
/// funded open accounts, matching the Python/kernel `apply_funding` transition.
/// Flat accounts are not funded and keep their prior account state.
fn materialize_apply_funding_auto(
    ...
    for account in &out.accounts {
        ...
        post_accounts.push(Account {
            ...
            // DbC postcondition: every open account that receives funding must
            // leave this transition with Python-equivalent liquidation state.
...
mod tests {
    ...
    fn apply_funding_auto_materializes_accounts_sinks_and_effect() {
        let mut open_liquidated = acct_json("aa", 300_000, 1_000_000, 100_000_000);
        open_liquidated["liquidated_this_step"] = json!(true);
        let mut flat_liquidated = acct_json("cc", 0, 42, 0);
        flat_liquidated["liquidated_this_step"] = json!(true);
        let accounts = json!([
            open_liquidated,
