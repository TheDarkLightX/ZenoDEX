...
//! every account) and an effects-equivalent summary, so Rust can become
//! authority-eligible for `perp_stateful` once parity is proven across all ops.
//!
//! Design:
...
fn fact_bool(facts: &Map<String, Value>, key: &str) -> bool {
    facts
        .get(key)
...
fn fact_i128(facts: &Map<String, Value>, key: &str) -> i128 {
    facts.get(key).and_then(|v| as_i128(v).ok()).unwrap_or(0)
}
...
pub fn materialize_isolated_op(request: &Value) -> Value {
    ...
    let facts_obj = obj
        ...
        .unwrap_or_default();
    let facts = Facts {
        operator_ok: fact_bool(&facts_obj, "operator_ok"),
        sender_bound_ok: fact_bool(&facts_obj, "sender_bound_ok"),
        all_positions_flat: fact_bool(&facts_obj, "all_positions_flat"),
        balance_available: fact_i128(&facts_obj, "balance_available"),
        oracle_adapter_ok: fact_bool(&facts_obj, "oracle_adapter_ok"),
        oracle_authorization_ok: fact_bool(&facts_obj, "oracle_authorization_ok"),
    };
...
mod tests {
    ...
    #[test]
    fn malformed_request_rejects() {
        assert_eq!(
            materialize_isolated_op(&json!({"bad": 1}))["reject_reason"],
