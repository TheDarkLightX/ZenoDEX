"""Research-only N-party clearinghouse market snapshot validation.

This module is deliberately outside :mod:`src.core`.  The model is retained as
design evidence, but its types and validators are not part of the production
state graph and the entire :mod:`src.nonproduction` package is removed from
...
def validate_np_account_record(*, account: Any, pubkey_bytes48: PubkeyBytes48) -> None:
    ...
    if type(account.pubkey) is not str or not account.pubkey:
...
def validate_np_pending_intent_record(*, intent: Any, pubkey_bytes48: PubkeyBytes48) -> None:
    ...
    if type(intent.pubkey) is not str or not intent.pubkey:
...
def _validate_market_identity(*, kind: str, expected_kind: str, quote_asset: str) -> None:
    ...
    if type(quote_asset) is not str or not quote_asset:
...
def _validate_global_keyset(global_state: Mapping[str, int]) -> None:
    if any(type(key) is not str for key in global_state):
        raise TypeError("global_state keys must be exact strings")
...
def _validate_account_collection(
    *,
    ...
    for account in accounts:
        if type(account) is not account_type:
...
def _validate_pending_intents(
    *,
    ...
    if type(pending_intents) is not tuple:
        raise TypeError("pending_intents must be a tuple")
    intent_bytes: list[bytes] = []
    for intent in pending_intents:
        if type(intent) is not pending_intent_type:
