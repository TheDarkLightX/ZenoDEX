"""
Sum-boost AMM swap kernel (v1 semantics).

Invariant family (symmetric, K(x,0)=0):

  B(x,y) := x*y*(x+y)
  K_scaled(x,y) := B(x,y) * (mu_den + mu_num*(x+y))

Equivalently with μ = mu_num/mu_den:

  K(x,y) = x*y*(x+y) * (1 + μ*(x+y))
        = x*y*(x+y) + μ*x*y*(x+y)^2
...
def _sum_boost_k(*, x: int, y: int, mu_num: int, mu_den: int) -> int:
    if x < 0 or y < 0:
    ...
    s = int(x) + int(y)
    base = int(x) * int(y) * s
    factor = int(mu_den) + int(mu_num) * s
    return base * factor


def _min_y_satisfying_k(*, x1: int, y_max: int, k0: int, mu_num: int, mu_den: int) -> int:
    if x1 <= 0:
    ...
    for _ in range(steps):
        if lo >= hi:
            break
        mid = (lo + hi) // 2
        if _sum_boost_k(x=x1, y=mid, mu_num=mu_num, mu_den=mu_den) >= k0:
            hi = mid
        else:
            lo = mid + 1
    return hi
...
def _min_x_satisfying_k(*, x0: int, y1: int, k0: int, mu_num: int, mu_den: int) -> int:
    if x0 <= 0:
    ...
    if _sum_boost_k(x=x0, y=y1, mu_num=mu_num, mu_den=mu_den) >= k0:
        return int(x0)

    lo = int(x0) + 1
    hi = lo
    for _ in range(128):
        if _sum_boost_k(x=hi, y=y1, mu_num=mu_num, mu_den=mu_den) >= k0:
            break
    ...
    for _ in range(steps):
        if lo >= hi:
            break
        mid = (lo + hi) // 2
        if _sum_boost_k(x=mid, y=y1, mu_num=mu_num, mu_den=mu_den) >= k0:
            hi = mid
        else:
            lo = mid + 1
    return hi
...
def swap_exact_in(
    *,
    ...
    k0 = _sum_boost_k(x=x0, y=y0, mu_num=int(mu_num), mu_den=int(mu_den))
    x_effective = x0 + dx_net
    if x_effective <= 0:
        raise ValueError("invalid post-swap reserve_in")

    y1 = _min_y_satisfying_k(x1=x_effective, y_max=y0, k0=k0, mu_num=int(mu_num), mu_den=int(mu_den))
    ...
    x_after = x0 + dx_gross
    k1 = _sum_boost_k(x=x_after, y=y1, mu_num=int(mu_num), mu_den=int(mu_den))
    if k1 < k0:
...
def swap_exact_out(
    *,
    ...
    x_eff = _min_x_satisfying_k(x0=x0, y1=y_target, k0=k0, mu_num=int(mu_num), mu_den=int(mu_den))
    if x_eff <= x0:
        x_eff = x0 + 1
    ...
    x_after = x0 + dx_gross
    y_after = y_target
    k1 = _sum_boost_k(x=x_after, y=y_after, mu_num=int(mu_num), mu_den=int(mu_den))
    if k1 < k0:
        raise ValueError("invariant violation: K decreased")
