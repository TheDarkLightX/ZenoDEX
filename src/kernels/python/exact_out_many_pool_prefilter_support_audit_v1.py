...
class ExactOutManyPoolPrefilterSupportAudit:
...
def audit_exact_out_many_pool_prefilter_support(
    ...
    *,
...
) -> ExactOutManyPoolPrefilterSupportAudit:
    if int(max_legs) <= 0:
    ...
    if not feasible_rows:
        raise ValueError("no feasible pools for exact-out support audit")

    feasible_pools = tuple(pool for pool, _cap, _in_i in feasible_rows)
    if len(feasible_pools) > int(max_full_domain_pools):
        raise ValueError("prefilter support audit exceeded max_full_domain_pools")
    ...
    bad_candidates = tuple(
        ...
        if not {leg.pool_id for leg in candidate.legs}.issubset(selected_pool_id_set)
    )
    ...
    return ExactOutManyPoolPrefilterSupportAudit(
        ...
        support_sound=(counterexample_quote is None),
