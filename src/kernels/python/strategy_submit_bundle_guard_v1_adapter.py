...
from ...state.intents import SignedIntent, require_exact_intent
...
def _require_signed_intents(value: object) -> tuple[SignedIntentEnvelope, ...]:
    ...
    for env in value:
        if type(env) is not SignedIntentEnvelope:
            raise TypeError("signed_intents must contain SignedIntentEnvelope items")
        require_exact_intent(env.intent)
        out.append(env)
    return tuple(out)
...
def _quote_receipt_hash(value: object) -> str | None:
    ...
    if type(receipt_hash) is not str or not receipt_hash:
        return None
    return receipt_hash
...
def _verify_signed_intent_bundle(
    *,
    ...
    signatures_present = all(
        type(env.signature) is str and env.signature.strip() for env in signed_intents
    )
    ...
    try:
        ...
        operations_roundtrip_ok = len(parsed) == len(signed_intents) and all(
            canonical_json_bytes(build_dex_intent_signing_dict_v1(parsed_env.intent))
            == canonical_json_bytes(build_dex_intent_signing_dict_v1(env.intent))
            and parsed_env.signature == env.signature
            and type(parsed_env.quote_receipt) is dict
            and type(env.quote_receipt) is dict
            and canonical_json_bytes(parsed_env.quote_receipt)
            == canonical_json_bytes(env.quote_receipt)
