"""Adapter for the `dex_step_core_v2` kernel spec.
...
# Bound to the kernel IR hash of `src/kernels/dex/dex_step_core_v2.yaml`.
# Checked by the toolchain's adapter lint step (fail-closed by default).
IR_HASH = "sha256:ffbb1375c247b0618ab0bce01f25499353fef6646a76f6fec5d4a0fb398f10af"
...
def _handle_generic(adapter: DexStepCoreV2Adapter, command: Any) -> Any:
    return _kernel_step(state=adapter._state, command=command, ir=adapter.ir)


def _commit_effect(adapter: DexStepCoreV2Adapter, effect_id: str, value: Any) -> None:
    adapter._pending_effects[str(effect_id)] = value


ACTION_HANDLERS: dict[str, Callable[[DexStepCoreV2Adapter, Any], Any]] = {
    "create_pool": _handle_generic,
    "swap_a_for_b": _handle_generic,
    "swap_b_for_a": _handle_generic,
    "swap_a_for_b_exact_out": _handle_generic,
    "swap_b_for_a_exact_out": _handle_generic,
    "add_liquidity": _handle_generic,
    "remove_liquidity": _handle_generic,
}


EFFECT_HANDLERS: dict[str, Callable[[DexStepCoreV2Adapter, str, Any], None]] = {
    "event": _commit_effect,
}


def make_adapter(ir: Any) -> DexStepCoreV2Adapter:
    return DexStepCoreV2Adapter(ir=ir)
