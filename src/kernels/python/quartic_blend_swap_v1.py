"""
Quartic-blend AMM swap kernel (v1 semantics).
...
def _quartic_blend_k(*, x: int, y: int, c_num: int, c_den: int) -> int:
    if x < 0 or y < 0:
...
def _min_y_satisfying_k(*, x1: int, y_max: int, k0: int, c_num: int, c_den: int) -> int:
    ...
    for _ in range(steps):
        if lo >= hi:
            break
        mid = (lo + hi) // 2
        if _quartic_blend_k(x=x1, y=mid, c_num=c_num, c_den=c_den) >= k0:
            hi = mid
        else:
            lo = mid + 1
    return hi
...
def _min_x_satisfying_k(*, x0: int, y1: int, k0: int, c_num: int, c_den: int) -> int:
    ...
    if _quartic_blend_k(x=x0, y=y1, c_num=c_num, c_den=c_den) >= k0:
        return int(x0)

    lo = int(x0) + 1
    hi = lo
    for _ in range(128):
        if _quartic_blend_k(x=hi, y=y1, c_num=c_num, c_den=c_den) >= k0:
            break
    ...
    for _ in range(steps):
        if lo >= hi:
            break
        mid = (lo + hi) // 2
        if _quartic_blend_k(x=mid, y=y1, c_num=c_num, c_den=c_den) >= k0:
            hi = mid
        else:
            lo = mid + 1
    return hi
...
def swap_exact_in(
    *,
    ...
    k0 = _quartic_blend_k(x=x0, y=y0, c_num=int(c_num), c_den=int(c_den))
    ...
    x_after = x0 + dx_gross
    k1 = _quartic_blend_k(x=x_after, y=y1, c_num=int(c_num), c_den=int(c_den))
    if k1 < k0:
...
def swap_exact_out(
    *,
    ...
    x_after = x0 + dx_gross
    y_after = y_target
    k1 = _quartic_blend_k(x=x_after, y=y_after, c_num=int(c_num), c_den=int(c_den))
    if k1 < k0:
        raise ValueError("invariant violation: K decreased")
