from __future__ import annotations

from dataclasses import dataclass, field
from typing import Any, Callable, Mapping

from ...core.perp_funding_apply_gate import evaluate_perp_funding_apply_gate

IR_HASH = "sha256:c62ac6429ab15e1d35066467854b647efe69c761a75984615c8b01a6c95fd15c"


def _guard_false(action_id: str) -> Any:
    from ESSO.kernel.interpreter import StepError  # type: ignore

    return StepError(code="GuardFalse", message=f"guard false for action '{action_id}'")


@dataclass
class PerpFundingApplyV1NativeAdapter:
    ir: Any
    _state: dict[str, Any] = field(default_factory=dict)
    _pending_effects: dict[str, Any] = field(default_factory=dict)

    def reset(self, *, state: Mapping[str, Any]) -> None:
        self._state = dict(state)
        self._pending_effects = {}

    def get_state(self) -> Mapping[str, Any]:
        return dict(self._state)

    def apply(self, command: Any) -> Any:
        self._pending_effects = {}
        handler = ACTION_HANDLERS.get(str(getattr(command, "tag", "")))
        if handler is None:
            from ESSO.kernel.interpreter import StepError  # type: ignore

            return StepError(code="UnknownAction", message="no handler for command.tag")

        res = handler(self, command)
        from ESSO.kernel.interpreter import StepOk  # type: ignore

        if isinstance(res, StepOk):
            self._state = dict(res.state)
            for eff_id, value in res.effects.items():
                eff_handler = EFFECT_HANDLERS.get(str(eff_id))
                if eff_handler is None:
                    continue
                eff_handler(self, str(eff_id), value)
        return res

    def drain_effects(self) -> Mapping[str, Any]:
        out = dict(self._pending_effects)
        self._pending_effects = {}
        return out


def _handle_evaluate_funding_apply_gate(adapter: PerpFundingApplyV1NativeAdapter, command: Any) -> Any:
    from ESSO.kernel.interpreter import StepOk  # type: ignore

    del command
    s = adapter._state
    action_id = "evaluate_funding_apply_gate"
    try:
        outcome = evaluate_perp_funding_apply_gate(
            now_epoch=s["now_epoch"],
            epoch_phase=s["epoch_phase"],
            auth_ok=s["auth_ok"],
            index_price_e8=s["index_price_e8"],
            oracle_last_update_epoch=s["oracle_last_update_epoch"],
            max_oracle_staleness_epochs=s["max_oracle_staleness_epochs"],
            oracle_seen=s["oracle_seen"],
            funding_last_applied_epoch=s["funding_last_applied_epoch"],
            funding_cap_bps=s["funding_cap_bps"],
            new_rate_bps=s["new_rate_bps"],
            position_base=s["position_base"],
            collateral_quote=s["collateral_quote"],
            maintenance_margin_bps=s["maintenance_margin_bps"],
            depeg_buffer_bps=s["depeg_buffer_bps"],
            funding_paid_cumulative=s["funding_paid_cumulative"],
        )
    except (KeyError, TypeError, ValueError):
        return _guard_false(action_id)

    return StepOk(
        state=dict(s),
        effects={
            "phase_allows_funding": bool(outcome.phase_allows_funding),
            "auth_ok": bool(outcome.auth_ok),
            "index_price_ok": bool(outcome.index_price_ok),
            "staleness_param_ok": bool(outcome.staleness_param_ok),
            "oracle_seen_ok": bool(outcome.oracle_seen_ok),
            "oracle_fresh": bool(outcome.oracle_fresh),
            "funding_not_applied_this_epoch": bool(outcome.funding_not_applied_this_epoch),
            "rate_within_cap": bool(outcome.rate_within_cap),
            "position_open_ok": bool(outcome.position_open_ok),
            "funding_payment_quote": int(outcome.funding_payment_quote),
            "collateral_after_quote": int(outcome.collateral_after_quote),
            "collateral_bounds_ok": bool(outcome.collateral_bounds_ok),
            "maint_req_quote": int(outcome.maint_req_quote),
            "maint_margin_ok": bool(outcome.maint_margin_ok),
            "cumulative_after_quote": int(outcome.cumulative_after_quote),
            "cumulative_bounds_ok": bool(outcome.cumulative_bounds_ok),
            "funding_apply_allowed": bool(outcome.funding_apply_allowed),
        },
    )


def _commit_effect(adapter: PerpFundingApplyV1NativeAdapter, effect_id: str, value: Any) -> None:
    adapter._pending_effects[str(effect_id)] = value


ACTION_HANDLERS: dict[str, Callable[[PerpFundingApplyV1NativeAdapter, Any], Any]] = {
    "evaluate_funding_apply_gate": _handle_evaluate_funding_apply_gate,
}

EFFECT_HANDLERS: dict[str, Callable[[PerpFundingApplyV1NativeAdapter, str, Any], None]] = {
    "phase_allows_funding": _commit_effect,
    "auth_ok": _commit_effect,
    "index_price_ok": _commit_effect,
    "staleness_param_ok": _commit_effect,
    "oracle_seen_ok": _commit_effect,
    "oracle_fresh": _commit_effect,
    "funding_not_applied_this_epoch": _commit_effect,
    "rate_within_cap": _commit_effect,
    "position_open_ok": _commit_effect,
    "funding_payment_quote": _commit_effect,
    "collateral_after_quote": _commit_effect,
    "collateral_bounds_ok": _commit_effect,
    "maint_req_quote": _commit_effect,
    "maint_margin_ok": _commit_effect,
    "cumulative_after_quote": _commit_effect,
    "cumulative_bounds_ok": _commit_effect,
    "funding_apply_allowed": _commit_effect,
}


def make_adapter(ir: Any) -> PerpFundingApplyV1NativeAdapter:
    return PerpFundingApplyV1NativeAdapter(ir=ir)
