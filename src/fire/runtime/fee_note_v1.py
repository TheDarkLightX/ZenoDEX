...
OBJECT_NAME = "FeeNote"
OBJECT_VERSION = "v1"
OBJECT_FAMILY = "capped_index_note"
SETTLEMENT_ASSET = "zUSD"
PAYOFF_SUMMARY = "N * min(FeeIndex_T, Cap)"
...
class FeeNoteArtifact:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.terms, FeeNoteTerms):
            raise TypeError("terms must be a FeeNoteTerms")
        object.__setattr__(
            self,
            "artifact_lower",
...
@dataclass(frozen=True)
class FeeNoteSettlement:
    artifact: FeeNoteArtifact
    witness_final: int
    holder_posted: int
    writer_posted: int
    holder_delta: int
...
class FeeNoteResult:
    ok: bool
    settlement: FeeNoteSettlement | None = None
    error: str | None = None


def _compile_state(terms: FeeNoteTerms) -> ref.State:
    result = ref.step(
        ref.init_state(),
        ref.Command(
    ...
    if not result.ok or result.state is None:
        raise RuntimeError(result.error or "compile_fee_note rejected")
    return result.state


def _certificate_env(terms: FeeNoteTerms) -> FireCertEnv:
    return FireCertEnv(
        exact_values={
        ...
        source_bounds={
            "fee_final": FireInterval(lower=0, upper=terms.source_upper),
        },
    )


def _compiled_state_from_artifact(artifact: FeeNoteArtifact) -> ref.State:
    state = ref.State(
...
def compile_terms(terms: FeeNoteTerms) -> FeeNoteArtifact:
    return compile_fmos_artifact(SPEC, terms)


def holder_collateral_required(artifact: FeeNoteArtifact) -> int:
    return fmos_holder_collateral_required(artifact)


def writer_collateral_required(artifact: FeeNoteArtifact) -> int:
    return fmos_writer_collateral_required(artifact)


def build_manifest(artifact: FeeNoteArtifact) -> FireObjectManifest:
    if not isinstance(artifact, FeeNoteArtifact):
        raise TypeError("artifact must be a FeeNoteArtifact")
    return build_fmos_manifest(SPEC, artifact)


def render_object_card(artifact: FeeNoteArtifact) -> str:
    return render_fmos_object_card(SPEC, artifact)


SPEC_PATH = resolve_fire_spec_path("fee_note_v1")


SPEC = bind_fire_math_object_spec_file(
    SPEC_PATH,
    terms_type=FeeNoteTerms,
    artifact_type=FeeNoteArtifact,
...
def verify_and_settle(
    *,
    ...
    if not ok or state is None or receipt is None:
        return FeeNoteResult(ok=False, error=err or "settlement_rejected")

    return FeeNoteResult(
        ok=True,
        settlement=FeeNoteSettlement(
            artifact=artifact,
