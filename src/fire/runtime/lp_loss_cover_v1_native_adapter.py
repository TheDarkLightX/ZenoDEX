"""Native shell adapter for `fire_lp_loss_cover_v1`.

Used with:
  python3 -m ESSO shell-lint src/kernels/dex/fire_lp_loss_cover_v1.yaml --adapter <this>:make_simulation_adapter
  python3 -m ESSO verify-shell src/kernels/dex/fire_lp_loss_cover_v1.yaml --adapter <this>:make_simulation_adapter
"""
...
def _commit_effect(adapter: "FireLPLossCoverV1NativeAdapter", effect_id: str, value: Any) -> None:
    adapter._pending_effects[str(effect_id)] = value


def _commit_receipt_effect(adapter: "FireLPLossCoverV1NativeAdapter", args: Mapping[str, Any]) -> None:
...
def _commit_settlement_packet_effect(adapter: "FireLPLossCoverV1NativeAdapter", args: Mapping[str, Any]) -> None:
    receipt = args.get("verifier_receipt")
...
def _run_kernel_step(adapter: "FireLPLossCoverV1NativeAdapter", tag: str, args: Mapping[str, Any]) -> Any:
    ...
    if adapter.require_settlement_receipt:
        receipt_error = validate_persisted_bundle_settlement_receipt(
            ...
            command_tag=tag,
            witness_inputs={
                "witness_hodl_final": args.get("witness_hodl_final_in"),
                "witness_lpv_final": args.get("witness_lpv_final_in"),
            },
        )
...
def _handle_compile_lp_loss_cover(adapter: "FireLPLossCoverV1NativeAdapter", command: Any) -> Any:
    return _run_kernel_step(adapter, "compile_lp_loss_cover", dict(getattr(command, "args", {}) or {}))


def _handle_firev_accept_and_settle(adapter: "FireLPLossCoverV1NativeAdapter", command: Any) -> Any:
...
@dataclass
class FireLPLossCoverV1NativeAdapter:
    ir: Any
...
def make_adapter(ir: Any) -> FireLPLossCoverV1NativeAdapter:
    return FireLPLossCoverV1NativeAdapter(ir=ir)


def make_simulation_adapter(ir: Any) -> FireLPLossCoverV1NativeAdapter:
    return FireLPLossCoverV1NativeAdapter(ir=ir, require_settlement_receipt=False)


ACTION_HANDLERS: dict[str, Callable[[FireLPLossCoverV1NativeAdapter, Any], Any]] = {
    "compile_lp_loss_cover": _handle_compile_lp_loss_cover,
    "firev_accept_and_settle": _handle_firev_accept_and_settle,
}


EFFECT_HANDLERS: dict[str, Callable[[FireLPLossCoverV1NativeAdapter, str, Any], None]] = {
...
__all__ = [
    "FireLPLossCoverV1NativeAdapter",
    "IR_HASH",
    "make_adapter",
