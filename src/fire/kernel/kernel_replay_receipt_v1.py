...
from src.fire.pathing_v1 import fire_kernel_replay_receipt_schema_path
...
FIRE_KERNEL_REPLAY_RECEIPT_SCHEMA = "zenodex/fire-kernel-replay-receipt/v1"
FIRE_KERNEL_REPLAY_RECEIPT_CHECK_REPORT_SCHEMA = "zenodex/fire-kernel-replay-receipt-check-report/v1"
...
def _validate_against_schema(
    ...
    *,
    ...
    if not errors:
        return True, None
    first = errors[0]
    return False, f"kernel_replay_receipt_schema_invalid:{_error_path(first)}:{first.message}"
...
def _parameter_values_from_instance(object_instance: "FireObjectInstanceManifest") -> dict[str, int]:
    return {item.name: item.value for item in object_instance.parameters}
...
def _compile_command(object_id: str, parameter_values: Mapping[str, int]) -> tuple[str, dict[str, int]]:
    if object_id not in _COMPILE_COMMAND_SPECS:
        raise KeyError(f"unsupported FIRE kernel replay object_id: {object_id}")
...
def _run_kernel_replay(
    *,
    object_id: str,
    object_instance: "FireObjectInstanceManifest",
    replay_input: FireReplayInput,
) -> tuple[dict[str, object], dict[str, object], str, dict[str, int], dict[str, object], dict[str, object], str, dict[str, int]]:
    ...
    compiled_state = _normalize_scalar_mapping("compiled_state", asdict(compile_result.state))
    compiled_effects = _normalize_scalar_mapping("compiled_effects", dict(compile_result.effects or {}))
    if compiled_state.get("phase") != "Compiled":
        raise RuntimeError("kernel replay compile command did not reach Compiled phase")

    settlement_tag, settlement_args = _settlement_command(object_id, replay_input)
    settlement_result = module.step(
        compile_result.state,
        module.Command(tag=settlement_tag, args=dict(settlement_args)),
    )
    if not settlement_result.ok or settlement_result.state is None:
        raise RuntimeError(settlement_result.error or f"{settlement_tag} rejected")
    settlement_state = _normalize_scalar_mapping("settlement_state", asdict(settlement_result.state))
    settlement_effects = _normalize_scalar_mapping("settlement_effects", dict(settlement_result.effects or {}))
    if settlement_state.get("phase") != "Settled":
        raise RuntimeError("kernel replay settlement command did not reach Settled phase")
    return (
        compiled_state,
        compiled_effects,
        compile_tag,
...
def build_fire_kernel_replay_receipt(
    *,
    ...
    kernel_settlement_receipt_sha256: str,
) -> dict[str, object]:
    ...
    (
        ...
        settlement_state,
        settlement_effects,
        settlement_command_tag,
        settlement_command_args,
    ) = _run_kernel_replay(
    ...
    if writer_delta != -holder_delta:
        raise RuntimeError("kernel replay writer_delta does not negate holder_delta")
    if not firev_accept:
        raise RuntimeError("kernel replay did not produce firev_accept=true")

    compile_state_sha256 = _sha256_json(compiled_state)
    compile_effects_sha256 = _sha256_json(compiled_effects)
    settlement_state_sha256 = _sha256_json(settlement_state)
    settlement_effects_sha256 = _sha256_json(settlement_effects)
    transcript_sha256 = _sha256_json(
        {
            "compile_command_tag": compile_command_tag,
            "compile_command_args": compile_command_args,
            "compile_state_sha256": compile_state_sha256,
            "compile_effects_sha256": compile_effects_sha256,
            "settlement_command_tag": settlement_command_tag,
            "settlement_command_args": settlement_command_args,
            "settlement_state_sha256": settlement_state_sha256,
            "settlement_effects_sha256": settlement_effects_sha256,
        }
    )
    delta_sha256 = _sha256_json(
        {
    ...
    return {
        "schema": FIRE_KERNEL_REPLAY_RECEIPT_SCHEMA,
        "object_id": entry.object_id,
...
class FireKernelReplayReceiptVerification:
    object_id: str
    object_hash: str
    instance_hash: str
    cert_sha256: str
    ...
    def to_report_dict(self) -> dict[str, object]:
        return {
            ...
            "kernel_settlement_receipt_sha256": self.kernel_settlement_receipt_sha256,
...
@dataclass(frozen=True)
class FireKernelReplayReceiptFileVerification:
    receipt_path: Path
    schema_path: Path
    ...
    def to_report_dict(self) -> dict[str, object]:
        return {
            "schema": FIRE_KERNEL_REPLAY_RECEIPT_CHECK_REPORT_SCHEMA,
            "ok": True,
...
def verify_fire_kernel_replay_receipt(
    payload: Mapping[str, object],
    *,
    object_manifest: "FireObjectManifest",
    object_instance: "FireObjectInstanceManifest",
    ...
    expected_compile_receipt_sha256: str | None = None,
    expected_kernel_receipt_sha256: str | None = None,
    expected_kernel_eval_receipt_sha256: str | None = None,
    expected_kernel_settlement_receipt_sha256: str | None = None,
) -> tuple[bool, str | None, FireKernelReplayReceiptVerification | None]:
    try:
        ...
        compile_receipt_sha256 = _require_sha256_prefixed("compile_receipt_sha256", payload.get("compile_receipt_sha256"))
        kernel_receipt_sha256 = _require_sha256_prefixed("kernel_receipt_sha256", payload.get("kernel_receipt_sha256"))
        kernel_eval_receipt_sha256 = _require_sha256_prefixed(
            "kernel_eval_receipt_sha256", payload.get("kernel_eval_receipt_sha256")
        )
        kernel_settlement_receipt_sha256 = _require_sha256_prefixed(
            "kernel_settlement_receipt_sha256",
            payload.get("kernel_settlement_receipt_sha256"),
        )
        ...
        _require_sha256_prefixed("compile_state_sha256", payload.get("compile_state_sha256"))
        _require_sha256_prefixed("compile_effects_sha256", payload.get("compile_effects_sha256"))
        ...
        _require_sha256_prefixed("settlement_state_sha256", payload.get("settlement_state_sha256"))
    ...
    if expected_compile_receipt_sha256 is not None and compile_receipt_sha256 != expected_compile_receipt_sha256:
        return False, "expected_compile_receipt_sha256_mismatch", None
    ...
    if (
        expected_kernel_settlement_receipt_sha256 is not None
    ...
    if dict(payload) != expected:
        return False, "kernel_replay_receipt_mismatch", None
    return (
        True,
        None,
        FireKernelReplayReceiptVerification(
            object_id=object_id,
...
def verify_fire_kernel_replay_receipt_file(
    path: str | Path,
    *,
    ...
    expected_kernel_eval_receipt_sha256: str | None = None,
    expected_kernel_settlement_receipt_sha256: str | None = None,
) -> tuple[bool, str | None, FireKernelReplayReceiptFileVerification | None]:
    from src.fire.registry.instance_v1 import load_fire_object_instance, verify_fire_object_instance_against_manifest
    from src.fire.registry.object_manifest_v1 import load_fire_object_manifest, verify_fire_object_manifest
    ...
    schema_path = fire_kernel_replay_receipt_schema_path().resolve()
    ...
    object_manifest, _ = load_fire_object_manifest(manifest_path)
    ok, err = verify_fire_object_manifest(object_manifest)
    if not ok:
        return False, f"object_manifest_invalid:{err or 'unknown'}", None

    object_instance, _ = load_fire_object_instance(instance_path)
    ok, err, _ = verify_fire_object_instance_against_manifest(object_instance, object_manifest=object_manifest)
    if not ok:
    ...
    ok, err, verification = verify_fire_kernel_replay_receipt(
        payload,
        object_manifest=object_manifest,
        object_instance=object_instance,
        replay_input=replay_input,
    ...
    if not ok or verification is None:
        return False, err or "kernel_replay_receipt_verification_failed", None
    ...
    return (
        True,
        None,
        FireKernelReplayReceiptFileVerification(
            ...
            compile_receipt_sha256=verification.compile_receipt_sha256,
            ...
            kernel_settlement_receipt_sha256=verification.kernel_settlement_receipt_sha256,
            transcript_sha256=verification.transcript_sha256,
            delta_sha256=verification.delta_sha256,
...
def write_fire_kernel_replay_receipt(
    path: str | Path,
    *,
    object_manifest: "FireObjectManifest",
    object_instance: "FireObjectInstanceManifest",
    ...
    out_path = Path(path)
    payload = build_fire_kernel_replay_receipt(
        object_manifest=object_manifest,
        object_instance=object_instance,
        replay_input=replay_input,
        replay_input_sha256=replay_input_sha256,
...
__all__ = [
    "FIRE_KERNEL_REPLAY_RECEIPT_CHECK_REPORT_SCHEMA",
    "FIRE_KERNEL_REPLAY_RECEIPT_SCHEMA",
    "FireKernelReplayReceiptFileVerification",
    "FireKernelReplayReceiptVerification",
    "build_fire_kernel_replay_receipt",
    "verify_fire_kernel_replay_receipt",
    "verify_fire_kernel_replay_receipt_file",
    "write_fire_kernel_replay_receipt",
]
