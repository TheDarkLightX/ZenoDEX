...
generators for the live autonomous-governance lane. Live use must call them
through `gov_loop.py`, `autonomous_governance_q_policy.py`, the trajectory
runner, and the file-backed live admission wrapper. A mis-tuned or poisoned
proposer can only produce a candidate; the gates, context hashes, policy pins,
and session-store admission decide whether state changes. Determinism is
load-bearing: every proposer below is pure integer / fixed-point code with NO
floats and NO randomness, and inputs are type-validated so the integer contract
actually holds.
