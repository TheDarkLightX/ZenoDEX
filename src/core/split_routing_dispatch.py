...
def _is_candidate_value_error(exc: ValueError) -> bool:
    """Return whether a ValueError is an infeasible local split candidate.

    Split routing probes many candidate pools, allocations, and output targets.
    Domain failures should remove only the candidate being probed. Errors that
    name an invariant, malformed quote, or unexpected allocation state are
    implementation faults and must propagate.
    """
...
def best_split_many_pools_exact_in_for_pools(
    ...
    *,
    ...
    if best_alloc is None:
        raise RuntimeError("split routing failed to initialize an allocation")
