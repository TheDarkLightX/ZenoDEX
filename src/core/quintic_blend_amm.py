"""
Quintic-blend AMM (new curve family):

  K(x,y) = x*y*(x+y) * (x^2 + y^2 + c*x*y)

with c = c_num/c_den (rational, c>=0).

This wraps `src/kernels/python/quintic_blend_swap_v1.py` in the functional core API.
"""
...
from ..kernels.python.quintic_blend_swap_v1 import swap_exact_in as _kernel_swap_exact_in_v1
from ..kernels.python.quintic_blend_swap_v1 import swap_exact_out as _kernel_swap_exact_out_v1
from ..state.balances import Amount


def swap_exact_in_quintic_blend(
    reserve_in: Amount,
    reserve_out: Amount,
    amount_in: Amount,
    *,
...
def swap_exact_out_quintic_blend(
    reserve_in: Amount,
    reserve_out: Amount,
    amount_out: Amount,
    *,
