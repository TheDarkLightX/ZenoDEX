...
sound. Those values must be anchored and verified by an external FCIS boundary.
Its purpose is to make the remaining global composition claim small: local edge
receipts plus a complete topology inventory imply declared-path coherence and
ordered gate mediation.
"""
    ...
    for binding in checked:
        fields.extend(
            (
                ...
                bytes.fromhex(binding.source_digest),
            )
        )
...
def _validate_gate_profile(
    gates: tuple[AuthorityGateV1, ...],
) -> tuple[AuthorityGateV1, ...]:
...
def _topological_order(
    nodes: dict[str, AuthorityNodeV1],
    edges: tuple[AuthorityEdgeV1, ...],
) -> tuple[str, ...]:
...
def verify_tree_chord_gate_certificate(
    *,
    ...
    try:
        ...
        source_lineage = _validate_lineage(
        ...
        parent_ids = _exact_text_tuple(
            certificate.parent_edge_ids,
            "parent_edge_ids",
        )
