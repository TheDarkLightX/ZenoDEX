...
from .zdex_fee_allocation_receipt_verification_v1 import VerifiedZDEXFeeAllocationV1
...
def verify_zdex_tokenomics_fee_lane_receipt_v1(
    ...
    if type(governed) is not GovernedZDEXFeeAllocationProfileV1:
        raise TypeError("ZDEX tokenomics fee profile must be verifier-constructed")
    _revalidate_governed_fee_profile(governed)
    candidate.lane_candidate.validate()
    _require_candidate_bindings(candidate, governed)
