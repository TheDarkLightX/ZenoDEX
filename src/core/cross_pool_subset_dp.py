...
DEFAULT_SUBSET_DP_LIMITS = SubsetDPLimits()
FULL_STATE_REFERENCE_LIMITS = SubsetDPLimits(
    max_intents=12,
    max_total_input=20_000,
    max_states_per_subset=500_000,
)
BRUTE_FORCE_TWO_POOL_LIMITS = SubsetDPLimits(max_intents=8, max_total_input=256)
BRUTE_FORCE_K_POOL_LIMITS = SubsetDPLimits(max_intents=7, max_total_input=256, max_pools=5)
...
def solve_two_pool_cpmm_subset_dp(
    ...
    *,
    limits: SubsetDPLimits = DEFAULT_SUBSET_DP_LIMITS,
    trace_mode: str = "path",
...
def solve_two_pool_cpmm_full_state_dp(
    ...
    intents: Iterable[int],
    *,
    limits: SubsetDPLimits = FULL_STATE_REFERENCE_LIMITS,
) -> CrossPoolBatchResult:
...
def brute_force_two_pool_cpmm_batch(
    ...
    p0 = _validate_pool(pool0, name="pool0")
    p1 = _validate_pool(pool1, name="pool1")
    intent_amounts = _normalize_intents(intents, limits=BRUTE_FORCE_TWO_POOL_LIMITS)
    n = len(intent_amounts)
    best_total = -1
...
def brute_force_k_pool_cpmm_batch(
    ...
    intents: Iterable[int],
    *,
    limits: SubsetDPLimits = BRUTE_FORCE_K_POOL_LIMITS,
) -> KPoolBatchResult:
