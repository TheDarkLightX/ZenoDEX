"""Guard functions for the versioned v4 perps engine.
...
from ..perp_v2.types import ActionParams, EpochPhase, PerpState
from .math import (
    BPS_SCALE,
    MAX_COLLATERAL,
    MAX_EPOCH,
...
def guard_apply_funding(state: PerpState, params: ActionParams) -> bool:
    ...
    if not outcome.funding_apply_allowed:
        return False
    v4_requirement = maint_margin_req(
        state.position_base,
        state.index_price_e8,
        state.maintenance_margin_bps,
        state.depeg_buffer_bps,
    )
    return outcome.collateral_after_quote >= v4_requirement
...
def guard_partial_liquidate(state: PerpState, params: ActionParams) -> bool:
    ...
    - fraction_bps == 0: auto-compute the exact minimum bounded fraction
    ...
    - The resulting collateral after penalty must stay non-negative
      and within bounds.
    """
    if state.epoch_phase != EpochPhase.OPEN or not params.auth_ok:
        return False
    if state.position_base == 0 or state.index_price_e8 <= 0:
        return False
    if not is_oracle_fresh(
        state.now_epoch,
