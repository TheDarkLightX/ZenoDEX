...
def _validate_cow_pair_index(
    *,
    ...
    reciprocal_counts: Counter[tuple[str, AssetId, AssetId, int, int]] = Counter()
    example_for_key: Dict[tuple[str, AssetId, AssetId, int, int], str] = {}
    for intent_id, entry in entries.items():
        if entry.asset_in == entry.asset_out:
            return False, f"COW_NETTED requires opposite assets: intent_id={intent_id}"
        key = (
            entry.pool_id,
            entry.asset_in,
            entry.asset_out,
        ...
        reciprocal_counts[key] += 1
        example_for_key.setdefault(key, intent_id)

    # CoW fills do not encode a pair id. The load-bearing condition is therefore
    # multiset reciprocity: every directed fill shape must have the same count as
    # its exact reverse. Requiring a unique counterparty would reject valid batches
    # with two or more identical reciprocal pairs generated by the live clearer.
    seen_keys: set[tuple[str, AssetId, AssetId, int, int]] = set()
    for key, count in reciprocal_counts.items():
        if key in seen_keys:
            continue
        pool_id, asset_in, asset_out, amount_in, amount_out = key
        reciprocal_key = (pool_id, asset_out, asset_in, amount_out, amount_in)
        reciprocal_count = reciprocal_counts.get(reciprocal_key, 0)
        if reciprocal_count != count:
            example_intent_id = example_for_key[key]
            return (
                False,
                "COW_NETTED reciprocal multiset mismatch: "
                f"intent_id={example_intent_id} count={count} reciprocal_count={reciprocal_count}",
            )
        seen_keys.add(key)
        seen_keys.add(reciprocal_key)
    return True, None
