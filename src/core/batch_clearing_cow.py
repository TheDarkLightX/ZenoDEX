    ...
    *,
    ...
    if use_bruteforce:
        ...
        for candidate in side_01:
            bal0[candidate.sender] = int(balances.get(candidate.sender, a0))
        for candidate in side_10:
            bal1[candidate.sender] = int(balances.get(candidate.sender, a1))
    ...
    remaining.extend([candidate.intent for candidate in side_01 if candidate.intent.intent_id not in matched_ids])
    remaining.extend([candidate.intent for candidate in side_10 if candidate.intent.intent_id not in matched_ids])
