...
from .lane_composition_receipt_verification_v1 import VerifiedLaneCompositionV1

VERIFIED_ROUTE_COMPOSITION_SCHEMA_V1: Final = "zenodex/verified-route-composition/v1"
ROUTE_COMPOSITION_ASSUMPTION_SCHEMA_V1: Final = (
    "zenodex/route-composition-assumption/v1"
)
...
class RouteCompositionReceiptCandidateV1:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.profile, EconomicProfileSnapshotV1):
            raise TypeError("route composition economic profile must be typed")
        if not isinstance(self.occurrence, EconomicCommandOccurrenceV1):
            raise TypeError("route composition command occurrence must be typed")
        if type(self.lane_journals) is not tuple:
            raise TypeError("route composition lane journals must be an exact tuple")
        if any(not isinstance(item, LaneCompositionJournalV1) for item in self.lane_journals):
            raise TypeError("route composition lane journals must be typed")
        if type(self.verified_lanes) is not tuple:
            raise TypeError("route composition verified lane witnesses must be an exact tuple")
        if any(not isinstance(item, VerifiedLaneCompositionV1) for item in self.verified_lanes):
            raise TypeError("route composition verified lane witnesses must be typed")
        if not isinstance(self.route_journal, RouteCompositionJournalV1):
            raise TypeError("route composition route journal must be typed")
        if not isinstance(self.receipt, RouteCompositionReceiptEnvelopeV1):
            raise TypeError("route composition receipt envelope must be typed")
    ...
    *,
    ...
    for name, root in roots.items():
        if (
            ...
            or len(root) != 66
            or not root.startswith("0x")
            or root == "0x" + "00" * 32
        ):
            raise ValueError(f"route assumption {name} must be a nonzero canonical root")
        try:
            decoded = bytes.fromhex(root[2:])
        except ValueError as exc:
            raise ValueError(
                f"route assumption {name} must be a nonzero canonical root"
            ) from exc
        if len(decoded) != 32 or root != root.lower():
            raise ValueError(f"route assumption {name} must be a nonzero canonical root")
...
def verify_route_composition_receipt_v1(
    ...
    if not isinstance(candidate, RouteCompositionReceiptCandidateV1):
        raise TypeError("route composition receipt candidate must be typed")
    route = _require_exact_route_composition_binding_v1(candidate)
    ...
    route_journal_bytes = canonical_global_bytes_v1(candidate.route_journal)
    ...
    return VerifiedRouteCompositionV1(
        ...
        _VerifiedRouteCompositionFieldsV1(
            ...
            route.ordered_lanes,
            tuple(item.binding_root for item in candidate.verified_lanes),
            tuple(item.journal_root for item in candidate.lane_journals),
            candidate.route_journal.journal_root,
