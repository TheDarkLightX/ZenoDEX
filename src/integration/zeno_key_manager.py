...
def is_secret_field_name(key: object) -> bool:
    ...
    if any(token in SECRET_FIELD_TOKEN_NAMES for token in tokens):
        return True
    return any(pair in zip(tokens, tokens[1:], strict=False) for pair in SECRET_FIELD_TOKEN_PAIRS)
...
def _bls_basic() -> Any:
    # Optional dependency state can be monkeypatched or partially imported; keep
    # the local signer fail-closed even when Python assertions are stripped.
    _require_bls()
    return G2Basic
