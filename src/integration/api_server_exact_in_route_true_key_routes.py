...
_BUILD_TRUE_KEY_PACKET_ENDPOINT = "/api/dex/build_exact_in_route_true_key_interpretation_packet"
_VERIFY_TRUE_KEY_PACKET_ENDPOINT = "/api/dex/verify_exact_in_route_true_key_interpretation_packet"
_TRUE_KEY_PACKET_SCHEMA = "zenodex/exact-in-route-true-key-interpretation-packet/v1"


class TrueKeyInterpretationPacket(Protocol):
    def to_dict(self) -> dict[str, object]:
        ...


def _write_build_packet_success(write_json: WriteJson, packet: TrueKeyInterpretationPacket | None) -> None:
    if packet is None:
    ...
    write_json(
        ...
        {
            "ok": True,
            "packet_schema": _TRUE_KEY_PACKET_SCHEMA,
            "verify_packet_endpoint": _VERIFY_TRUE_KEY_PACKET_ENDPOINT,
            "packet": packet.to_dict(),
        },
    )


def _handle_build_true_key_packet(
    obj: dict[str, object],
...
def _handle_verify_true_key_packet(obj: dict[str, object], write_json: WriteJson) -> None:
...
def maybe_handle_exact_in_route_true_key_route(
    *,
    path: str,
    obj: dict[str, object],
    parse_pools: ParsePools,
    ...
    if path == _BUILD_TRUE_KEY_PACKET_ENDPOINT:
        _handle_build_true_key_packet(obj, parse_pools, write_json)
        return True
    if path == _VERIFY_TRUE_KEY_PACKET_ENDPOINT:
        _handle_verify_true_key_packet(obj, write_json)
        return True
    return False
