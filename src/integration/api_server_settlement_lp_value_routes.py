...
_BUILD_LP_VALUE_CONTRACT_ENDPOINT = "/api/dex/build_settlement_lp_value_contract"
_VERIFY_LP_VALUE_CONTRACT_ENDPOINT = "/api/dex/verify_settlement_lp_value_contract"


@dataclass(frozen=True)
class LPValueRequest:
    ...
    lp_unit_values: dict[str, object]
...
def _parse_lp_value_request(obj: dict[str, object]) -> LPValueRequest:
    ...
    lp_unit_values = _required_non_empty_dict(obj.get("lp_unit_values"), "bad_lp_unit_values")
...
def _required_non_empty_dict(value: object, error: str) -> dict[str, object]:
    parsed = _required_dict(value, error)
    if not parsed:
        raise BadRequest(error)
    return parsed
...
def _parse_lp_unit_values(lp_unit_values_obj: dict[str, object]) -> dict[str, int]:
...
def _handle_build_lp_value_contract(obj: dict[str, object], write_json: WriteJson) -> None:
    try:
        req = _parse_lp_value_request(obj)
    ...
    try:
        ...
        contract = _build_contract(req, settlement, lp_unit_values)
...
def _build_contract(
    ...
    if req.price_attestation is not None:
        return _build_contract_from_attestation(req, settlement, lp_unit_values)
    if req.price_packet is not None:
        return _build_contract_from_packet(req, settlement, lp_unit_values)
    return _build_contract_from_prices(req, settlement, lp_unit_values)


def _build_contract_from_attestation(
    req: LPValueRequest,
    settlement: object,
    lp_unit_values: dict[str, int],
) -> object:
...
def _build_contract_from_packet(
    req: LPValueRequest,
    settlement: object,
    lp_unit_values: dict[str, int],
) -> object:
...
def _build_contract_from_prices(
    req: LPValueRequest,
    settlement: object,
    lp_unit_values: dict[str, int],
) -> object:
...
def _handle_verify_lp_value_contract(obj: dict[str, object], write_json: WriteJson) -> None:
    try:
        req = _parse_lp_value_request(obj)
    ...
    try:
        ...
        settlement = _parse_settlement(req.settlement)
        lp_unit_values = _parse_lp_unit_values(req.lp_unit_values)
        ok, err = _verify_contract(req, settlement, lp_unit_values, contract)
...
def _verify_contract(
    ...
    if req.price_attestation is not None:
        return _verify_contract_from_attestation(req, settlement, lp_unit_values, contract)
    if req.price_packet is not None:
        return _verify_contract_from_packet(req, settlement, lp_unit_values, contract)
    return _verify_contract_from_prices(req, settlement, lp_unit_values, contract)
...
def maybe_handle_settlement_lp_value_route(
    *,
    path: str,
    obj: dict[str, object],
    write_json: WriteJson,
) -> bool:
    if path == _BUILD_LP_VALUE_CONTRACT_ENDPOINT:
        _handle_build_lp_value_contract(obj, write_json)
        return True
    if path == _VERIFY_LP_VALUE_CONTRACT_ENDPOINT:
        _handle_verify_lp_value_contract(obj, write_json)
        return True
    return False
