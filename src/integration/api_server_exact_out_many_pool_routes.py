...
_REPLACEMENT_SHADOW_PACKET_ENDPOINT = (
...
def _handle_repaired_key_cover_interpretation_packet(
    obj: dict[str, object],
    parse_pools: ParsePools,
    write_json: WriteJson,
) -> None:
    try:
        ...
        response = _packet_build_response(
            ...
            error_fallback="many_pool_repaired_key_cover_witness_interpretation_inconsistent",
        )
        response["quote_policy"] = "repaired_key_cover_interpretation_v1"
        write_json(200, response)
    except _BadRequest as exc:
        _write_bad_request(write_json, exc)
    except Exception:
...
def _handle_replacement_shadow_packet(
    obj: dict[str, object],
    parse_pools: ParsePools,
    write_json: WriteJson,
) -> None:
...
def _handle_default_packet(
    ...
    try:
        ...
        response = _packet_build_response(
            ...
            packet_schema=EXACT_OUT_MANY_POOL_CERTIFIED_ADVISORY_PACKET_SCHEMA,
            verify_packet_endpoint="/api/dex/verify_exact_out_many_pool_default_packet",
            error_fallback="many_pool_default_packet_not_ok",
        )
        response["quote_policy"] = "certified_advisory_v1"
        if not packet.packet_ok:
...
def _handle_bounded_workaround_packet(
    obj: dict[str, object],
    parse_pools: ParsePools,
    write_json: WriteJson,
) -> None:
    try:
        ...
        # This build endpoint preserves the legacy shape: a successful builder
        # call returns ok=true, and packet_ok is checked by the verifier route.
        write_json(
            200,
            {
                "ok": True,
