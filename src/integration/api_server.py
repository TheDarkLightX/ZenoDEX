...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _maybe_handle_dex_api(
        ...
        from src.integration.api_server_pokayoke_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_pokayoke_swap_route,
        )

        if maybe_handle_pokayoke_swap_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
        ...
        from src.integration.api_server_quote_routes import maybe_handle_quote_route  # pylint: disable=import-outside-toplevel

        if maybe_handle_quote_route(
            path=path,
            obj=obj,
            server=self.server,
        ...
        from src.integration.api_server_exact_in_route_guard_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_exact_in_route_guard_route,
        )

        if maybe_handle_exact_in_route_guard_route(
            path=path,
            obj=obj,
            parse_pools=_parse_pools,
            write_json=lambda status, payload: self._write_json(
        ...
        from src.integration.api_server_exact_in_route_packet_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_exact_in_route_packet_route,
        )

        if maybe_handle_exact_in_route_packet_route(
            path=path,
            obj=obj,
            parse_pools=_parse_pools,
            write_json=lambda status, payload: self._write_json(
        ...
        from src.integration.api_server_exact_in_route_rank_projection_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_exact_in_route_rank_projection_route,
        )

        if maybe_handle_exact_in_route_rank_projection_route(
            path=path,
            obj=obj,
            parse_pools=_parse_pools,
            write_json=lambda status, payload: self._write_json(
        ...
        from src.integration.api_server_exact_in_route_true_key_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_exact_in_route_true_key_route,
        )

        if maybe_handle_exact_in_route_true_key_route(
            path=path,
            obj=obj,
            parse_pools=_parse_pools,
            write_json=lambda status, payload: self._write_json(
        ...
        from src.integration.api_server_settlement_spot_value_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_settlement_spot_value_route,
        )

        if maybe_handle_settlement_spot_value_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
        ...
        from src.integration.api_server_settlement_lp_value_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_settlement_lp_value_route,
        )

        if maybe_handle_settlement_lp_value_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
        ...
        from src.integration.api_server_settlement_value_packet_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_settlement_value_packet_route,
        )

        if maybe_handle_settlement_value_packet_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
        ...
        from src.integration.api_server_settlement_endogenous_lp_value_packet_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_settlement_endogenous_lp_value_packet_route,
        )

        if maybe_handle_settlement_endogenous_lp_value_packet_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
        ...
        from src.integration.api_server_settlement_end_to_end_certificate_routes import (  # pylint: disable=import-outside-toplevel
            maybe_handle_settlement_end_to_end_certificate_route,
        )

        if maybe_handle_settlement_end_to_end_certificate_route(
            path=path,
            obj=obj,
            write_json=lambda status, payload: self._write_json(
                status,
