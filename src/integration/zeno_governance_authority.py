...
def _normalize_backend_descriptor(raw: object, *, name: str) -> tuple[dict[str, Any] | None, str | None]:
    try:
        ...
        descriptor = KeyBackendDescriptor(
            ...
            active=obj.get("active") if type(obj.get("active")) is bool else False,
            no_raw_private_key_exposure=obj.get("no_raw_private_key_exposure")
            if type(obj.get("no_raw_private_key_exposure")) is bool
            else False,
            metadata=_require_mapping(obj.get("metadata", {}), name=f"{name}.metadata"),
        )
        if type(obj.get("active")) is not bool:
            return None, f"{name}.active_must_be_bool"
        if type(obj.get("no_raw_private_key_exposure")) is not bool:
            return None, f"{name}.no_raw_private_key_exposure_must_be_bool"
