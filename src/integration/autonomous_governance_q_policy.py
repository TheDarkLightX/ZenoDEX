    ...
    policy = {
        ...
        "q_layers": [
            ...
            },
            {
                ...
                "q_table": {
                    "*": {},
                    "2|0": {"raise_fee_10": 8, "raise_fee_10_tighten_funding_5": -8},
                    "2|1": {"raise_fee_10": 8, "raise_fee_10_tighten_funding_5": -8},
                    "2|2": {"raise_fee_10": 8, "raise_fee_10_tighten_funding_5": -8},
                    "2|3": {"raise_fee_10": 8, "raise_fee_10_tighten_funding_5": -8},
                    "3|0": {"raise_fee_10": 8, "raise_fee_10_tighten_funding_5": -8},
...
def commit_autonomous_governance_surface_q_policy_v1(
    *,
    ...
    trajectory_used_after = _advance_trajectory_used_after_step(
        ...
        trajectory_budget=receipt.get("trajectory_budget", {}),
        trajectory_used=receipt.get("trajectory_used", {}),
    )
...
def _advance_trajectory_used_after_step(
    *,
    admitted: bool,
    committed_state: Mapping[str, int],
    applied_state: Mapping[str, int],
    trajectory_budget: object,
    trajectory_used: object,
) -> dict[str, int]:
    used_before, _ = _normalize_trajectory_used(
        trajectory_used if isinstance(trajectory_used, Mapping) else None
    )
    budget, _ = _normalize_surface_int_map(
        trajectory_budget if isinstance(trajectory_budget, Mapping) else {},
        name="trajectory_budget",
    )
    out = dict(used_before)
    if not admitted or not budget:
        return {key: out[key] for key in sorted(out)}
    for name in sorted(budget):
        if name not in committed_state or name not in applied_state:
            continue
        out[name] = int(out.get(name, 0)) + abs(int(applied_state[name]) - int(committed_state[name]))
    return {key: out[key] for key in sorted(out)}
