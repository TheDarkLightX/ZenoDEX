...
from src.integration._zrpf_spot_v7_settlement_envelope_contract import (
    ...
    _SETTLEMENT_REPLAY_OBSERVATION_SEAL_V2,
    ...
    _derive_replay_material_root_v2,
...
@final
class SpotV7SettlementEnvelopeReplayAdapterV2(_NonTransferableSettlementReplayV1):
    """Replay one envelope while retaining exact bounded history inputs."""
    ...
    def __init_subclass__(cls, **_kwargs: object) -> NoReturn:
        raise TypeError("SpotV7SettlementEnvelopeReplayAdapterV2 cannot be subclassed")
    ...
    def evaluate(
        ...
        *,
        ...
        """Return the unchanged deterministic V1 data-only replay receipt."""
    ...
    def authenticate(
        ...
        *,
        ...
        """Replay exact inputs and retain their canonical bytes behind a seal."""
        ...
        return _seal_observation_v2(
            ...
            evaluation=evaluation,
            config_digest=self._config_digest,
            exact_config_document_bytes=self._config_document_bytes,
            pre_snapshot=pre_value,
        )
...
def _seal_observation_v2(
    *,
    ...
    header_hash = canonical_header_hash_v0(header)
    body_root = canonical_body_root_v0(body)
    candidate_commitment = _derive_capability_commitment(candidate)
    envelope_sha256 = _sha256(evaluation.envelope_bytes)
    config_document_sha256 = _sha256(exact_config_document_bytes)
    exact_pre_state_snapshot_bytes = canonical_json_bytes_v0(pre_snapshot)
    pre_state_snapshot_sha256 = _sha256(exact_pre_state_snapshot_bytes)
    replay_material_root = _derive_replay_material_root_v2(
        chain_id=header["chain_id"],
        height=header["height"],
        candidate_settlement_commitment=candidate_commitment,
    ...
    evidence = _observation_evidence_v2(
        candidate=candidate,
        evaluation=evaluation,
        header_hash=header_hash,
        body_root=body_root,
        ...
        config_document_sha256=config_document_sha256,
        pre_state_snapshot_sha256=pre_state_snapshot_sha256,
        replay_material_root=replay_material_root,
    )
    evidence_bytes = canonical_json_bytes_v0(evidence)
    if not evidence_bytes or len(evidence_bytes) > MAX_OBSERVATION_EVIDENCE_BYTES_V1:
        raise SpotV7SettlementEnvelopeReplayErrorV1("observation_evidence")
    ...
    return _AuthenticatedSpotV7SettlementReplayObservationV2(
        projection,
        exact_header_bytes=canonical_json_bytes_v0(header),
        exact_body_bytes=canonical_json_bytes_v0(body),
        exact_envelope_bytes=evaluation.envelope_bytes,
        ...
        seal=_SETTLEMENT_REPLAY_OBSERVATION_SEAL_V2,
    )
...
def _observation_evidence_v2(
    *,
    ...
    return {
        ...
        "status": "authenticated_exact_replay_material_retained",
        "candidate_settlement_commitment": _derive_capability_commitment(candidate),
        "header_hash": header_hash,
        "body_root": body_root,
        "envelope_sha256": _sha256(evaluation.envelope_bytes),
