...
def step(s: State, cmd: Command) -> StepResult:
    ...
    if cmd.tag == "advance_rate_epoch":
        ...
        if "new_implied_rate_bps" not in cmd.args or not (
            isinstance(cmd.args["new_implied_rate_bps"], int)
            and not isinstance(cmd.args["new_implied_rate_bps"], bool)
            and -10000 <= cmd.args["new_implied_rate_bps"] <= 10000
        ):
        ...
        if not (
            ...
            and (cmd.args["new_implied_rate_bps"] <= s.funding_cap_bps)
            and (False == s.frozen)
            and (False == s.settled_this_epoch)
            and (cmd.args["new_implied_rate_bps"] >= (0 - s.funding_cap_bps))
        ...
        if "realized_rate_bps" not in cmd.args or not (
            ...
            and -10000 <= cmd.args["realized_rate_bps"] <= 10000
        ):
            return StepResult(ok=False, error="invalid param realized_rate_bps")
        if "settle_long_payout" not in cmd.args or not (
            isinstance(cmd.args["settle_long_payout"], int)
            and not isinstance(cmd.args["settle_long_payout"], bool)
            and 0 <= cmd.args["settle_long_payout"] <= 1000000000000
        ):
            return StepResult(ok=False, error="invalid param settle_long_payout")
        if "settle_short_payout" not in cmd.args or not (
            isinstance(cmd.args["settle_short_payout"], int)
            and not isinstance(cmd.args["settle_short_payout"], bool)
            and 0 <= cmd.args["settle_short_payout"] <= 1000000000000
        ):
            return StepResult(ok=False, error="invalid param settle_short_payout")
        if "settle_protocol_fee" not in cmd.args or not (
            isinstance(cmd.args["settle_protocol_fee"], int)
            and not isinstance(cmd.args["settle_protocol_fee"], bool)
            and 0 <= cmd.args["settle_protocol_fee"] <= 1000000000000
        ):
            return StepResult(ok=False, error="invalid param settle_protocol_fee")
        if "settle_mark_price_e8" not in cmd.args or not (
            isinstance(cmd.args["settle_mark_price_e8"], int)
            and not isinstance(cmd.args["settle_mark_price_e8"], bool)
            and 1 <= cmd.args["settle_mark_price_e8"] <= 1000000000000
        ):
            return StepResult(ok=False, error="invalid param settle_mark_price_e8")
        if "settle_index_price_e8" not in cmd.args or not (
            isinstance(cmd.args["settle_index_price_e8"], int)
            and not isinstance(cmd.args["settle_index_price_e8"], bool)
            and 1 <= cmd.args["settle_index_price_e8"] <= 1000000000000
        ):
        ...
        if not (
            ...
            and (
                (
                    (cmd.args["settle_long_payout"] + cmd.args["settle_short_payout"])
                    + cmd.args["settle_protocol_fee"]
                )
                == s.premium_pool
            )
            and ((s.rate_long_exposure + s.rate_short_exposure) > 0)
            and (cmd.args["realized_rate_bps"] >= (0 - s.funding_cap_bps))
        ):
