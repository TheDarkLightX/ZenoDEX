...
IR hash: sha256:b2cf19b7e72a7392fb86d1f36c80aa515646ab542a25038c67345b7b2725cbf4
...
def step(s: State, cmd: Command) -> StepResult:
    ...
    if cmd.tag == 'advance_epoch':
        ...
        if 'auth_ok' not in cmd.args or not (isinstance(cmd.args['auth_ok'], bool)):
            return StepResult(ok=False, error='invalid param auth_ok')
        if not ((((cmd.args['protocol_fee'] + s.protocol_fee_pool) <= 1000000000000) and (cmd.args['capped_payout'] <= s.margin_pool) and (cmd.args['protocol_fee'] <= cmd.args['capped_payout']) and (False == s.settled_this_epoch) and (True == s.snapshot_taken) and cmd.args['auth_ok'])):
