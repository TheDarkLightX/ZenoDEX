...
class Command:
    """Command to execute."""
    tag: Literal('advance_epoch', 'close_long', 'close_short', 'open_long_il', 'open_short_il', 'settle_il_epoch', 'snapshot_epoch_start')
...
def check_invariants(s: State) -> tuple[bool, str | None]:
    ...
    if not (isinstance(s.coverage_ratio_bps, int) and not isinstance(s.coverage_ratio_bps, bool) and 0 <= s.coverage_ratio_bps <= 10000):
        return False, 'domain_coverage_ratio_bps'
    if not (isinstance(s.epoch, int) and not isinstance(s.epoch, bool) and 0 <= s.epoch <= 1000000000):
        return False, 'domain_epoch'
    if not (isinstance(s.long_exposure, int) and not isinstance(s.long_exposure, bool) and 0 <= s.long_exposure <= 1000000000000):
        return False, 'domain_long_exposure'
    if not (isinstance(s.margin_pool, int) and not isinstance(s.margin_pool, bool) and 0 <= s.margin_pool <= 1000000000000):
        return False, 'domain_margin_pool'
    if not (isinstance(s.max_leverage_bps, int) and not isinstance(s.max_leverage_bps, bool) and 1 <= s.max_leverage_bps <= 50000):
        return False, 'domain_max_leverage_bps'
    if not (isinstance(s.pool_snapshot_reserve_x, int) and not isinstance(s.pool_snapshot_reserve_x, bool) and 0 <= s.pool_snapshot_reserve_x <= 1000000000000):
        return False, 'domain_pool_snapshot_reserve_x'
    if not (isinstance(s.pool_snapshot_reserve_y, int) and not isinstance(s.pool_snapshot_reserve_y, bool) and 0 <= s.pool_snapshot_reserve_y <= 1000000000000):
    ...
    if not (isinstance(s.realized_il_bps, int) and not isinstance(s.realized_il_bps, bool) and 0 <= s.realized_il_bps <= 10000):
        return False, 'domain_realized_il_bps'
    if not (isinstance(s.settled_this_epoch, bool)):
        return False, 'domain_settled_this_epoch'
    if not (isinstance(s.short_exposure, int) and not isinstance(s.short_exposure, bool) and 0 <= s.short_exposure <= 1000000000000):
    ...
    if not (((s.margin_pool >= 0) and (s.premium_pool >= 0) and (s.protocol_fee_pool >= 0))):
...
def step(s: State, cmd: Command) -> StepResult:
    ...
    if cmd.tag == 'advance_epoch':
        ...
        if not (((cmd.args['amount'] <= s.long_exposure) and ((False == s.snapshot_taken) or (True == s.settled_this_epoch)) and cmd.args['auth_ok'])):
        ...
        if 'auth_ok' not in cmd.args or not (isinstance(cmd.args['auth_ok'], bool)):
            return StepResult(ok=False, error='invalid param auth_ok')
        if not (((cmd.args['amount'] <= s.short_exposure) and ((False == s.snapshot_taken) or (True == s.settled_this_epoch)) and cmd.args['auth_ok'])):
        ...
        if 'amount' not in cmd.args or not (isinstance(cmd.args['amount'], int) and not isinstance(cmd.args['amount'], bool) and 1 <= cmd.args['amount'] <= 1000000000000):
            return StepResult(ok=False, error='invalid param amount')
        if 'premium_amount' not in cmd.args or not (isinstance(cmd.args['premium_amount'], int) and not isinstance(cmd.args['premium_amount'], bool) and 1 <= cmd.args['premium_amount'] <= 100000000):
            return StepResult(ok=False, error='invalid param premium_amount')
        if 'auth_ok' not in cmd.args or not (isinstance(cmd.args['auth_ok'], bool)):
            return StepResult(ok=False, error='invalid param auth_ok')
        if not ((((10000 * (cmd.args['amount'] + s.long_exposure)) <= (s.max_leverage_bps * s.short_exposure)) and ((cmd.args['amount'] + s.long_exposure) <= 1000000000000) and ((cmd.args['premium_amount'] + s.premium_pool) <= 1000000000000) and (False == s.snapshot_taken) and (s.short_exposure > 0) and cmd.args['auth_ok'])):
        ...
        if 'auth_ok' not in cmd.args or not (isinstance(cmd.args['auth_ok'], bool)):
            return StepResult(ok=False, error='invalid param auth_ok')
        if not ((((cmd.args['amount'] + s.margin_pool) <= 1000000000000) and ((cmd.args['amount'] + s.short_exposure) <= 1000000000000) and (False == s.snapshot_taken) and cmd.args['auth_ok'])):
        ...
        if 'il_bps' not in cmd.args or not (isinstance(cmd.args['il_bps'], int) and not isinstance(cmd.args['il_bps'], bool) and 0 <= cmd.args['il_bps'] <= 10000):
            return StepResult(ok=False, error='invalid param il_bps')
        if 'capped_payout' not in cmd.args or not (isinstance(cmd.args['capped_payout'], int) and not isinstance(cmd.args['capped_payout'], bool) and 0 <= cmd.args['capped_payout'] <= 1000000000000):
        ...
        if 'auth_ok' not in cmd.args or not (isinstance(cmd.args['auth_ok'], bool)):
            return StepResult(ok=False, error='invalid param auth_ok')
        if not ((((cmd.args['protocol_fee'] + s.protocol_fee_pool) <= 1000000000000) and (cmd.args['capped_payout'] <= (s.margin_pool + s.premium_pool)) and (cmd.args['protocol_fee'] <= cmd.args['capped_payout']) and (False == s.settled_this_epoch) and (True == s.snapshot_taken) and cmd.args['auth_ok'])):
        ...
        new_state = State(
            ...
            pool_snapshot_reserve_x=s.pool_snapshot_reserve_x,
            pool_snapshot_reserve_y=s.pool_snapshot_reserve_y,
            premium_pool=(s.premium_pool - (cmd.args['capped_payout'] - min(cmd.args['capped_payout'], s.margin_pool))),
            protocol_fee_bps=s.protocol_fee_bps,
            protocol_fee_pool=(cmd.args['protocol_fee'] + s.protocol_fee_pool),
            realized_il_bps=cmd.args['il_bps'],
            settled_this_epoch=True,
            short_exposure=(s.margin_pool - min(cmd.args['capped_payout'], s.margin_pool)),
            snapshot_taken=s.snapshot_taken,
        )
        effects = {
            'event': 'EpochSettled',
        ...
        if 'reserve_x' not in cmd.args or not (isinstance(cmd.args['reserve_x'], int) and not isinstance(cmd.args['reserve_x'], bool) and 1 <= cmd.args['reserve_x'] <= 1000000000000):
            return StepResult(ok=False, error='invalid param reserve_x')
        if 'reserve_y' not in cmd.args or not (isinstance(cmd.args['reserve_y'], int) and not isinstance(cmd.args['reserve_y'], bool) and 1 <= cmd.args['reserve_y'] <= 1000000000000):
