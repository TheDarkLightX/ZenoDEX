...
theorem cappedBonusPerWinner_le_raw
    (usage bonusRateBps capPerWinner : Nat) :
    cappedBonusPerWinner usage bonusRateBps capPerWinner <= rawBonusPerWinner usage bonusRateBps := by
  unfold cappedBonusPerWinner
  exact min_le_left _ _

theorem cappedBonusPerWinner_le_cap
    (usage bonusRateBps capPerWinner : Nat) :
    cappedBonusPerWinner usage bonusRateBps capPerWinner <= capPerWinner := by
  unfold cappedBonusPerWinner
...
theorem paidBonus_zero_when_no_expected_winners
    (winProbBps usage bonusRateBps capPerWinner budget honestDemand : Nat) :
    paidBonus 0 winProbBps usage bonusRateBps capPerWinner budget honestDemand = 0 := by
  unfold paidBonus expectedWinnersFloor
  simp

theorem realizedBonus_zero_when_ev_zero
    (paid : Nat) :
    realizedBonus paid 0 = 0 := by
  unfold realizedBonus
  simp
