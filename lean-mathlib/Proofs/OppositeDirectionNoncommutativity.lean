...
| # | Name | Statement |
|---|------|-----------|
| 1 | `second_mover_advantage_Y` | outY ≥ when done after YtoX |
| 2 | `second_mover_advantage_X` | outX ≥ when done after XtoY |
| 3 | `second_mover_strict_Y` | outY strictly > under denominator-width gap |
| 4 | `second_mover_strict_X` | outX strictly > under denominator-width gap |
| 5 | `swapXtoY_K_exact` | K = K₀ + (y·a) mod (x+a) |
| 6 | `swapYtoX_K_exact` | K = K₀ + (x·b) mod (y+b) |
| 7 | `path1_K_gap_exact` | Two-swap K decomposes as sum of remainders |
| 8 | `K_gap_path_difference` | K-difference = cross-remainder difference (ℕ) |
| 9 | `K_gap_path_difference_Z` | Signed ℤ K-gap: ↑K₁-↑K₂ = ↑r₁-↑r₂ |
| 10 | `first_swap_K_dominance_iff` | K-ordering iff remainder dominance (single swap) |
| 11 | `path_rx_strict_order` | Reserve ordering from strict second-mover advantage |
| 12 | `generic_path_noncommutativity` | ∀-quantified path noncommutativity |
| 13 | `path_K_ordering_iff` | Complete iff: K(path₁)≥K(path₂) ⟺ rem-sum ordering |
| 14 | `path_K_noncommutative_iff` | Complete iff: K(path₁)≠K(path₂) ⟺ rem-sums differ |
| 15 | `path2_K_gap_exact` | Two-swap K composition (YtoX first) |
-/
...
/-! ## Part 7: Canonicalization Safety Rule

The results above justify a conservative reorder policy:
opposite-direction same-pool swaps must not be assumed to commute.
