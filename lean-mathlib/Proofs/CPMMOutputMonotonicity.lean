...
| # | Name | Kind | Statement |
|---|------|------|-----------|
| 1 | `swapOut_mono_y` | theorem | y₁ ≤ y₂ → out(x,y₁,a) ≤ out(x,y₂,a) (Nat.div_le_div_right) |
| 2 | `swapOut_anti_x` | theorem | x₁ ≤ x₂ → out(x₂,y,a) ≤ out(x₁,y,a) (Nat.div_le_div_left) |
| 3 | `swapOut_strict_y` | theorem | (y₂−y₁)·a ≥ x+a → strict increase (nat_div_lt_of_add_le) |
| 4 | `swapOut_joint_mono` | theorem | 2-variable composition of mono_y + anti_x |
| 5 | `swapOut_shift_exact` | theorem | shift = δ·a/(x+a) + carry, carry ∈ {0,1} (tight bounds) |
| 6 | `swapOut_contraction` | theorem | 1-Lipschitz: shift ≤ δ (no rounding slack, exact division) |
| 7 | `swapOut_contraction_tight` | theorem | Tightness: equality at x=0 proves bound is best possible |
| 8 | `swapOut_le_input` | theorem | y ≤ x+a → out ≤ a (balanced-pool no-free-tokens) |
| 9 | `swapOut_approx_additive` | theorem | 1-approx AddMonoidHom: f(y₁+y₂) = f(y₁)+f(y₂)+carry |
| 10 | `swapOut_shift_carry_formula` | theorem | Carry = remainder-overflow indicator (exact, not ≤) |
| 11 | `swapOut_route_monotone` | theorem | Route mono: a₁≤a₂ → pipeline(a₁)≤pipeline(a₂) via amount |
| 12 | `swapOut_diminishing_returns` | theorem | Price impact: depleted pool gives less output |
| 13 | `swapOut_zero_iff` | theorem | Complete zero characterization: out=0 ↔ y·a < x+a |
| 14 | `swapOut_sublinear` | theorem | Spot price bound: out ≤ y·a/x (sublinearity) |
| 15 | `fee_reduces_output` | theorem | Fee-reduced input → less output |
| — | `swapOut_monotone_y` | Mathlib wrapper | Mathlib `Monotone` instance (wraps mono_y) |
| — | `swapOut_antitone_x` | Mathlib wrapper | Mathlib `Antitone` instance (wraps anti_x) |
| — | `multiHop_monotone` | Mathlib wrapper | `Monotone.comp` — reserve-parameter pipeline |
