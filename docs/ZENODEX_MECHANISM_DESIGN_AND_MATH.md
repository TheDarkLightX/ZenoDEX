...
| Add a 13th dust intent to flip `optimal_ab_bounded → limit_price` | `_MAX_SWAP_ORDERING_BRUTE_FORCE_N` | **[NUMERICAL]** one rejected dust intent across the bound rejects the victim and drops the global (A,B) in the witness (O-SS-05) |
...
| Liquidate A to push the mark price into B's liquidation | per-account isolation (real kernel) + explicit modeled price impact | **[NUMERICAL]** bounded cascade model gives exact K=0 threshold and strict-progress bound under the stated book-depth assumption (O-PT-04) |
...
| Adversary move | Rule engaged | Status |
|---|---|---|
| Fastest prover always wins → others exit (monopoly) | first-valid-wins | **[NUMERICAL]** deterministic speed-ranking model supports participation collapse (O-VM-01) |
| Stop proving when `reward(epoch) < cost` with pool non-empty | decay schedule | **[NUMERICAL]** halving cliff leaves stranded pool balance (O-VM-02) |
| Choose `miner_id`/route shape to win improvement ties | `_route_tiebreak_key` | **[NUMERICAL]** implemented key makes equal-improvement ties selectable (O-VM-03) |
| Split improvement δ across rounds to dodge `max_reward` and double `base_reward` | per-round cap formula | **[NUMERICAL]** concrete split beats one-shot under cap (O-VM-04) |
| Flood submissions up to per-block caps | submission fee / rate limits | **[NUMERICAL]** exact break-even fee floor derived and swept (O-VM-05) |
| Drain the pool past conservation | pool accounting | **[NUMERICAL]** admissible-award no-op model conserves pool value (O-VM-06) |
...
| O-SS-05 | cardinality cliff | adding one intent across `_MAX_SWAP_ORDERING_BRUTE_FORCE_N` can strictly lower a victim's fill and the global (A,B) | settlement continuity in batch size | H-MD-SS-005, H-MD-SS-006 | bounded pytest evidence — [numeric: one dust intent across a reduced bound (12 monkeypatched to 3 on the live `compute_settlement` path) flips the victim fill 47619 to REJECT and the global (A,B) from (50000, 619) to (10000, 100); at the untouched constant 12, n=13 returns exactly the `limit_price` order and rejects the victim while n=3 brute force reorders and fills it; the literal 12-intent brute-force side is not replayable (12! = 479001600 orderings), so the production 12-to-13 pair is bound branch-by-branch instead of as one instance] |
...
| O-PT-04 | price-mediated cascade bound | cascade depth K is bounded by strict progress in the tested model; K = 0 iff every non-seed account's post-PnL collateral at the seed-impacted settle price remains at least maintenance | unbounded cross-account contagion | H-MD-PT-004 | bounded pytest evidence under explicit modeled price-impact coupling — [numeric: real per-account kernel transitions, K=0 threshold at maint(sp1)+loss(sp1), 4-account chain reaches K=3, K weakly decreases over swept book depth; no production multi-account book coupling claimed] |
...
| O-VM-04 | improvement withholding | `2·min(M, b + ⌊bps·δ/2/10⁴⌋) > min(M, b + ⌊bps·δ/10⁴⌋)` for concrete (b, bps, M, δ) ⇒ splitting across rounds dominates | "submit-everything-now is optimal" | H-MD-VM-004 | bounded pytest evidence — [numeric: b=10, bps=2500, M=25, δ=80 gives 40 split vs 25 one-shot] |
| O-VM-05 | sybil fee floor | flooding to per-block caps is unprofitable iff `fee ≥ ceil(reward/max_slots)` | underpriced submission spam | H-MD-VM-005 | bounded pytest evidence — [numeric: rewards 1..128, caps 1..16 satisfy exact threshold] |
| O-VM-06 | pool conservation binding | `total_paid + pool = initial` holds across admissible awards, with rejected and insufficient-budget claims treated as no-ops | silent payout leak | H-MD-VM-006 | bounded pytest evidence — [numeric: mixed valid/invalid sequence conserves; budget rejection is no-op in the model] |
