...
Companion to `docs/AUTOGOVNEXT_GAME_THEORY_AND_MECHANISM_DESIGN.md`, which
treats the autonomous-governance mechanism. This document extends the same
treatment to ZenoDEX's *economic* mechanisms: the spot batch-clearing
auction, the sealed-bid auction family, perpetuals epoch timing and keeper
incentives, and the verification markets (proof mining and host
compensation).
...
>
> Every backticked path in this document is checked mechanically by
...
> The working tree is dirty; verify citations against the commit being
> reviewed.
...
- **Autonomous governance** — owned by the AutoGovNEXT stream; its
  obligations O1–O8 live in
  `docs/AUTOGOVNEXT_GAME_THEORY_AND_MECHANISM_DESIGN.md`.
- **zUSD buyback execution (Q-3/Q-4/Q-9)** — staged for review in
  `experiments/zusd_hybrid_economics_v1/math_notes/experimental_fable_20260609/`;
  another lane, not raced here.
- **Settled prior art** — re-derivation of H-GT-001..020
  (`experiments/research_program_v2/HYPOTHESIS_REGISTRY.md`), the
  perp-incentives principles (docs/deep_insights.md @perp-incentives), and
  zUSD economics I-1..I-18
  (`experiments/zusd_hybrid_economics_v1/math_notes/INSIGHTS.md`). New
  hypotheses must cite the prior result they extend.
...
- Batch ordering *quality*: `lean-mathlib/Proofs/BatchOptimality.lean`,
  `lean-mathlib/Proofs/BatchGreedyOptimality.lean`,
  `lean-mathlib/Proofs/BatchGreedyApproximation.lean`,
  `lean-mathlib/Proofs/BatchUniformClearing.lean` — these bound how good
...
- Bounty payout safety:
  `lean-mathlib/Proofs/BountyAuctionMechanisms.lean` (eligibility,
  payout ≤ cap, budget bounds, winner-key dominance). The *participation
  equilibrium* is open.
...
| Choose `miner_id`/route shape to win improvement ties | `_route_tiebreak_key` | **[open]** (O-VM-03) |
...
| O-VM-03 | tie-break selectability | improvement ties resolve by `_route_tiebreak_key` incl. submitter-chosen `miner_id` ⇒ tie wins are costlessly selectable | "ties are rare/neutral" | H-MD-VM-003 | pytest — [open] |
