...
| Julia math lane | `tools/zeno_oracle_math_witness_sweep.jl` checks bounded witnesses for median deviation, zero-scale/equal-value sanity, bounded median-deviation side-obligation decomposition, source cartel, dispute griefing, reward conservation/caps, live-economics escrow floor and timelock receipts, settlement-execution total matching/drift rejection, split-brain, epoch-lag symmetry, O5 independence-witness cases, and O3 action-binding DAG/runtime/sync-window cases. | Bounded witness |
