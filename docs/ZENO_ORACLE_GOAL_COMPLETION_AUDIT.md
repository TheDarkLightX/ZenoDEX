...
| O3 receipt flow | `tools/zeno_oracle_o3_receipt_flow_replay.py` replays the focused local path from feed registry through reporter lifecycle, signed report, admission, admitted median3, accepted read, action adapter, and terminal DAG replay with `8/8` accepted stages. Devnet service tools and `bash scripts/check_zeno_oracle_devnet_alpha.sh` provide broader shell coverage. `tools/check_zeno_oracle_production_network_config.py` adds a fail-closed production-candidate network config gate, including typed isolated perps settlement authorization, protected-swap authorization, guarded-quote authorization, zUSD authorization, trigger authorization, critical-settlement authorization, and a local receipt bundle for reporter-registry deployment, feed-governance deployment, feed-governance approval/execution, signed release artifacts, signed-release transparency-log root binding, runtime-control attestation, and explicit receipt-dependency links from registry deployment to governance deployment to approval/execution to signed release to transparency log to runtime controls. Live chain receipt verification and public soak remain open. | Devnet complete plus config gate |
...
| Reporter economics | `tools/zenodex_oracle_reporter_economics_replay.py` and tests cover bonds, rewards, disputes, slashing, withdrawals, fee splits, and budget rejection. `tools/zenodex_oracle_reporter_token_settlement_replay.py` binds the accepted economics ledger to an approved local policy and explicit no-mint transfer ledger. `tools/check_zeno_oracle_live_economics_policy.py` binds the replay to a production-candidate token, escrow, governance receipt, fee-split, dispute-bond, slash-cap, and withdrawal-delay policy. The same checker now verifies a local receipt bundle for governance approval, governance execution, escrow funding against the replay-derived escrow floor, settlement execution totals against the replay, and explicit receipt-dependency links from approval to execution to funding to settlement. Live chain receipt replay, on-chain funded escrow verification, and on-chain settlement execution remain open. | Replay plus policy gate |
| Disaster corpus | `tools/zenodex_oracle_devnet_disaster_harness.py` reports 17 selected states unreachable; `tools/zeno_oracle_disaster_class_corpus.py` reports 15 named families closed, including settlement-execution total drift, O5 independence-spoofing, proof-timeout fail-closed behavior, manipulable internal spot overvaluation, governance parameter drift, stale quote free-option value, recursive collateral dependency, funding deadband, and pre-slash withdrawal escape witnesses. `tools/check_zeno_oracle_disaster_frontier.py` tracks 35 production-candidate disaster families: 30 closed by public/devnet evidence and 5 left as explicit blocker/backlog families. `tools/check_zeno_oracle_perps_snapshot_gate.py` adds bounded replay evidence for usable perps oracle snapshots, including stale pre-drift action-ID rejection after snapshot price drift and clearinghouse 2p/3p action binding; `ZenoOracleMathWitness.lean` adds a restricted usable-snapshot obligation theorem for the same frontier family. `tools/check_zeno_oracle_cross_domain_finality_gate.py` adds local receipt-bundle replay for source finality checkpoint and target adapter acceptance binding, including source finality receipt block chronology against the finalized source block. `tools/check_zeno_oracle_reporter_soak_gate.py` adds local reporter-soak observation replay for operator-diversity, source-diversity, report-volume, and signed-report-root uniqueness thresholds. `tools/check_zeno_oracle_compositional_disaster_regressions.py` records sanitized private-campaign summaries and accepts seven public branch-local regressions with no deferred projection among the selected private witnesses. | First shell plus frontier gate |
...
5. General formal math: continue lifting current Julia/Lean witnesses into
   ...
   has restricted Lean theorems and 45 Julia witnesses, but full production
