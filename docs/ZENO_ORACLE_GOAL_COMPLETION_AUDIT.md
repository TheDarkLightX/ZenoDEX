...
4. Broader disaster search: close the five explicit frontier blockers currently
   ...
   receipts and public soak evidence remain required. The compositional private
   campaigns now have a public regression projection, with the confidential
   live-admission projection still requiring source-surface migration before
   claim promotion.
...
7. ZenoProof productionization: close the production governance gate blockers:
   ...
   drill, public proof-network soak, and live proof-mining token settlement.
