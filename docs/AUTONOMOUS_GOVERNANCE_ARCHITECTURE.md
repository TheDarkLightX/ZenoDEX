...
| The **autonomous proposers** (PID controller, frozen Q-learning table) | **Production proposer step built** — `src/integration/autonomous_governance_live_proposer.py` turns one observation into at most one admitted update: it byte-loads the frozen policy artifact, pins it to an expected content hash (recomputed *and* embedded hash must both match), reads the committed surface and every carry-in from the session-store head pin (never from the caller), evaluates through the exact gates inside the trajectory runner, and admits only through the live guard. Inside the proposer, every refusal returns a no-op receipt with the committed state, and refused steps do not grow the store. `src/tau_specs/governance/gov_proposers.py` (deterministic PI + frozen Q-table) remains the reference implementation for proposer semantics; production proposals are policy artifacts in the frozen Q schema (the factory's PID-shaped baseline covers the controller form). Tests: `tests/integration/test_autonomous_governance_live_proposer.py`. |
...
| The **live autonomous loop** (the reference loop driving *committed on-chain* parameters from *attested* state) | **Wired into the deployed node, flag-gated and off by default.** `tools/zeno_ledger_node.py` exposes `GET /api/governance/surface` (committed store head) and `POST /api/governance/propose-step` (bearer write-auth always required — enabling governance without a configured token refuses to start, so the unauthenticated-testnet-writes loophole cannot reach it), enabled only when `run --serve` is started with `--autonomous-governance-store`, `--autonomous-governance-policy`, and `--autonomous-governance-expected-policy-hash` together (partial configuration refuses to start). Malformed request bodies and policy-load failures refuse at the HTTP layer with plain fail-closed errors before the proposer runs; once the proposer runs, refusals are no-op receipts. The propose endpoint re-pins the policy file on every call and routes exclusively through the production proposer step and `autonomous_governance_live_apply.py` — the guard is the only store-head writer. Tests: `tests/integration/test_zeno_ledger_node_autonomous_governance.py`. Still open **WS5** work: the governance surface lives in the node-local session store, not in ledger consensus state (no state-root binding, no global ordering/distribution); the join-network config path does not carry the flags; and observation authenticity stays at the L2 oracle posture — the gates bound freshness/divergence/volatility and the step/budget envelope, they do not authenticate the feed. |

**The autonomous proposer path is wired but disabled by default.** A node serves the
governance endpoints only when an operator explicitly supplies the session store, the
frozen policy artifact, and its pinned hash; with the flags absent (the default), no
autonomous proposer runs and the endpoints refuse. The admitted surface state is
node-local session-store state — it does not change ledger consensus parameters, and
default governance authority is unchanged. The PID/Q-table sections below describe the
proposer/gate architecture; the production path evaluates frozen policy artifacts
through the exact gates.
