...
transition bit. When a trajectory budget is active, the step receipt also hashes
`trajectory_used_after`, so budget advancement is part of the auditable
transition output.
...
- if a trajectory budget is supplied, the proposed absolute movement remains
  within that budget and the post-step used amount is receipt-bound.

The receipt explicitly does not claim settlement authority, immutable rule
changes, oracle truth, or online Q-table training.

## Measured Policy Quality

The governance-surface sample policy is checked against the factory's bounded
240-scenario replay grid. The metric compares the selected action with the
exhaustive verifier-labeled frontier for each scenario.

| Version | Opportunity misses | Frontier regret total | Utility completion |
|---|---:|---:|---:|
