...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 444 |
| Functions scanned | 5,828 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 214 |
...
14. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
15. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
16. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
17. `_maybe_handle_dex_api` remains the dispatcher and response writer.
18. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
19. The baseline records the reduced handler complexity and line count.

Next, move `build_settlement_endogenous_lp_value_packet` and
`verify_settlement_endogenous_lp_value_packet` into a route helper with tests for
malformed settlement, missing price input, bad pool snapshots, price input
precedence, success response shape, and packet verification errors.
