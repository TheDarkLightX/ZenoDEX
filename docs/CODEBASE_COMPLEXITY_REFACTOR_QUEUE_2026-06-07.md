...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 442 |
| Functions scanned | 5,781 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 342 |
...
12. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
13. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
14. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
15. `_maybe_handle_dex_api` remains the dispatcher and response writer.
16. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
17. The baseline records the reduced handler complexity and line count.

Next, move `build_settlement_lp_value_contract` and
`verify_settlement_lp_value_contract` into a route helper with tests for
malformed settlement, missing price input, bad LP unit value containers, success
response shape, and contract verification errors.
