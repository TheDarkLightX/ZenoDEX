...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 435 |
| Functions scanned | 5,740 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 469 |
| Maximum function length | 2,098 lines |
...
5. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
6. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
7. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
8. `_maybe_handle_dex_api` remains the dispatcher and response writer.
9. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
10. The baseline records the reduced handler complexity and line count.

Next, move `guard_exact_in_route_canonicality` into a route helper with tests for
bridge rejection, malformed fields, pool-parse failure, and contract response
shape.
