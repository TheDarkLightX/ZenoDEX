...
| Functions scanned | 5,671 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 807 |
| Maximum function length | 4,394 lines |
...
| ---: | --- | ---: | --- | --- | --- |
| 1 | `src/integration/api_server.py::_Handler._maybe_handle_dex_api` | 807 complexity, 4,394 lines | D | One method mixes routing, auth, JSON parsing, DTO coercion, service calls, and response shaping for many unrelated API families. It is the highest injection and regression surface in the repo. | Continue the route-table extraction. The read-only routes live in `api_server_dex_readonly_routes.py`; exact-out many-pool contract builders, repaired-selected-domain quotes, repaired-advisory quotes, repaired-full-domain certified quotes, bounded-advisory quotes, the default certified-advisory quote, and the adaptive liveness quote live in `api_server_exact_out_many_pool_routes.py`; the next slice is exact-out many-pool certified-advisory quote and packet routes. |
...
Next, move exact-out many-pool certified-advisory and packet
