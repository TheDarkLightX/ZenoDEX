...
| Maximum complexity | 895 |
| Maximum function length | 5,077 lines |
...
| ---: | --- | ---: | --- | --- | --- |
| 1 | `src/integration/api_server.py::_Handler._maybe_handle_dex_api` | 895 complexity, 5,077 lines | D | One method mixes routing, auth, JSON parsing, DTO coercion, service calls, and response shaping for many unrelated API families. It is the highest injection and regression surface in the repo. | Continue the route-table extraction. The read-only routes live in `api_server_dex_readonly_routes.py`; exact-out many-pool contract builders live in `api_server_exact_out_many_pool_routes.py`; the next slice is exact-out many-pool quote/packet routes. |
...
2. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
   exact-out many-pool contract-builder endpoints.
...
Next, move exact-out many-pool quote and packet endpoints one family at a time
