...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 445 |
| Functions scanned | 5,847 |
...
15. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
16. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
17. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
18. `_maybe_handle_dex_api` remains the dispatcher and response writer.
19. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
20. The baseline records the reduced handler complexity and line count.

Next, move `build_settlement_feature_extension_packet` and
`verify_settlement_feature_extension_packet` into a route helper with tests for
malformed feature-extension inputs, bad packet payloads, success response shape,
and packet verification errors.
