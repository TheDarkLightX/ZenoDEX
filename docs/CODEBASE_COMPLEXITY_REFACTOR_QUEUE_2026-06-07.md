...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 448 |
| Functions scanned | 5,888 |
...
22. `_maybe_handle_dex_api` remains the dispatcher and response writer.
23. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
24. The baseline records the reduced handler complexity and line count.

Next, move `pokayoke_swap_suggest`, `pokayoke_swap_suggest_heavy`, and
`proof_mining_status` into route helpers with tests for malformed numeric fields,
option filtering, target-action filtering, proof-mining cheap rejection order,
success response shape, and generic error envelopes.
