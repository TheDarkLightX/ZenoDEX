...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 443 |
| Functions scanned | 5,807 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 270 |
...
13. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
14. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
15. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
16. `_maybe_handle_dex_api` remains the dispatcher and response writer.
17. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
18. The baseline records the reduced handler complexity and line count.

Next, move `build_settlement_value_packet` and
`verify_settlement_value_packet` into a route helper with tests for malformed
settlement, missing price input, bad LP unit value contents, success response
