...
| Metric | Value |
| --- | ---: |
| Python source files scanned | 441 |
| Functions scanned | 5,759 |
| Functions over complexity 5 | 1,605 |
| Functions over 60 lines | 435 |
| Maximum complexity | 400 |
...
11. `src/integration/api_server_exact_out_certificate_routes.py` handles
   `build_exact_out_route_certificate` and `verify_exact_out_route_certificate`.
12. `src/integration/api_server_exact_out_audit_routes.py` handles
   `audit_exact_out_two_pool_canonicality` and
   `audit_exact_out_many_pool_canonicality`.
13. `src/integration/api_server_exact_out_many_pool_routes.py` handles the
...
14. `_maybe_handle_dex_api` remains the dispatcher and response writer.
15. Focused route tests cover success, error mapping, unhandled-path behavior,
   bad integer fields, and pool-parse error precedence.
16. The baseline records the reduced handler complexity and line count.

Next, move `build_settlement_spot_value_contract` and
`verify_settlement_spot_value_contract` into a route helper with tests for
malformed settlement, missing price input, bad price containers, success
