...
Artifact:
[ZENO_ENERGY_SYNTHETIC_CANDIDATE_COVERAGE.md](./ZENO_ENERGY_SYNTHETIC_CANDIDATE_COVERAGE.md)

Static JSON:
`data/upba_energy/upba_v2_energy_synthetic_candidate_coverage_seed20260540.json`

Command:

```bash
python3 tools/audit_upba_energy_synthetic_coverage.py \
  --batches 100 \
  --candidates-per-batch 32 \
  --seed 20260540 \
  --output-json data/upba_energy/upba_v2_energy_synthetic_candidate_coverage_seed20260540.json \
  --output-markdown docs/ZENO_ENERGY_SYNTHETIC_CANDIDATE_COVERAGE.md
```

Observed aggregate:

| batches | candidates | winner batch rate | hard-negative rate | invalid | valid | duplicate hash batches |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 100 | 3,196 | 0.9700 | 0.1248 | 2,749 | 447 | 0 |

Positive knowledge:

```text
The seeded synthetic generator covers all required mutation classes and all
hard-negative classes in this audit, emits only synthetic rows, records no
live-secret key hits, and preserves the expected 96/51/147 feature dimensions.
```

Residual limit:

```text
This is a generator coverage audit. It supports bounded synthetic training and
benchmark replay, but it is not evidence that production candidate
distributions match the synthetic distribution.
```

Research consequence: use this audit as the minimum evidence gate for future
synthetic dataset refreshes. A production-shadow dataset should rerun the same
coverage checks after user identifiers and secrets are removed.
...
Synthetic candidate coverage refs:
...
Research evidence replay v7 refs:
