...
- bounded price-grid enumeration gives a concrete complete audit set for every
  candidate generated by a configured integer price grid;
- an upper-bound certificate over that complete bounded price grid gives global
  weak optimality over the bounded grid-generated candidate family;
- the v2 partial-fill theorem extends the bounded-grid bridge to a finite list
  of admitted bounded partial-fill plans;
...
feasible candidate is in the audited set. For UPBA v2, that means every
canonical bounded-grid price and every admitted bounded partial-fill plan.

The bounded grid theorem supplies that completeness argument when the deployed
solver/auditor enumerates every integer price pair up to configured
`maxNum/maxDen` bounds and scores each price deterministically. The resulting
...
The first admission verifier is `src/core/uniform_batch_admission.py`.
It implements this deterministic local policy:

```text
zenodex/upba_admission_v1/canonical_intent_id_prefix
```

Given a known homogeneous eligible set for one supported UPBA pool, the verifier
sorts eligible `SWAP_EXACT_IN` or `SWAP_EXACT_OUT` intents by canonical
`intent_id` and admits the prefix up to `max_admitted`. It rejects mixed-kind
and mixed-asset-pair sets. The certificate binds:

- the eligible intent-set hash;
- the admitted intent-set hash used by the UPBA clearing certificate;
- the overflow intent-set hash;
- counts for eligible, admitted, and overflow members;
- the pool id and admission cap.

This closes the local “solver chose a convenient subset” gap for a supplied
eligible set. It does not establish that the eligible set itself was fair,
complete, censorship-resistant, or Sybil-resistant. That evidence must come from
the batch-builder, ledger, or mempool admission layer that defines the eligible
set boundary.

## Exact-Out Runtime Surface

The first exact-out UPBA runtime verifier is documented in
`docs/UPBA_V3_EXACT_OUT_CERTIFICATE.md`. It adds the fixed policy:

```text
zenodex/upba_v3/fixed_admission_full_fill_cpmm_exact_out
```

For each admitted exact-out intent, the certificate must fill exactly
`intent.amount_out`, stay within `intent.max_amount_in`, satisfy the uniform
price inequality, and use the minimal gross input at that price after deterministic
fee rounding. This is a runtime settlement-verifier claim. The bounded-grid
global optimality bridge exists as a conditional model theorem. The runtime
helper `build_uniform_batch_exact_out_grid_audit_candidates_v1` enumerates the
accepted v3 exact-out candidates over a reduced integer price grid and feeds the
generic finite audit-set optimality checker.
...
```bash
pytest -q tests/core/test_uniform_batch_admission.py
...
The subsequent local proof pass added bounded price-grid completeness:
...
Aristotle project `b13d05d2-05d5-49b2-9feb-f29427562537` was submitted as an
independent proof-search lane for this theorem family. The local repository
proof is the accepted artifact once it passes the local Lean checker and trust
scan.

Second UPBA v2 Aristotle lane:

```text
5ab11e96-962e-46a7-a342-17e65b796141
```

Purpose: independent proof-search for the bounded partial-fill grid bridge:
...
Result: build succeeds and the trust scan is clean. The receipt is
`lean-mathlib/proof_receipts/upba_v2_aristotle_optimality.md`.
...
- volume-maximizing or surplus-maximizing clearing across all unbounded prices;
- unbounded-rational price-grid completeness;
- economic sufficiency of a particular grid resolution;
- fair order inclusion;
- fair eligible-set construction;
- censorship resistance;
- oracle-safe mark price construction;
- exact-out economic sufficiency of a particular grid resolution;
- unbounded exact-out rational-price completeness;
- multi-hop support;
- LP add/remove support inside the uniform batch;
- full MEV elimination.

The v2 claim is narrower: for the scoped single-pool exact-in surface, a
certificate can express partial fills and explicit unfilled members while
remaining bound to deterministic aggregate-flow arithmetic and aggregate CPMM
invariant checks. The v3 exact-out runtime surface is separate. It is currently
limited to full fills under a fixed admitted set, with a bounded reduced
integer-price grid enumerator available for audited candidate construction.
