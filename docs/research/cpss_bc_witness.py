...
def main() -> int:
    ...
    print("Suite 1: Fixed intent order (moderate baseline, insufficient for proof)")
        ...
        def search(
            ...
            for b in range(0, d + 1):
                ...
                for i, alloc in enumerate([b, d - b]):
                    ...
                    r[i] = [x + alloc, y - out, fee]
                    t += out
                search(k + 1, r, t, ordered_intents)

        search(0, [list(pools[0]), list(pools[1])], 0, ordered)
    return best
...
# ---------------------------------------------------------------------------
# Suite 9: k-Pool Subset DP vs brute force
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Suite 10: Multi-set DP vs subset DP
# ---------------------------------------------------------------------------
