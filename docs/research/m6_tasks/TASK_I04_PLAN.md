...
payload root. A duplicate attempt returns `ALREADY_ACCEPTED` with the same
destination receipt root. A same-ID payload or destination change rejects
...
observational duplicate idempotence, payload/destination/profile binding, and
canonical destination-record collections. It does not prove any real network
