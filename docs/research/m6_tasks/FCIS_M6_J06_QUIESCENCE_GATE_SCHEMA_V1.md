...
comparison. The gate is valid only at `QUIESCED`, binds the J04 manifest and
K01 entrypoint roots, and requires the current and replay heads to be equal.
...
Its root is the canonical hash of every field except the self-reference.
...
isolation proof, process barrier, runtime call-graph proof, or deployment
reachability proof. Those remain J07 and K-wave implementation obligations.
