...
select a second publication tuple alongside the witness. D08 provenance and
every retained verifier source are freshly replayed once per publication
attempt. All request fingerprints and successor fields derive from the one
locally validated atom. It returns newly-committed, already-committed, or
...
- Publication-field ownership, state-bound collision, and sequence witnesses
  passed.
- Focused K02 suite passed: 8 passed.
- A permanent call-count test proves that one publication attempt performs one
  complete D08 replay rather than repeating it through request properties.
...
fingerprint collision, stale head, sequence crossing, retry-state mutation,
and repeated point-of-use replay amplification witnesses.
