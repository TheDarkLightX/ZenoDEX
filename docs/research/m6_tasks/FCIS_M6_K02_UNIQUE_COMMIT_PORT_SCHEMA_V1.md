...
The request must contain an exact `D08CombinedANFAcceptV1` verifier witness,
commit identity, expected pre-state root, successor state root, authority epoch
root, effect root, and contiguous sequence. The port recomputes a request
fingerprint from all of these fields. A same-identity/same-fingerprint retry
returns `ALREADY_COMMITTED`; a same-identity/different-fingerprint request
returns `COMMIT_COLLISION`.

The port state is immutable. A new commit advances the head root and sequence
in one returned value. Rejection returns no successor state. The port identity
