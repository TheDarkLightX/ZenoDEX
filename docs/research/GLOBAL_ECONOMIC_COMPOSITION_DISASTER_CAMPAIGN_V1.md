...
| External backend returns truthy non-Boolean CAS or acknowledges without changing its current head | `CLOSED_BOUNDED` | Require exact `bool`, decode a fresh current observation after success, and require byte-exact successor equality. |
