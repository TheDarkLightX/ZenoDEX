...
- generic authority from ordinary receipt verification and V4 journal decode;
- authenticated V4 runtime identity outside the exact historical sealed
  verifier, because the V4 guest ABI carries a host declaration;
...
The V4 bytes and retained receipt remain frozen as historical evidence. The
active successor must commit a proof-neutral value proposal with no runtime
self-image field. Its sealed verifier will attach the verified program ID,
backend manifest, receipt-security profile, and claim binding after receipt
authentication.
