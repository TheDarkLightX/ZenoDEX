...
- The ZRPF V3 structural lane now has a separate current-source, same-host
  retained-byte replay verifier anchored at commit `ff76ff9c`. Its selected dependency
  ...
  the exact 44-file source closure before and after compilation, disables
...
Response rendering consumes those facts and cannot reopen, decode, or
...
while preserving the response schema. The separate
...
pinned. It validates artifact identity, source closure, transcript identity,
and mutation semantics; it does not execute the verifier or establish
historical execution provenance, public replay, cross-host equality, release
or settlement authority, privacy, or production readiness. The combined
V1-plus-V2 promotion therefore remains withheld until an executable
current-host replay checker is governed.
