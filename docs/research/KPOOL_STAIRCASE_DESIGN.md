...
4. **State-count and work-count profiling:** `tools/profile_state_counts.py`
   ...
   sparse, moderate, dense, and adversarial pool configurations. The default
   profile cases are bounded for quick review; pass `--include-large` for the
   original large-domain envelope cases. This data establishes the actual
