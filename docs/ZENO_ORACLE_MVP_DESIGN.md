...
Perps `settle_epoch` paths, critical zUSD demo API commands, and the exact-in
guarded routing quote API now have the first runtime hooks. When configured to
require an Oracle bridge, isolated,
...
same pattern to `mint_zusd` and `liquidate`. The guarded routing quote API
applies it to `zenodex.routing / guarded_quote`, including the route request,
route policy, routing reference-price query, and pool snapshot hash. Trigger,
production zUSD, and remaining routing consumers remain adapter-only until they
get equivalent hooks.
