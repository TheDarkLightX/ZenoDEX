...
| --- | --- | --- |
| Confirm value-moving paths all go through safe profile. | `value_moving_paths_use_safe_profile` | `dex_engine_defaults_fail_closed`, `core_dex_defaults_use_strong_settlement_profile`, `named_safe_profiles_force_production_closure`, `tau_testnet_dex_plugin_enters_through_dex_engine`, `public_operator_node_preflight_blocks_unsigned_testnet_mutation` |
...
| No direct pure-core ingress accidentally exposed. | `no_direct_pure_core_ingress_exposed` | `direct_settlement_apply_helper_unexposed`, `api_server_does_not_expose_direct_value_moving_core_ingress`, `tau_testnet_dex_plugin_enters_through_dex_engine` |
