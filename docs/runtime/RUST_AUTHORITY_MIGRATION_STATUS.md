...
shadow check. The other four isolated ops remain explicitly
...
four are still consumed as shadow checks only).
...
`deposit_collateral` / `withdraw_collateral` in manual authority policies.
...
above, Rust post-checks Python's accepted transition and does not decide
accept/reject from the pre-state or commit its materialized result. Accordingly
**`perp_stateful` remains
