...
every perp/zUSD/cpmm/fee kernel calls. Added from the isolated hardening branch
and re-run on `claude/finish-rust-runtime-authority` (2026-05-31), extending
the existing 19-harness CBC core receipt (`cbc_runtime_core_kani_v1`).
...
|---------|-----------------|
| `floor_div_i128_is_total` | **TOTALITY over the full symbolic i128 x i128 domain**: `floor_div_i128` never panics/overflows/traps for any `(numerator, denominator)`. This machine-proves the D-1 fix: `checked_div` and `checked_rem` turn the `i128::MIN / -1` overflow and div-by-zero cases into `None`. (41 checks) |
| `mul_div_floor_is_total` | TOTALITY: `mul_div_floor` never panics. The `denominator == 0` guard and `checked_mul` make overflow/div-by-zero typed rejects. (8 checks) |
| `checked_add_total_and_exact` | `checked_add` is total and returns `Ok` iff the native checked op is `Some` (fail-closed on overflow, never wrap/panic). (36 checks) |
