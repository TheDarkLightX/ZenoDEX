...
| zUSD full single-vault — `src/core/zusd.py` (authority) + Rust shadow `zenodex-runtime-core::zusd` | Python (+ Rust shadow) | yes | **Rust (Phase 6, done)** | ✅ shadows the full single-vault `step`: mint/repay/deposit-sp/withdraw-sp/redeem/liquidate + oracle bootstrap/report/commit and recovery-mode gating (`tcr_ok`/`in_recovery_mode`/`risky_ops_allowed`). ✅ golden trace, ✅ Python/Rust differential incl. >u128 amounts (bignum CDP-ratio math), ✅ semantic invariants (supply conservation, balance-sheet deltas, no bad debt). Remaining: `_reference` + overflow tests (gap-map Phase D), fuzz (Phase 9). The multi-vault `step` (`zusd.py` L850+) stays Python-only |
