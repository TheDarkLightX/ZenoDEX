...
| Perps risk math — `src/core/perp_v2/math.py` (+ Rust `perp_math`) | Rust+Python shadow on public-testnet | yes | **Rust math (public-testnet promoted)** | ✅ stateless slice live-wired for oracle freshness/move/clamp, margin, signed PnL, liquidation eligibility, and funding. ✅ signed `i128` + sign-symmetry invariants, static/4×500 differential, active-policy live-path tests, disaster-state rows, deterministic fuzz, malformed-output fail-closed checks, explicit signed safe-domain rejection (`abs(value) <= 1e18`, `abs(bps) <= 1e7`), and Kani totality/non-vacuity on checked materializer-effect helpers. Production remains Python authority. The stateful epoch lifecycle / clearinghouse settlement / insurance (`engine.py`, `updates.py`) is a separate E2 surface |
| Perps stateful — `advance_epoch` + `publish_clearing_price` + `settle_epoch` + `partial_liquidate` + `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_advance_epoch`, `_apply_isolated_publish_clearing_price`, `_apply_isolated_settle_epoch`, `_apply_isolated_partial_liquidate`, `_apply_isolated_apply_funding_auto`) + account ops `deposit_collateral`/`withdraw_collateral`/`set_position`/`clear_breaker`) + `set_market_params` (+ Rust `perp_advance_epoch`, `perp_publish_clearing_price`, `perp_settle_epoch`, `perp_partial_liquidate`, `perp_funding_auto`, `perp_account_ops`, `perp_set_market_params`) | Python authority + live Rust shadow on public-testnet | yes | **Rust shadow checker (E2 live); all 10 isolated ops have manual authority slices** | ✅ Every `_ISOLATED_ACTION_HANDLERS` op is Rust-shadowed with real-authority differentials, golden traces, Rust unit/proptests, fuzz/input-disaster coverage, and funding-sink Lean/ESSO evidence. `public-testnet` now configures `perp_stateful: rust_shadow`; accepted live isolated transitions call the matching Rust checker and fail closed on available Rust disagreement. Rust full-state/effect materialization covers all 10 isolated ops. The request carries pre-state integration facts for operator/sender/oracle/balance checks, including a `balance_available` deposit guard so future Rust authority cannot accept deposits the Python shell would reject for insufficient wallet balance. The Python shell has a strict parser for accepted Rust post-states, with a live round-trip regression on an account-mutating op. Manual `rust_authority*` policies now let every isolated perps op decide and commit from Rust, with Python shadow disagreement fail-closed in `rust_authority_with_python_shadow`; deposit/withdraw also debit/credit the Python wallet after Rust accepts. No deployment profile flips. Funding-auto bounded-sink arithmetic also has exact Kani receipts on the heap-free helpers used by the running Rust transition; the wrapper remains differential/live-shadow covered. |
