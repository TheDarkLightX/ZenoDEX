...
| Perps stateful — `advance_epoch` + `publish_clearing_price` + `settle_epoch` + `partial_liquidate` + `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_advance_epoch`, `_apply_isolated_publish_clearing_price`, `_apply_isolated_settle_epoch`, `_apply_isolated_partial_liquidate`, `_apply_isolated_apply_funding_auto`) + account ops `deposit_collateral`/`withdraw_collateral`/`set_position`/`clear_breaker`) + `set_market_params` (+ Rust `perp_advance_epoch`, `perp_publish_clearing_price`, `perp_settle_epoch`, `perp_partial_liquidate`, `perp_funding_auto`, `perp_account_ops`, `perp_set_market_params`) | Python authority + live Rust shadow on public-testnet | yes | **Rust shadow checker (E2 live); 7 manual authority slices (`advance_epoch`, `publish_clearing_price`, `clear_breaker`, `set_position`, `deposit_collateral`, `withdraw_collateral`, `set_market_params`)** | ✅ Every `_ISOLATED_ACTION_HANDLERS` op is Rust-shadowed with real-authority differentials, golden traces, Rust unit/proptests, fuzz/input-disaster coverage, and funding-sink Lean/ESSO evidence. `public-testnet` now configures `perp_stateful: rust_shadow`; accepted live isolated transitions call the matching Rust checker and fail closed on available Rust disagreement. Rust full-state/effect materialization covers all 10 isolated ops. The request carries pre-state integration facts for operator/sender/oracle/balance checks, including a `balance_available` deposit guard so future Rust authority cannot accept deposits the Python shell would reject for insufficient wallet balance. The Python shell has a strict parser for accepted Rust post-states, with a live round-trip regression on an account-mutating op. Manual `rust_authority*` policies now let `advance_epoch`, `publish_clearing_price`, `clear_breaker`, `set_position`, `deposit_collateral`, `withdraw_collateral`, and `set_market_params` decide and commit from Rust, with Python shadow disagreement fail-closed in `rust_authority_with_python_shadow`; deposit/withdraw also debit/credit the Python wallet after Rust accepts. All other stateful ops stay blocked under `rust_authority*` until individually promoted. No deployment profile flips. Funding-auto bounded-sink arithmetic also has exact Kani receipts on the heap-free helpers used by the running Rust transition; the wrapper remains differential/live-shadow covered. |
