...
| zUSD full single-vault — `src/core/zusd.py` (authority) + Rust shadow `zenodex-runtime-core::zusd` | Rust+Python shadow on public-testnet | yes | **Rust (public-testnet promoted)** | ✅ full single-vault `step` live-wired through `zusd-op`: mint/repay/deposit-sp/withdraw-sp/redeem/liquidate + oracle bootstrap/report/commit, epoch advance, and recovery-mode gating (`tcr_ok`/`in_recovery_mode`/`risky_ops_allowed`). ✅ golden trace, ✅ Python/Rust differential incl. >u128 amounts (bignum CDP-ratio math), ✅ semantic invariants, ✅ `_reference` unit suite, ✅ active-policy live-path tests, ✅ disaster-state rows and deterministic fuzz. Event/effect payloads remain Python-derived after Rust/Python state-root and receipt agreement. Production remains Python authority. The multi-vault `step` (`zusd.py` L850+) stays Python-only |
| Buyback accrual + burn floor — accrual in `fee_router` (`cum_buyburn`) ✅; burn **accounting rails** `src/core/burn_receipts.py` + Rust shadow `zenodex-runtime-core::burn_receipts` | Rust+Python shadow on public-testnet | yes | **Rust rails (public-testnet promoted)**; burn *execution* later | ✅ accrual conservation, ✅ burn rails (budget/floor, supply conservation, batch accumulator) shadowed + differential + semantic invariants, ✅ live authority wiring, ✅ disaster-state rows and deterministic fuzz. Receipt envelope (schema, canonical-JSON hash, lenient `int()` coercion) stays Python-owned before the authority-gated rail tuple. Burn **execution** still gated on TWAP / pool-depth / budget+slippage caps / wash-trade controls before any execution path |
| Batch clearing — `src/core/batch_clearing.py`, `settlement*.py` | Python orchestration; Rust+Python shadow for per-pool CPMM quotes on public-testnet | yes | Rust per-surface | ✅ per-pool CPMM settlement primitive live-wired and promoted on public-testnet (`quote_cpmm_swap_exact_in/out` → `cpmm-op`), including exact-out overdelivery-gap parity. ✅ golden traces, ✅ Python/Rust differential, ✅ semantic invariants, ✅ disaster-state rows and deterministic fuzz. Orchestration (multi-pool/CoW/ordering/liquidity) still Python-only; existing Tau/Lean batch obligations stay green |
| Perps risk math — `src/core/perp_v2/math.py` (+ Rust `perp_math`) | Rust+Python shadow on public-testnet | yes | **Rust math (public-testnet promoted)** | ✅ stateless slice live-wired for oracle freshness/move/clamp, margin, signed PnL, liquidation eligibility, and funding. ✅ signed `i128` + sign-symmetry invariants, static/4×500 differential, active-policy live-path tests, disaster-state rows, deterministic fuzz, malformed-output fail-closed checks, and explicit signed safe-domain rejection (`abs(value) <= 1e18`, `abs(bps) <= 1e7`). Production remains Python authority. The stateful epoch lifecycle / clearinghouse settlement / insurance (`engine.py`, `updates.py`) is a separate E2 surface |
| Perps stateful — `advance_epoch` + `publish_clearing_price` + `settle_epoch` + `partial_liquidate` + `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_advance_epoch`, `_apply_isolated_publish_clearing_price`, `_apply_isolated_settle_epoch`, `_apply_isolated_partial_liquidate`, `_apply_isolated_apply_funding_auto`) + account ops `deposit_collateral`/`withdraw_collateral`/`set_position`/`clear_breaker`) + `set_market_params` (+ Rust `perp_advance_epoch`, `perp_publish_clearing_price`, `perp_settle_epoch`, `perp_partial_liquidate`, `perp_funding_auto`, `perp_account_ops`, `perp_set_market_params`) | Python authority + live Rust shadow on public-testnet | yes | **Rust shadow checker (E2 live); 9 manual authority slices (`advance_epoch`, `publish_clearing_price`, `settle_epoch`, `clear_breaker`, `set_position`, `deposit_collateral`, `withdraw_collateral`, `set_market_params`, `apply_funding_auto`)** | ✅ Every `_ISOLATED_ACTION_HANDLERS` op is Rust-shadowed with real-authority differentials, golden traces, Rust unit/proptests, fuzz/input-disaster coverage, and funding-sink Lean/ESSO evidence. `public-testnet` now configures `perp_stateful: rust_shadow`; accepted live isolated transitions call the matching Rust checker and fail closed on available Rust disagreement. Rust full-state/effect materialization covers all 10 isolated ops. The request carries pre-state integration facts for operator/sender/oracle/balance checks, including a `balance_available` deposit guard so future Rust authority cannot accept deposits the Python shell would reject for insufficient wallet balance. The Python shell has a strict parser for accepted Rust post-states, with a live round-trip regression on an account-mutating op. Manual `rust_authority*` policies now let `advance_epoch`, `publish_clearing_price`, `settle_epoch`, `clear_breaker`, `set_position`, `deposit_collateral`, `withdraw_collateral`, `set_market_params`, and `apply_funding_auto` decide and commit from Rust, with Python shadow disagreement fail-closed in `rust_authority_with_python_shadow`; deposit/withdraw also debit/credit the Python wallet after Rust accepts. All other stateful ops stay blocked under `rust_authority*` until individually promoted. No deployment profile flips. Funding-auto bounded-sink arithmetic also has exact Kani receipts on the heap-free helpers used by the running Rust transition; the wrapper remains differential/live-shadow covered. |
