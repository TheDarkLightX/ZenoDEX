...
| Batch clearing settlement — CPMM swap quote `settlement_swap_runtime_v1.py` (authority) + Rust shadow `zenodex-runtime-core::cpmm_swap` | Python (+ Rust shadow) | yes | **Rust settlement primitive (Phase 6, done)** | ✅ per-pool exact-in/out golden trace + Python/Rust differential + semantic invariants (constant-product k non-decreasing, exact reserve conservation, slippage admission). Multi-pool aggregation, the swap-ordering heuristics (greedy/optimal-AB/MCI/CoW), and liquidity ops in `batch_clearing.py` are orchestration **staged** on top; existing Tau/Lean batch obligations stay green |
