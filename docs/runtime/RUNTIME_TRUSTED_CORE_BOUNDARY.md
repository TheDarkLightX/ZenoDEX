...
| Perps stateful — `advance_epoch` + `publish_clearing_price` + `settle_epoch` + `partial_liquidate` + `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_advance_epoch`, `_apply_isolated_publish_clearing_price`, `_apply_isolated_settle_epoch`, `_apply_isolated_partial_liquidate`, `_apply_isolated_apply_funding_auto`) + account ops `deposit_collateral`/`withdraw_collateral`/`set_position`/`clear_breaker`) + `set_market_params` (+ Rust `perp_advance_epoch`, `perp_publish_clearing_price`, `perp_settle_epoch`, `perp_partial_liquidate`, `perp_funding_auto`, `perp_account_ops`, `perp_set_market_params`) | Python authority + live Rust shadow on public-testnet | yes | **Rust shadow checker (E2 live)** | ✅ Every `_ISOLATED_ACTION_HANDLERS` op is Rust-shadowed with real-authority differentials, golden traces, Rust unit/proptests, fuzz/input-disaster coverage, and funding-sink Lean/ESSO evidence. `public-testnet` now configures `perp_stateful: rust_shadow`; accepted live isolated transitions call the matching Rust checker and fail closed on available Rust disagreement. Python still owns the commit path, and the Rust request now carries pre-state integration facts for operator/sender/oracle/balance checks. Rust full-state/effect materialization covers all 10 isolated ops, including a `balance_available` deposit guard so future Rust authority cannot accept deposits the Python shell would reject for insufficient wallet balance. The Python shell also has a strict parser for accepted Rust post-states, with a live round-trip regression on an account-mutating op. Funding-auto bounded-sink arithmetic also has exact Kani receipts on the heap-free helpers used by the running Rust transition; the wrapper remains differential/live-shadow covered. `rust_authority*` is explicitly rejected for this surface until the authority path is inverted so Rust decides from pre-state and commits. |
