...
| Perps stateful — `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_apply_funding_auto`, post bounded-sink fix) (+ Rust `perp_funding_auto`) | Python (+ Rust shadow) | yes | **Rust shadow (E2 funding-auto slice only)** | ✅ FIRST stateful-perps slice shadowed: the bounded-sink funding settlement. The shadow mutates exactly the fields the authority does — each open account's `collateral_quote`, `funding_paid_cumulative`, `funding_last_applied_epoch` (so same-epoch replay/double-apply rejects) and the global `funding_rate_bps` — plus `fee_pool/fee_income/insurance += projected_net` (fail-closed sink bounds; no counterparty residual; no `Σ position_base == 0` requirement). All transition arithmetic is **checked** (`checked_add/_sub`, fail-closed `funding_arithmetic_overflow`). Evidence: 12 Rust unit tests + a `proptest` conservation property + Python/Rust differential (static incl. double-apply replay + 3×40 randomized, drives the real authority via `apply_perp_ops`, compares per-account state, funding_rate_bps, sinks, **and reject reason**) + golden trace (`funding_auto_smoke.json`); gate `tools/run_perp_funding_auto_sink_assurance_gate.sh` + Lean `Proofs.PerpFundingSinkConservation` wired into `run_perps_evidence.sh`. **Shadow only — NOT Rust authority.** Funding-rate derivation (`settle_price`/`compute_funding_rate_bps`) and the oracle/clearing auto-gate stay Python (rate is handed to the shadow). Remaining E2: `advance_epoch`, `publish_clearing_price`, `settle_epoch`, liquidation, insurance accounting |
