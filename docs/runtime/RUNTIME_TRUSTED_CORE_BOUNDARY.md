...
| Perps stateful — `apply_funding_auto` settlement (`src/integration/perp_engine.py::_apply_isolated_apply_funding_auto`, post bounded-sink fix) (+ Rust `perp_funding_auto`) | Python (+ Rust shadow) | yes | **Rust shadow (E2 funding-auto slice only)** | ✅ FIRST stateful-perps slice shadowed: the bounded-sink funding settlement (per-account funding + `fee_pool/fee_income/insurance += projected_net`, fail-closed sink bounds, no counterparty residual, no `Σ position_base == 0` requirement). 11 Rust property tests + Python/Rust differential (static + 3×40 randomized, drives the real authority via `apply_perp_ops`) + golden trace (`funding_auto_smoke.json`). **Shadow only — NOT Rust authority.** Funding-rate derivation (`settle_price`/`compute_funding_rate_bps`) and the oracle/clearing auto-gate stay Python (rate is handed to the shadow). Remaining E2: `advance_epoch`, `publish_clearing_price`, `settle_epoch`, liquidation, insurance accounting |
