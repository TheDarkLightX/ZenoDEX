...
| objective_tuned.schemas | pass | objective-tuned stress and hard-case schemas identify the expected checkpoint |
| objective_tuned.cross_seed_safety | pass | objective-tuned scorer has zero invalid accepts, complete top-10 recall, and low p99 calls |
| objective_tuned.beats_hand | pass | objective-tuned scorer improves mean calls and top-1 recall over hand energy |
| objective_tuned.hard_case_top10 | pass | objective-tuned hard-case mining keeps all winners in top 10 with p99 at most 2 |
| objective_tuned.negative_vs_gap_weighted | pass | objective-tuned does not strictly beat the gap-weighted default |
| synthetic_candidate_coverage.schema | pass | expected 'zenodex/energy/upba_v2_synthetic_candidate_coverage/v1', observed 'zenodex/energy/upba_v2_synthetic_candidate_coverage/v1' |
| synthetic_candidate_coverage.synthetic_only | pass | coverage rows are synthetic-only and contain no live-secret key hits |
| synthetic_candidate_coverage.candidate_types | pass | all required mutation and hard-negative candidate types are present |
| synthetic_candidate_coverage.bounded_rows | pass | bounded synthetic generation stays within request limits with no duplicate-hash batches |
| synthetic_candidate_coverage.winner_and_hard_negative_balance | pass | audit has frequent verifier winners, hard negatives, and more invalid than valid candidates |
| synthetic_candidate_coverage.feature_schema_dims | pass | all audited rows use the expected aggregate, set, and set-aware feature dimensions |
...
```json
{
  ...
  "objective_tuned": {
    "cross_seed_configs": 9,
    "hand_mean_verifier_calls": 1.3260782880941833,
    "hard_case_top5_miss_count": 1,
    "hard_case_top_10_recall": 1.0,
    "learned_invalid_accept_count_total": 0,
    "learned_mean_verifier_calls": 1.019298354770919,
    "learned_top_10_recall_min": 1.0,
  ...
  "synthetic_candidate_coverage": {
    ...
    "coverage_ok": true,
    "hard_negative_rate": 0.12484355444305381,
    "invalid_candidate_count": 2749,
    "synthetic_only": true,
    "valid_candidate_count": 447,
    "winner_batch_rate": 0.97
  },
  "topk_sweep": {
    "batches": 1983,
