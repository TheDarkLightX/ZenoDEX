from __future__ import annotations

import json
import subprocess
import sys
from pathlib import Path

import pytest

from tools.check_ab_child_frontier_corpus_root_tau_certificate_20260629 import (
    EXPECTED_CASE_COUNT,
    EXPECTED_CORPUS_ROOT,
    EXPECTED_ROW_RECEIPT_COUNT,
    REPORT_JSON,
    build_report,
    find_tau_bin,
)

ROOT = Path(__file__).resolve().parents[2]


@pytest.fixture(scope="module")
def tau_certificate_report() -> dict[str, object]:
    if not find_tau_bin(ROOT, profile="latest"):
        pytest.skip("latest Tau binary not found")
    return build_report()


def test_ab_child_frontier_corpus_root_tau_certificate_report(
    tau_certificate_report: dict[str, object],
) -> None:
    corpus = tau_certificate_report["corpus"]

    assert tau_certificate_report["schema"] == "zenodex.ab_child_frontier_corpus_root_tau_certificate_report.v1"
    assert tau_certificate_report["breakthrough"]["spec_id"] == (
        "ab_child_frontier_corpus_root_scope_certificate_v1"
    )
    assert tau_certificate_report["tau"]["ok"] is True
    assert tau_certificate_report["tau"]["invalid_accepts"] == 0
    assert all(value == 1 for value in tau_certificate_report["facts"].values())
    assert corpus["case_count"] == EXPECTED_CASE_COUNT
    assert corpus["row_receipt_count"] == EXPECTED_ROW_RECEIPT_COUNT
    assert corpus["covered_row_receipt_count"] == EXPECTED_ROW_RECEIPT_COUNT
    assert corpus["corpus_root"] == EXPECTED_CORPUS_ROOT


def test_ab_child_frontier_corpus_root_tau_negative_cases(
    tau_certificate_report: dict[str, object],
) -> None:
    cases = {case["case_id"]: case for case in tau_certificate_report["tau"]["case_results"]}

    assert cases["corpus_root_certificate_pass"]["got"]["o7"] == 1
    for case_id in (
        "missing_source_report_reject",
        "wrong_scope_reject",
        "wrong_corpus_root_reject",
        "missing_case_roots_reject",
        "missing_row_receipts_reject",
        "membership_mismatch_reject",
        "negative_controls_missing_reject",
        "nondeterministic_replay_reject",
        "linked_cross_binding_reject",
        "digest_pin_reject",
        "authority_boundary_reject",
        "authority_effect_reject",
        "empty_corpus_reject",
    ):
        assert cases[case_id]["got"]["o7"] == 0
    assert cases["inactive_safe"]["got"]["o8"] == 1


def test_ab_child_frontier_corpus_root_tau_scope_and_pins(
    tau_certificate_report: dict[str, object],
) -> None:
    nonclaims = "\n".join(tau_certificate_report["non_claims"])
    corpus = tau_certificate_report["corpus"]

    assert corpus["case_summaries_digest"] == (
        "afd7706fd7ea10cee0df44d7578dabf44fc82a26d238f814d717c5fee3b5bc28"
    )
    assert corpus["row_receipts_digest"] == (
        "d52f8c24411e841ae777999d6bfd3ec3fef5bb0a26cd98887f4e0a5902c0f092"
    )
    assert corpus["linked_cross_binding_digest"] == (
        "0996b976f70eeea56e4c828a9ff25abefdb8930b39896b4427291284e1e73551"
    )
    assert corpus["deterministic_replay_hash"] == (
        "b857b66aa96007bda748ae9489ee10f972248eaa30af25fd5ac7dffca73f4591"
    )
    assert "bounded to the committed n=7 zero-min corpus-root report" in nonclaims
    assert "Python-to-Lean refinement" in nonclaims
    assert "nonzero min_amount_out" in nonclaims
    assert "does not authorize settlement" in nonclaims


def test_ab_child_frontier_corpus_root_tau_cli_replay() -> None:
    if not find_tau_bin(ROOT, profile="latest"):
        pytest.skip("latest Tau binary not found")

    proc = subprocess.run(
        [
            sys.executable,
            "tools/check_ab_child_frontier_corpus_root_tau_certificate_20260629.py",
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=120,
    )

    assert proc.returncode == 0, proc.stdout + proc.stderr
    report = json.loads(REPORT_JSON.read_text(encoding="utf-8"))
    assert report["tau"]["ok"] is True
    assert report["corpus"]["corpus_root"] == EXPECTED_CORPUS_ROOT
    assert report["tau"]["invalid_accepts"] == 0
