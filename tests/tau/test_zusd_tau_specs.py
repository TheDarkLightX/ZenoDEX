...
def test_zusd_tau_specs_trace(spec_rel: str, steps: list[dict[str, int]], expected_o4: list[int]) -> None:
    ...
    for idx, exp in enumerate(expected_o4):
        got = outputs.get(idx, {}).get("o4")
        assert got == exp, f"{spec_rel} step {idx}: expected o4={exp}, got {got}"
