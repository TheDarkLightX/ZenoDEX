...
def test_goal_completion_audit_maps_all_prompt_items_and_blocks_goal_closure() -> None:
    ...
    assert "lean-mathlib/Proofs/DisasterAntichainBasis.lean" in items[6]["evidence_files"]
    assert "lean-mathlib/proof_receipts/disaster_antichain_basis_v1.json" in items[6]["evidence_files"]
    assert items[6]["status"] == "accepted"
    assert items[6]["complete"] is True
    assert "antichain projection must expand with newly promoted frontier axes" in items[6]["limits"]
