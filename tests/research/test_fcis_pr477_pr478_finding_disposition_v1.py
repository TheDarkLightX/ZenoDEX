...
def test_disposition_source_hashes_bind_exact_repaired_subject() -> None:
    payload = json.loads(REGISTRY.read_text(encoding="utf-8"))
    subject = payload["subjects"]["successor_gate_compatibility"]
    subprocess.run(
    ...
    for artifact in payload["source_artifacts"]:
        committed = subprocess.run(
            ["git", "show", f"{subject}:{artifact['path']}"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
        assert hashlib.sha256(committed).hexdigest() == artifact["sha256"]
