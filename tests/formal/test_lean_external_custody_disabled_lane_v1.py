...
PROOF = LEAN_PROJECT / "Proofs" / "ExternalCustodyDisabledLaneV1.lean"
...
def test_disabled_external_lane_proof_has_required_surface() -> None:
    source = PROOF.read_text(encoding="utf-8")

    assert "import Mathlib" not in source
    assert re.search(r"\b(?:sorry|admit|axiom)\b", source) is None
    for declaration in (
        "inductive ExternalCommand",
        "def allExternalCommands",
        "theorem all_external_commands_length",
        "theorem all_external_commands_complete",
...
def test_disabled_external_lane_proof_checks_with_pinned_lean() -> None:
