...
def test_lean_misc_proof_family_builds_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [
                ...
                '--wfail',
                'build',
                'Proofs.DeterministicAgentTieBreakSort',
                'Proofs.ImpossibilityTheoremV2',
            ],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
...
def test_lean_misc_proof_family_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check TauSwap.Agent.min\'_eq_sorted0\n'
        '#check TauSwap.ImpossibilityV2.impossibility\n'
    )
...
def test_lean_misc_proof_family_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
