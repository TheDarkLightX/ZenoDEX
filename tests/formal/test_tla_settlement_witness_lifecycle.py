...
def test_tla_settlement_witness_lifecycle_model_checks(tmp_path: Path) -> None:
    from tools.run_tla_models import DEFAULT_JAR, TlaModelError, _find_java, _find_tla_jar

    root = Path(__file__).resolve().parents[2]
    src_cfg = root / "formal" / "tla" / "SettlementWitnessLifecycle.cfg"
    src_tla = root / "formal" / "tla" / "SettlementWitnessLifecycle.tla"
    model_dir = tmp_path / "tla"
    model_dir.mkdir(parents=True, exist_ok=True)
    cfg = model_dir / "SettlementWitnessLifecycle.cfg"
    tla = model_dir / "SettlementWitnessLifecycle.tla"
    shutil.copyfile(src_cfg, cfg)
    shutil.copyfile(src_tla, tla)
    log_dir = tmp_path / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_path = log_dir / "SettlementWitnessLifecycle.log"
    ...
    assert "SettlementWitnessLifecycle" in content
