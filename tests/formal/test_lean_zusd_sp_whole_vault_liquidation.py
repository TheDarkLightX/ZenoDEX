...
TARGET = "Proofs.ZUSDSPWholeVaultLiquidation"
...
def test_lean_zusd_sp_whole_vault_liquidation_builds_without_warnings() -> None:
...
def test_lean_zusd_sp_whole_vault_liquidation_exported_via_proofs_root() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
    ...
    smoke = (
        "import Proofs\n"
        "#check Proofs.ZUSDSPWholeVaultLiquidation.whole_vault_ratio_weakly_improves\n"
        "#check Proofs.ZUSDSPWholeVaultLiquidation.whole_vault_can_zero_out_sp_debt\n"
    )
...
def test_lean_zusd_sp_whole_vault_liquidation_included_in_default_proofs_build() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
