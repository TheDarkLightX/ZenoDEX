...
def test_lean_settlement_family_exported_via_proofs_root() -> None:
    ...
    smoke = (
        ...
        '#check TauSwap.Settlement.PriceHistoryCertificate.verifyCertificate_iff\n'
        '#check TauSwap.SettlementValuePacket.packetOk_iff\n'
    )
...
def test_lean_settlement_family_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
