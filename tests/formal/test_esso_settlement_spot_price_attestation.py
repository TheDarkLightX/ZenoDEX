...
MODEL = ROOT / "src" / "kernels" / "dex" / "settlement_spot_price_attestation_v1.yaml"


@pytest.mark.skipif(importlib.util.find_spec("ESSO") is None, reason="ESSO is not available")
def test_esso_settlement_spot_price_attestation_verifies() -> None:
    ...
    assert report["queries"]["inductive_rebuild_attestation"]["final_result"] == "unsat"
