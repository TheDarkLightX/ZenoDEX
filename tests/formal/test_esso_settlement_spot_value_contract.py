...
MODEL = ROOT / "src" / "kernels" / "dex" / "settlement_spot_value_contract_v1.yaml"


@pytest.mark.skipif(importlib.util.find_spec("ESSO") is None, reason="ESSO is not available")
def test_esso_settlement_spot_value_contract_verifies() -> None:
    ...
    assert report["queries"]["inductive_rebuild_contract"]["final_result"] == "unsat"
