...
def test_lean_extra_proof_library_modules_build_without_warnings() -> None:
...
def test_lean_extra_proof_library_exported_via_proofs_root() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
    ...
    smoke = (
        "import Proofs\n"
        "#check ZenoDEX.cpmm_constden_1100000_safe_under_model_bounds\n"
        "#check witness_liq_price\n"
    )
...
def test_lean_extra_proof_library_included_in_default_proofs_build() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
