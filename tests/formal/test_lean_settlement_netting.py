...
TARGET = "Proofs.SettlementNetting"
...
def test_lean_settlement_netting_builds_without_warnings() -> None:
...
def test_lean_settlement_netting_exported_via_proofs_root() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
    ...
    smoke = (
        "import Proofs\n"
        "#check Proofs.SettlementNetting.netProject_universal\n"
        "#check Proofs.SettlementNetting.savings_exact_opposing\n"
    )
...
def test_lean_settlement_netting_included_in_default_proofs_build() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
