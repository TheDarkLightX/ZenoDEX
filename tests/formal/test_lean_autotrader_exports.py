...
def test_lean_autotrader_family_exported_via_proofs_root() -> None:
    ...
    smoke = (
        'import Proofs\n'
        '#check TauSwap.AutoTrader.BinaryDecision.winnerPair\n'
        '#check TauSwap.AutoTrader.DecisionBinding.decisionBindingOk\n'
        '#check TauSwap.AutoTrader.LiveReleaseCertificate.releaseOk\n'
        '#check TauSwap.AutoTrader.StageCertificate.releaseEligible\n'
    )
...
def test_lean_autotrader_family_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
