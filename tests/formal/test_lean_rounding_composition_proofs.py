...
TARGETS = ["Proofs.CrossSliceComposition", "Proofs.CPMMEdgeRounding"]
...
def test_lean_rounding_composition_modules_build_without_warnings() -> None:
...
def test_lean_rounding_composition_exported_via_proofs_root() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
    ...
    smoke = (
        "import Proofs\n"
        "#check Proofs.CrossSliceComposition.margin_absorbs_perturbation\n"
        "#check Proofs.CPMMEdgeRounding.cpmm_edge_gap_le_one\n"
    )
...
def test_lean_rounding_composition_included_in_default_proofs_build() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
