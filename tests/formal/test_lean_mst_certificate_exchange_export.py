...
def test_lean_mst_certificate_exchange_modules_build_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [
                ...
                '--wfail',
                'build',
                'Proofs.MSTCertificateSoundness',
                'Proofs.MSTCertificateExchangeStep',
            ],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
...
def test_lean_mst_certificate_exchange_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check TauSwap.MST.le_weight_offTreeEdge_of_mem_path\n'
        '#check TauSwap.MST.exists_heaviest_edge_on_path_le_offEdge\n'
    )
...
def test_lean_mst_certificate_exchange_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
