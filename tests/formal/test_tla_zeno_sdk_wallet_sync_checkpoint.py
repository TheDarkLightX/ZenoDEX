...
def test_tla_zeno_sdk_wallet_sync_checkpoint_model_checks(tmp_path: Path) -> None:
    from tools.run_tla_models import DEFAULT_JAR, TlaModelError, _find_java, _find_tla_jar

    root = Path(__file__).resolve().parents[2]
    src_cfg = root / "formal" / "tla" / "ZenoSdkWalletSyncCheckpoint.cfg"
    src_tla = root / "formal" / "tla" / "ZenoSdkWalletSyncCheckpoint.tla"
    model_dir = tmp_path / "tla"
    model_dir.mkdir(parents=True, exist_ok=True)
    cfg = model_dir / "ZenoSdkWalletSyncCheckpoint.cfg"
    tla = model_dir / "ZenoSdkWalletSyncCheckpoint.tla"
    shutil.copyfile(src_cfg, cfg)
    shutil.copyfile(src_tla, tla)
    log_path = tmp_path / "ZenoSdkWalletSyncCheckpoint.log"
    ...
    assert "ZenoSdkWalletSyncCheckpoint" in content
