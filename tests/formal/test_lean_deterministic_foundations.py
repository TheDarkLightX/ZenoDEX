...
TARGETS = [
    "Proofs.DeterministicAgentTieBreak",
...
def test_lean_deterministic_foundations_build_without_warnings() -> None:
...
def test_lean_deterministic_foundations_exported_via_proofs_root() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
    ...
    smoke = (
        "import Proofs\n"
        "#check TauSwap.Agent.exists_unique_min_of_finset_nonempty\n"
...
def test_lean_deterministic_foundations_included_in_default_proofs_build() -> None:
    lake = shutil.which("lake")
    if not lake:
        pytest.skip("lake not installed")
