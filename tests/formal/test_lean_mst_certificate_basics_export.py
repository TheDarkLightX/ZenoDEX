...
def test_lean_mst_certificate_basics_builds_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [lake, '--wfail', 'build', 'Proofs.MSTCertificateBasics'],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
...
def test_lean_mst_certificate_basics_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check TauSwap.MST.existsUnique_path\n'
        '#check TauSwap.MST.le_maxWeightOnPath_of_mem\n'
        '#check TauSwap.MST.mem_edgeFinset_of_mem_pathEdges\n'
    )
...
def test_lean_mst_certificate_basics_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
