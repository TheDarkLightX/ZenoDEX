...
def test_lean_zusd_integer_modules_build_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [
                ...
                '--wfail',
                'build',
                'Proofs.ZUSDCollateralFlowAlgebra',
                'Proofs.ZUSDDebtHomomorphism',
                'Proofs.ZUSDDualConservation',
            ],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
...
def test_lean_zusd_integer_family_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check Proofs.ZUSDCollateralFlowAlgebra.totalFlowHom\n'
        '#check Proofs.ZUSDDebtHomomorphism.Δ_zusd\n'
        '#check Proofs.ZUSDDualConservation.joint_conserves_iff\n'
    )
...
def test_lean_zusd_integer_family_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
