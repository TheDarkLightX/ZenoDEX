...
MODEL = ROOT / "src" / "kernels" / "dex" / "tau_state_app_hash_provenance_guard_v1.yaml"


@pytest.mark.skipif(importlib.util.find_spec("ESSO") is None, reason="ESSO is not available")
def test_esso_tau_state_app_hash_provenance_guard_verifies() -> None:
    ...
    assert (
        report["queries"]["inductive_rebuild_tau_state_app_hash_provenance_guard"]["final_result"]
        == "unsat"
    )
