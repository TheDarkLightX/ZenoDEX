...
def test_settlement_witness_lifecycle_v1_model_validates() -> None:
    import yaml

    from ESSO.ir.schema import CandidateIR

    root = Path(__file__).resolve().parents[2]
    model_path = root / "src" / "kernels" / "dex" / "settlement_witness_lifecycle_v1.yaml"
    ir = CandidateIR.from_json_dict(
        yaml.safe_load(model_path.read_text(encoding="utf-8"))
    ).canonicalized()
    assert ir.meta["model_id"] == "settlement_witness_lifecycle_v1"
    assert len(ir.state_vars) == 0
    assert len(ir.actions) == 1
    assert len(ir.observables.effects) == 6


def test_settlement_witness_lifecycle_v1_tau_spec_shape() -> None:
    root = Path(__file__).resolve().parents[2]
    ...
    assert "witness_coherent" in content
    assert "settled_requires_witness" in content
    assert "rejection_total" in content
    assert "lifecycle_progress" in content
    assert "settlement_witness_lifecycle_ok" in content
