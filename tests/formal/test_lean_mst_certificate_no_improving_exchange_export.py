...
def test_lean_mst_certificate_no_improving_exchange_builds_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [lake, '--wfail', 'build', 'Proofs.MSTCertificateNoImprovingExchange'],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
...
def test_lean_mst_certificate_no_improving_exchange_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check TauSwap.MST.no_improving_exchange_along_path\n'
        '#check TauSwap.MST.exists_exchange_edge_with_max_weight\n'
    )
...
def test_lean_mst_certificate_no_improving_exchange_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
