...
def test_lean_split_routing_argmax_plateau_builds_without_warnings() -> None:
    ...
    try:
        proc = subprocess.run(
            [lake, '--wfail', 'build', 'Proofs.SplitRoutingArgmaxPlateau'],
            cwd=lean_dir,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
...
def test_lean_split_routing_argmax_plateau_exported_via_proofs_root() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
    ...
    smoke = (
        'import Proofs\n'
        '#check Proofs.SplitRouting.optimal_index_mem_plateau\n'
        '#check Proofs.SplitRouting.plateau_left_edge_is_smallest_optimal\n'
        '#check Proofs.SplitRouting.zero_gap_if_selected_in_plateau\n'
    )
...
def test_lean_split_routing_argmax_plateau_included_in_default_proofs_build() -> None:
    lake = shutil.which('lake')
    if not lake:
        pytest.skip('lake not installed')
    root = Path(__file__).resolve().parents[2]
