...
TARGET = "Proofs/FCISM6FeeSourceBinding.lean"
...
def test_lean_fcis_m6_fee_source_binding_typechecks() -> None:
...
def test_lean_fcis_m6_fee_source_binding_has_no_placeholders() -> None:
    source = (LEAN_DIR / TARGET).read_text(encoding="utf-8")
    forbidden = ("sorry", "admit", "axiom ", "unsafe ")
    assert not any(token in source for token in forbidden)


def test_lean_fcis_m6_fee_source_binding_is_in_default_aggregate() -> None:
    source = AGGREGATE.read_text(encoding="utf-8")
    assert "import Proofs.FCISM6FeeSourceBinding" in source.splitlines()
