...
TARGET_SHAPES = ROOT / "docs" / "zenodex" / "world_model_promoted" / "zenodex_target_shapes.seed.json"
EVAL_TOOL = ROOT / "tools" / "world_model_target_eval.py"


def test_shape_pp_eval_has_expected_supported_and_blocked_profile() -> None:
    report = evaluate_target_shapes(TARGET_SHAPES)
    assert report["schema"] == "zenodex/target-shape-eval-report/v1"
