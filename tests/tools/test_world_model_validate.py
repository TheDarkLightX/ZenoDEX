...
VALIDATOR = ROOT / "tools" / "world_model_validate.py"
WORLD_MODEL = ROOT / "docs" / "zenodex" / "world_model_promoted" / "zenodex_world_model.seed.json"
NEGATIVE_KNOWLEDGE = (
    ROOT / "docs" / "zenodex" / "world_model_promoted" / "zenodex_negative_knowledge.seed.json"
)
TARGET_SHAPES = (
    ROOT / "docs" / "zenodex" / "world_model_promoted" / "zenodex_target_shapes.seed.json"
)
...
def test_negative_knowledge_v1_allows_missing_remaining_excluded_domain(tmp_path: Path) -> None:
    negative_knowledge = json.loads(NEGATIVE_KNOWLEDGE.read_text(encoding="utf-8"))
    negative_knowledge["schema"] = "zenodex/negative-knowledge-seed/v1"
    for record in negative_knowledge["records"]:
        if record["status"] == "narrowed":
            record.pop("remaining_excluded_domain", None)
            break
