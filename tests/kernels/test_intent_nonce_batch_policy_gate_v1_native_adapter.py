...
MODEL = Path("src/kernels/dex/intent_nonce_batch_policy_gate_v1.yaml")
ADAPTER = "src.kernels.python.intent_nonce_batch_policy_gate_v1_native_adapter:make_adapter"
...
def test_intent_nonce_batch_policy_gate_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_intent_nonce_batch_policy_gate_unknown_action(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import intent_nonce_batch_policy_gate_v1_native_adapter as module
...
def test_intent_nonce_batch_policy_gate_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import intent_nonce_batch_policy_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_batch_policy", args={}))
    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"batch_ok": True, "return_copy": False, "reject_tag": 0}


def test_intent_nonce_batch_policy_gate_mixed_presence(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import intent_nonce_batch_policy_gate_v1_native_adapter as module
    ...
    result = adapter.apply(SimpleNamespace(tag="evaluate_batch_policy", args={}))
    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"batch_ok": False, "return_copy": False, "reject_tag": 3}


def test_intent_nonce_batch_policy_gate_rejects_noncanonical_state(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import intent_nonce_batch_policy_gate_v1_native_adapter as module
