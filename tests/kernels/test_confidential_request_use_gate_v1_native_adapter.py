...
MODEL = Path("src/kernels/dex/confidential_request_use_gate_v1.yaml")
ADAPTER = "src.kernels.python.confidential_request_use_gate_v1_native_adapter:make_adapter"
...
def test_confidential_request_use_gate_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_confidential_request_use_gate_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import confidential_request_use_gate_v1_native_adapter as module
    ...
    monkeypatch.setitem(
        ...
        lambda _adapter, _command, _interp=interp_mod, _effect_key=effect_key: _interp.StepOk(
            state={"after": "confidential_request_use_gate"},
    ...
    assert dict(adapter.get_state()) == {"after": "confidential_request_use_gate"}
    assert dict(adapter.drain_effects()) == {effect_key: 7}
    assert dict(adapter.drain_effects()) == {}


def test_confidential_request_use_gate_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import confidential_request_use_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_confidential_request_use", args={}))
...
def test_confidential_request_use_gate_check_only_keeps_fresh_state(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import confidential_request_use_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["consume_request"] = 0
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_confidential_request_use", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert result.effects["request_unused_ok"] is True
    assert result.effects["transition_ok"] is True
    assert result.effects["consume_applied"] is False
    assert result.effects["request_used_after"] is False


def test_confidential_request_use_gate_replay_consume_fails_closed(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import confidential_request_use_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["request_used_before"] = 1
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_confidential_request_use", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert result.effects["request_unused_ok"] is False
    assert result.effects["transition_ok"] is False
    assert result.effects["consume_applied"] is False
    assert result.effects["request_used_after"] is True


def test_confidential_request_use_gate_rejects_noncanonical_flag(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import confidential_request_use_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["request_used_before"] = 2
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_confidential_request_use", args={}))
