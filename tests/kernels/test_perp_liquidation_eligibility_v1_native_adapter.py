...
MODEL = Path("src/kernels/dex/perp_liquidation_eligibility_v1.yaml")
ADAPTER = "src.kernels.python.perp_liquidation_eligibility_v1_native_adapter:make_adapter"
...
def test_perp_liquidation_eligibility_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_perp_liquidation_eligibility_v1_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import perp_liquidation_eligibility_v1_native_adapter as module
    ...
    monkeypatch.setitem(
        ...
        lambda _adapter, _command, _interp=interp_mod, _effect_key=effect_key: _interp.StepOk(
            state={"after": "perp_liquidation_eligibility"},
    ...
    assert dict(adapter.get_state()) == {"after": "perp_liquidation_eligibility"}
    assert dict(adapter.drain_effects()) == {effect_key: 7}
    assert dict(adapter.drain_effects()) == {}


def test_perp_liquidation_eligibility_v1_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import perp_liquidation_eligibility_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_liquidation_eligibility", args={}))
    ...
    assert dict(result.effects) == {
        ...
        "oracle_seen_ok": True,
        "oracle_fresh": True,
        "effective_maint_bps": 600,
        "maint_req_quote": 6_000,
        "liquidatable": True,
        "partial_liquidation_allowed": True,
    }


def test_perp_liquidation_eligibility_v1_rejects_safe_account(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import perp_liquidation_eligibility_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["collateral_quote"] = 7_500
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_liquidation_eligibility", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert result.effects["liquidatable"] is False
    assert result.effects["partial_liquidation_allowed"] is False


def test_perp_liquidation_eligibility_v1_rejects_noncanonical_flag(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import perp_liquidation_eligibility_v1_native_adapter as module
