...
MODEL = Path("src/kernels/dex/zusd_redeem_fee_collateral_v1.yaml")
ADAPTER = "src.kernels.python.zusd_redeem_fee_collateral_v1_native_adapter:make_adapter"
...
def _base_state() -> dict[str, int]:
    return {
        ...
        "free_debt_e8": 20_000_000_000,
        "protocol_collateral_e8": 0,
        "max_protocol_coll_e8": 2_000_000_000_000,
        "mcr_bps": 11_000,
        "redemption_fee_floor_bps": 200,
        "redemption_fee_max_bps": 200,
        "decayed_base_rate_bps": 0,
    }


def test_zusd_redeem_fee_collateral_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_zusd_redeem_fee_collateral_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import zusd_redeem_fee_collateral_v1_native_adapter as module
    ...
    assert dict(adapter.get_state()) == {"after": "zusd_redeem"}
    assert dict(adapter.drain_effects()) == {effect_key: 7}
    assert dict(adapter.drain_effects()) == {}


def test_zusd_redeem_fee_collateral_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import zusd_redeem_fee_collateral_v1_native_adapter as module
    ...
    result = adapter.apply(
        SimpleNamespace(
            tag="apply_redeem_fee_collateral",
            args={
                "amount_e8": 10_000_000_000,
                "fee_bps": 200,
                "gross_collateral_e8": 100_000_000,
                "redemption_fee_collateral_e8": 2_000_000,
                "collateral_out_e8": 98_000_000,
            },
        )
    )
    ...
    assert dict(result.state) == {
        ...
        "debt_e8": 10_000_000_000,
        "free_debt_e8": 10_000_000_000,
        "protocol_collateral_e8": 2_000_000,
        "max_protocol_coll_e8": 2_000_000_000_000,
        "mcr_bps": 11_000,
        "redemption_fee_floor_bps": 200,
        "redemption_fee_max_bps": 200,
    ...
    assert dict(result.effects) == {
        "redeemed_zusd_e8": 10_000_000_000,
        "redeemed_collateral_gross_e8": 100_000_000,
        "redeemed_collateral_out_e8": 98_000_000,
        "redemption_fee_collateral_e8": 2_000_000,
        "redemption_fee_bps": 200,
        "mcr_post_ok": True,
    }


def test_zusd_redeem_fee_collateral_rejects_protocol_cap(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import zusd_redeem_fee_collateral_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["protocol_collateral_e8"] = 1_999_999_000_000
    state["max_protocol_coll_e8"] = 1_999_999_999_999
...
def test_zusd_redeem_fee_collateral_rejects_fee_consumes_all(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import zusd_redeem_fee_collateral_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["redemption_fee_floor_bps"] = 10_000
    state["redemption_fee_max_bps"] = 10_000
    adapter.reset(state=state)
    result = adapter.apply(
        SimpleNamespace(
            tag="apply_redeem_fee_collateral",
            args={
                ...
                "fee_bps": 10_000,
                "gross_collateral_e8": 100_000_000,
                "redemption_fee_collateral_e8": 100_000_000,
                "collateral_out_e8": 0,
            },
        )
    )
...
def test_zusd_redeem_fee_collateral_rejects_post_mcr_failure(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import zusd_redeem_fee_collateral_v1_native_adapter as module
    ...
    state["free_debt_e8"] = 10_000_000_000
    adapter.reset(state=state)
    result = adapter.apply(
        SimpleNamespace(
            tag="apply_redeem_fee_collateral",
            args={
                ...
                "fee_bps": 200,
                "gross_collateral_e8": 10_000_000,
                "redemption_fee_collateral_e8": 200_000,
                "collateral_out_e8": 9_800_000,
            },
        )
    )
