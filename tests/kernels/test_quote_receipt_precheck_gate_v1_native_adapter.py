...
MODEL = Path("src/kernels/dex/quote_receipt_precheck_gate_v1.yaml")
ADAPTER = "src.kernels.python.quote_receipt_precheck_gate_v1_native_adapter:make_adapter"
...
def _base_state() -> dict[str, bool]:
    return {
        ...
        "hash_matches": True,
        "kind_ok": True,
        "canonical_certificate_allowed": True,
        "body_assets_ok": True,
        "quote_epoch_ok": True,
        "pools_object_ok": True,
        "legs_list_ok": True,
    }


def test_quote_receipt_precheck_gate_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_quote_receipt_precheck_gate_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_precheck_gate_v1_native_adapter as module
    ...
    monkeypatch.setitem(
        ...
        lambda _adapter, _command, _interp=interp_mod, _effect_key=effect_key: _interp.StepOk(
            state={"after": "quote_receipt_precheck_gate"},
    ...
    assert dict(adapter.get_state()) == {"after": "quote_receipt_precheck_gate"}
    assert dict(adapter.drain_effects()) == {effect_key: 7}
    assert dict(adapter.drain_effects()) == {}


def test_quote_receipt_precheck_gate_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_precheck_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_precheck_gate", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"precheck_ok": True, "reject_tag": 0}


def test_quote_receipt_precheck_gate_hash_mismatch_wins(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_precheck_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["hash_matches"] = False
    state["kind_ok"] = False
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_precheck_gate", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"precheck_ok": False, "reject_tag": 3}


def test_quote_receipt_precheck_gate_rejects_noncanonical_flag(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_precheck_gate_v1_native_adapter as module
