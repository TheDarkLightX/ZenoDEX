...
MODEL = Path("src/kernels/dex/quote_receipt_certificate_gate_v1.yaml")
ADAPTER = "src.kernels.python.quote_receipt_certificate_gate_v1_native_adapter:make_adapter"
...
def test_quote_receipt_certificate_gate_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_quote_receipt_certificate_gate_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_certificate_gate_v1_native_adapter as module
    ...
    monkeypatch.setitem(
        ...
        lambda _adapter, _command, _interp=interp_mod, _effect_key=effect_key: _interp.StepOk(
            state={"after": "quote_receipt_certificate_gate"},
    ...
    assert dict(adapter.get_state()) == {"after": "quote_receipt_certificate_gate"}
    assert dict(adapter.drain_effects()) == {effect_key: 7}
    assert dict(adapter.drain_effects()) == {}


def test_quote_receipt_certificate_gate_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_certificate_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_certificate_gate", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"certificate_ok": True, "reject_tag": 0}


def test_quote_receipt_certificate_gate_bad_type_wins(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_certificate_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["cert_dict_ok"] = False
    state["amount_in_match"] = False
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_certificate_gate", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"certificate_ok": False, "reject_tag": 1}


def test_quote_receipt_certificate_gate_rejects_noncanonical_flag(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_certificate_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["cert_present"] = 2
    adapter.reset(state=state)
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_certificate_gate", args={}))
