...
MODEL = Path("src/kernels/dex/quote_receipt_totals_gate_v1.yaml")
ADAPTER = "src.kernels.python.quote_receipt_totals_gate_v1_native_adapter:make_adapter"
...
def _base_state() -> dict[str, bool]:
    return {"body_amounts_ok": True, "totals_match": True}


def test_quote_receipt_totals_gate_v1_adapter_shell_lint_and_verify(tmp_path: Path) -> None:
...
def test_quote_receipt_totals_gate_unknown_action_and_effect_drain(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_totals_gate_v1_native_adapter as module
...
def test_quote_receipt_totals_gate_happy_path(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_totals_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    adapter.reset(state=_base_state())
    result = adapter.apply(SimpleNamespace(tag="evaluate_route_quote_receipt_totals_gate", args={}))
    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {"totals_ok": True, "reject_tag": 0}


def test_quote_receipt_totals_gate_rejects_noncanonical_flag(monkeypatch) -> None:
    interp_mod = _install_fake_interpreter(monkeypatch)
    from src.kernels.python import quote_receipt_totals_gate_v1_native_adapter as module

    adapter = module.make_adapter(ir={"schema": "fake"})
    state = _base_state()
    state["body_amounts_ok"] = 2
