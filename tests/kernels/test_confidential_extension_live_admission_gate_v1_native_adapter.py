...
def test_confidential_extension_live_admission_gate_unknown_action_and_effect_drain(monkeypatch) -> None:
    ...
    monkeypatch.setitem(
        ...
        lambda _adapter, _command, _interp=interp_mod, _effect_key=effect_key: _interp.StepOk(
            state={"after": "confidential_extension_live_admission_gate"},
    ...
    assert dict(adapter.get_state()) == {"after": "confidential_extension_live_admission_gate"}
...
def test_confidential_extension_live_admission_gate_happy_path(monkeypatch) -> None:
    ...
    result = adapter.apply(SimpleNamespace(tag="evaluate_confidential_extension_live_admission", args={}))

    assert isinstance(result, interp_mod.StepOk)
    assert dict(result.effects) == {
        "do_execute_ok": True,
        "receipt_verified_ok": True,
        "policy_digest_match_ok": True,
