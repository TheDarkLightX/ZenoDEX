...
def _repinned_plan_snapshot_v1(
    ...
    pins[plan_index] = replace(pins[plan_index], sha256=plan_sha256, git_blob_sha=replacement_blob)
...
def test_r5_iterative_canonical_encoder_matches_twenty_thousand_deterministic_oracle_cases() -> (
    None
):
    # Arrange. This corpus varies scalar types, key order, nesting, Unicode,
    # signs, and empty/nonempty containers without relying on random state.
    def specimen(ordinal: int) -> object:
        scalar: object
        selector = ordinal % 6
        if selector == 0:
            scalar = None
            ...
            scalar = ordinal - 10_000
        elif selector == 3:
            scalar = f'ascii-{ordinal}-\\"-\\\\'
        elif selector == 4:
            scalar = f"unicode-{chr(0x03B1 + ordinal % 24)}"
        else:
            scalar = f"non-bmp-{chr(0x1F600 + ordinal % 64)}"
        shape = ordinal % 5
        if shape == 0:
            return scalar
        if shape == 1:
            return [scalar, ordinal, not bool(ordinal % 2)]
        if shape == 2:
            return {"z": scalar, "a": ordinal}
        if shape == 3:
            return {"outer": [{"value": scalar}], "empty": []}
        return [{"b": ordinal, "a": scalar}, {}, [None]]

    # Act / Assert.
    for ordinal in range(20_000):
        value = specimen(ordinal)
        expected = json.dumps(
            value,
            allow_nan=False,
            ensure_ascii=True,
        ...
        assert core.canonical_json_bytes_v1(value) == expected
...
def test_r5_repin_accepts_format_and_object_key_order_without_semantic_change(
    ...
    source_bytes = json.dumps(reordered, ensure_ascii=True, indent=2, sort_keys=False).encode(
        "ascii"
    )
...
def _assert_r5_mutation_surface_coverage_v1(
    ...
    assert any(
        operation.startswith("duplicate-index-") for operation, _path, _value in mutation_specs
    )
...
def test_safe_reader_fifo_substitution_before_open_rejects_without_blocking(
    ...
    def substitute_before_final_open(
        ...
        *,
        ...
        if path == "regular":
            expected_flags = os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK
            if flags != expected_flags:
                pytest.fail(
                    "final FIFO-sensitive open omitted required no-follow/nonblocking flags"
                )
    ...
    monkeypatch.setattr(build_shell.os, "open", substitute_before_final_open)
    ...
    # Assert.
    assert captured.value.code == "FILE_NONREGULAR"
    assert substituted is True
    assert observed_flags == [os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK]
