...
CHECK_CLI = REPO_ROOT / "tools" / "check_fire_kernel_settlement_receipt.py"
...
def test_check_fire_kernel_settlement_receipt_cli_roundtrip(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    build_report = _build_burn_bundle(bundle_dir)

    receipt_path = bundle_dir / "kernel_settlement_receipt.json"
    ...
    assert payload["schema"] == "zenodex/fire-kernel-settlement-receipt-check-report/v1"
    ...
    assert payload["replay_input_sha256"].startswith("sha256:")
    assert payload["kernel_receipt_sha256"].startswith("sha256:")
    assert payload["kernel_eval_receipt_sha256"].startswith("sha256:")
    assert payload["holder_delta"] == 0
    assert payload["writer_delta"] == 0
    assert payload["payoff_out"] == 0
    assert payload["firev_accept"] is True


def test_check_fire_kernel_settlement_receipt_cli_rejects_tampered_receipt(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    _build_burn_bundle(bundle_dir)

    receipt_path = bundle_dir / "kernel_settlement_receipt.json"
    payload = json.loads(receipt_path.read_text(encoding="utf-8"))
    payload["payoff_out"] = 1
    ...
    assert error["schema"] == "zenodex/fire-kernel-settlement-receipt-check-report/v1"
    assert error["ok"] is False
    assert error["error"] == "payoff_out_mismatch"


def test_check_fire_kernel_settlement_receipt_cli_rejects_nonconserving_delta(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    _build_burn_bundle(bundle_dir)
...
def test_check_fire_kernel_settlement_receipt_cli_rejects_false_accept_flag(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    _build_burn_bundle(bundle_dir)
