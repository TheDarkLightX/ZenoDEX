...
CHECK_CLI = REPO_ROOT / "tools" / "check_fire_kernel_receipt.py"
...
def test_check_fire_kernel_receipt_cli_roundtrip(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    build_report = _build_burn_bundle(bundle_dir)

    receipt_path = bundle_dir / "kernel_receipt.json"
    ...
    assert payload["schema"] == "zenodex/fire-kernel-receipt-check-report/v1"
    ...
    assert payload["kernel_ref_file_sha256"].startswith("sha256:")


def test_check_fire_kernel_receipt_cli_rejects_tampered_receipt(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    _build_burn_bundle(bundle_dir)

    receipt_path = bundle_dir / "kernel_receipt.json"
    payload = json.loads(receipt_path.read_text(encoding="utf-8"))
    payload["kernel_model_id"] = "fire_drifted_kernel"
    ...
    assert error["schema"] == "zenodex/fire-kernel-receipt-check-report/v1"
    assert error["ok"] is False
    assert error["error"] == "kernel_receipt_mismatch"
