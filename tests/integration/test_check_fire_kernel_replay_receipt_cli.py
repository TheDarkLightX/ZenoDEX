...
CHECK_CLI = REPO_ROOT / "tools" / "check_fire_kernel_replay_receipt.py"
...
def test_check_fire_kernel_replay_receipt_cli_roundtrip(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "zenodex/fire-kernel-replay-receipt-check-report/v1"
    ...
    assert payload["compile_receipt_sha256"].startswith("sha256:")
    assert payload["kernel_receipt_sha256"].startswith("sha256:")
    assert payload["kernel_eval_receipt_sha256"].startswith("sha256:")
    assert payload["kernel_settlement_receipt_sha256"].startswith("sha256:")
    assert payload["transcript_sha256"].startswith("sha256:")
    assert payload["delta_sha256"].startswith("sha256:")
...
def test_check_fire_kernel_replay_receipt_cli_rejects_tampered_receipt(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "burn_bundle"
    _build_burn_bundle(bundle_dir)

    receipt_path = bundle_dir / "kernel_replay_receipt.json"
    payload = json.loads(receipt_path.read_text(encoding="utf-8"))
    payload["delta_sha256"] = "sha256:" + ("1" * 64)
