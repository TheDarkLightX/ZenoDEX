...
def test_confidential_crypto_readiness_blocks_external_only_tee_and_missing_surfaces() -> None:
    ...
    assert _surface(report, "sss_backup")["state"] == "fixture-or-beta-only"
...
def test_encrypted_sss_missing_backup_reports_production_custody_blockers() -> None:
    from src.integration.perps_wallet_encrypted_sss_backup import evaluate_perps_wallet_encrypted_sss_backup_v1

    status = evaluate_perps_wallet_encrypted_sss_backup_v1(None, None)

    assert status["encrypted_sss_backup_ready"] is False
    assert status["production_custody_ready"] is False
    assert status["cryptography_posture"] == "local-fixture-or-beta-only"
    assert "encrypted SSS backup evaluator is blocked" in status["production_readiness_gaps"]
    assert "external encrypted SSS audit evidence is not ready" in status["production_readiness_gaps"]
