...
def _write_recursive_lifecycle_fixture(
    ...
    return headers_dir, bodies_dir, proof_metadata_dir, recursive_admission_dir, header, metadata, packet
...
def _base_verify_args(headers_dir: Path, bodies_dir: Path, proof_metadata_dir: Path) -> list[str]:
    return [
        "--headers-dir",
        str(headers_dir),
        "--bodies-dir",
...
def test_recursive_lifecycle_admission_accepts_valid_packet(tmp_path: Path) -> None:
    headers_dir, bodies_dir, proof_metadata_dir, recursive_admission_dir, _, _, _ = (
...
def test_recursive_lifecycle_admission_packet_required_for_recursive_metadata(tmp_path: Path) -> None:
    headers_dir, bodies_dir, proof_metadata_dir, _, _, _, _ = _write_recursive_lifecycle_fixture(tmp_path)

    proc = _run_verify(*_base_verify_args(headers_dir, bodies_dir, proof_metadata_dir))
...
def test_recursive_lifecycle_admission_rejects_malformed_packet_without_accepting_height(
    ...
    headers_dir, bodies_dir, proof_metadata_dir, recursive_admission_dir, _, _, packet = (
        _write_recursive_lifecycle_fixture(tmp_path)
    )
