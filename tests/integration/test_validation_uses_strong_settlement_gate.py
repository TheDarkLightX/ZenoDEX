...
def test_validate_operations_requires_end_to_end_certificate_inputs() -> None:
    intent = Intent(
        module="TauSwap",
        version="0.1",
        kind=IntentKind.SWAP_EXACT_IN,
...
def test_validate_operations_reports_invalid_end_to_end_certificate_inputs(
    ...
    monkeypatch.setattr(
        "src.integration.validation.enforce_settlement_end_to_end_certificate",
        _boom,
    )
    ...
    assert ok is False
    assert err == "invalid settlement end-to-end certificate inputs: bad certificate payload"
