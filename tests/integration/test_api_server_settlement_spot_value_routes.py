...
from src.integration.api_server_settlement_spot_value_routes import (
    maybe_handle_settlement_spot_value_route,
)
...
class _FakeContract:
    def to_dict(self) -> dict[str, object]:
        return {
            "schema": "fake-settlement-spot-value-contract",
...
def test_unknown_spot_value_route_is_not_handled() -> None:
    writes, write_json = _capture()

    handled = maybe_handle_settlement_spot_value_route(
        path="/api/dex/not_spot_value",
        obj={},
...
def test_build_spot_value_rejects_bad_settlement_before_import(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _fail_on_settlement_import(monkeypatch)

    handled = maybe_handle_settlement_spot_value_route(
        path="/api/dex/build_settlement_spot_value_contract",
...
def test_build_spot_value_rejects_missing_price_input_before_import(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _fail_on_settlement_import(monkeypatch)
...
def test_build_spot_value_rejects_bad_price_containers_before_import(monkeypatch: Any) -> None:
    _fail_on_settlement_import(monkeypatch)
    ...
    cases = [
        ...
        (_request(price_attestation={}, consumer_now_epoch=True, max_attestation_age_epochs=5), "bad_consumer_now_epoch"),
        (_request(price_attestation={}, consumer_now_epoch=3, max_attestation_age_epochs=True), "bad_max_attestation_age_epochs"),
        (
            _request(
                price_attestation={},
...
def test_build_spot_value_asset_prices_success_and_arguments(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    captured: dict[str, object] = {}
    parsed_settlement = object()
    _patch_parse_settlement(monkeypatch, parsed_settlement)

    def build_settlement_spot_value_contract(**kwargs: object) -> _FakeContract:
        captured.update(kwargs)
        return _FakeContract()

    monkeypatch.setattr(
        "src.integration.settlement_value_contract.build_settlement_spot_value_contract",
...
def test_build_spot_value_invalid_asset_price_maps_to_generic_error(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _patch_parse_settlement(monkeypatch, object())

    handled = maybe_handle_settlement_spot_value_route(
        path="/api/dex/build_settlement_spot_value_contract",
        obj=_request(asset_prices={" ": 100}),
        write_json=write_json,
    )
...
def test_build_spot_value_uses_attestation_before_packet_or_asset_prices(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    captured: dict[str, object] = {}
    parsed_settlement = object()
    ...
    def build_settlement_spot_value_contract_from_price_attestation(**kwargs: object) -> _FakeContract:
        captured.update(kwargs)
        return _FakeContract()
    ...
    monkeypatch.setattr(
        "src.integration.settlement_value_contract.build_settlement_spot_value_contract_from_price_attestation",
        build_settlement_spot_value_contract_from_price_attestation,
    )
    monkeypatch.setattr(
        "src.integration.settlement_value_contract.build_settlement_spot_value_contract_from_price_packet",
...
def test_build_spot_value_uses_price_packet_before_asset_prices(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    captured: dict[str, object] = {}
    parsed_settlement = object()
    fake_packet = object()
    ...
    def build_settlement_spot_value_contract_from_price_packet(**kwargs: object) -> _FakeContract:
        captured.update(kwargs)
        return _FakeContract()
...
def test_verify_spot_value_rejects_bad_contract_before_import(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _fail_on_settlement_import(monkeypatch)

    handled = maybe_handle_settlement_spot_value_route(
        path="/api/dex/verify_settlement_spot_value_contract",
...
def test_verify_spot_value_asset_prices_success_and_arguments(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    captured: dict[str, object] = {}
    contract = {"schema": "fake-contract"}
    _patch_parse_settlement(monkeypatch, object())

    def verify_settlement_spot_value_contract_payload(**kwargs: object) -> tuple[bool, None]:
        captured.update(kwargs)
        return True, None

    monkeypatch.setattr(
        "src.integration.settlement_value_contract.verify_settlement_spot_value_contract_payload",
...
def test_verify_spot_value_preserves_rejection_error(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _patch_parse_settlement(monkeypatch, object())

    def verify_settlement_spot_value_contract_payload(**_kwargs: object) -> tuple[bool, str]:
    ...
    assert handled is True
    assert writes == [(200, {"ok": False, "error": "settlement spot value contract mismatch"})]


def test_verify_spot_value_price_packet_and_attestation_arguments(monkeypatch: Any) -> None:
    parsed_settlement = object()
    _patch_parse_settlement(monkeypatch, parsed_settlement)
    contract = {"schema": "fake-contract"}
    ...
    monkeypatch.setattr(
        "src.integration.settlement_value_contract.verify_settlement_spot_value_contract_payload_from_price_packet",
        verify_from_price_packet,
    )
    monkeypatch.setattr(
        "src.integration.settlement_value_contract.verify_settlement_spot_value_contract_payload_from_price_attestation",
        verify_from_price_attestation,
    )

    writes_packet, write_packet = _capture()
    handled_packet = maybe_handle_settlement_spot_value_route(
        path="/api/dex/verify_settlement_spot_value_contract",
    ...
    handled_attestation = maybe_handle_settlement_spot_value_route(
        path="/api/dex/verify_settlement_spot_value_contract",
        obj=_request(
            price_packet={"packet": "present"},
            price_attestation={"attestation": "present"},
...
def test_verify_spot_value_exception_payload(monkeypatch: Any) -> None:
    writes, write_json = _capture()
    _patch_parse_settlement(monkeypatch, object())
