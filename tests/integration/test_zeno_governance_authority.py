...
def test_governance_authority_rejects_integer_backed_backend_booleans() -> None:
    backend = _backend().public_dict()
    backend["active"] = 1
    backend["no_raw_private_key_exposure"] = 1

    receipt = _evaluate(backend_descriptors=[backend])

    assert receipt["ok"] is False
    assert "backend_descriptors[0].active_must_be_bool" in receipt["errors"]
