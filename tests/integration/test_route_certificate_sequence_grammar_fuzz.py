...
def test_route_certificate_sequence_minimize_case_preserves_candidate_set_hash_mismatch() -> None:
    ...
    # The path hash includes traced source line numbers, so branch-head and
    # pull-request merge contexts can differ after unrelated base-line shifts.
    assert witness.path_id in {"78e394d9fa5a75ae", "fe54ed0a39de3b41"}
...
def test_route_certificate_sequence_minimize_case_preserves_reordered_candidate_set_hash_mismatch() -> None:
    ...
    # The path hash includes traced source line numbers, so branch-head and
    # pull-request merge contexts can differ after unrelated base-line shifts.
    assert witness.path_id in {"722391bcba709dce", "8937e149990eff93"}
