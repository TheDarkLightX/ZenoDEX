...
def test_settle_epoch_3p_oracle_adapter_bridge_required_when_configured() -> None:
    from src.integration.perp_engine import PerpEngineConfig, apply_perp_ops

    market_id = "perp:ch3p:oracle-adapter-required"
    ...
    res = apply_perp_ops(
        config=cfg,
        state=state,
        operations={"5": [_op(market_id, "settle_epoch", version="1.1")]},
        tx_sender_pubkey=_ALICE_PUBKEY,
