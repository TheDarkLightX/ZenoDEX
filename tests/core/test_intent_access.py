...
import src.core.intent_access as intent_access_module
...
def _sum_boost_create_and_add_liq() -> tuple[str, str, str, Intent, Intent]:
    pk_a = "0x" + "11" * 48
    pk_b = "0x" + "22" * 48
    asset0 = "0x" + "03" * 32
    asset1 = "0x" + "04" * 32
    ...
    add_liq = Intent(
        ...
        sender_pubkey=pk_b,
        deadline=9999999999,
        fields={"pool_id": pool_id, "amount0_desired": 10, "amount1_desired": 20},
    )
    return pk_b, asset0, asset1, create_pool, add_liq
...
def test_partition_uses_live_curve_config_for_add_liquidity_into_new_pool() -> None:
    pk_b, asset0, asset1, create_pool, add_liq = _sum_boost_create_and_add_liq()
    pool_other = "0x" + "bb" * 32
    pools = {
        pool_other: PoolState(
            pool_id=pool_other,
            asset0=asset0,
            asset1=asset1,
            reserve0=1000,
    ...
    groups = partition_independent_intents([create_pool, add_liq, swap_touching_pkb], pools=pools)
    group_ids = [{i.intent_id for i in group} for group in groups]
    assert any({add_liq.intent_id, swap_touching_pkb.intent_id}.issubset(ids) for ids in group_ids)


def test_partition_propagates_created_pool_helper_bug(monkeypatch: pytest.MonkeyPatch) -> None:
    pk = "0x" + "11" * 48
    create_pool = Intent(
        module="TauSwap",
        version="0.1",
        ...
        sender_pubkey=pk,
        deadline=9999999999,
        fields={"asset0": "0x" + "01" * 32, "asset1": "0x" + "02" * 32, "fee_bps": 30},
    )

    def broken_compute_pool_id(*args: object, **kwargs: object) -> str:
        raise RuntimeError("pool-id helper bug")

    monkeypatch.setattr(intent_access_module, "compute_pool_id", broken_compute_pool_id)
    with pytest.raises(RuntimeError, match="pool-id helper bug"):
        partition_independent_intents([create_pool], pools={})


def test_access_for_intent_propagates_create_pool_helper_bug(monkeypatch: pytest.MonkeyPatch) -> None:
    pk = "0x" + "11" * 48
    create_pool = Intent(
        module="TauSwap",
        version="0.1",
    ...
    monkeypatch.setattr(intent_access_module, "compute_pool_id", broken_compute_pool_id)
    with pytest.raises(RuntimeError, match="pool-id helper bug"):
        access_for_intent(create_pool, pools={}, created_pools={})
