...
def test_partition_does_not_swallow_pool_id_implementation_fault(monkeypatch) -> None:
    pk_a = "0x" + "11" * 48
    asset0 = "0x" + "01" * 32
    asset1 = "0x" + "02" * 32
    ...
    def broken_compute_pool_id(*_args, **_kwargs):
        raise RuntimeError("injected pool-id implementation fault")

    monkeypatch.setattr(intent_access, "compute_pool_id", broken_compute_pool_id)

    with pytest.raises(RuntimeError, match="injected pool-id implementation fault"):
