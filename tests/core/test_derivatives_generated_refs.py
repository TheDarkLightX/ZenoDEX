...
def test_funding_rate_market_ref_import_and_one_step() -> None:
    ...
    cmd = ref.Command(tag="open_rate_long", args={"amount": 1, "new_implied_rate_bps": 0, "auth_ok": True})
    res = ref.step(s, cmd)
...
def test_curve_selection_market_ref_import_and_one_step() -> None:
    ...
    cmd = ref.Command(tag="stake_on_curve", args={"amount": 1, "curve_id": 0, "auth_ok": True})
    res = ref.step(s, cmd)
    assert res.ok, res.error
    assert res.state is not None
