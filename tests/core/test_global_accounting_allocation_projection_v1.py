...
def _no_certificate_reconciles(state) -> str:
    """Build the only state-consistent fragment and return the checker code that refuses it.
    ...
    candidate = renderer._certificate_with_fragments(base, (fragment, *base.ordered_lane_fragments[1:]))
...
def test_reject_codes_are_closed_and_ordered() -> None:
    ...
    assert len(ALLOCATION_PROJECTION_REJECT_CODES_V1) == 13
    import re as _re
