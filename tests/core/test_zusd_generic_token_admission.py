...
def _expected_code(
    ...
    *,
    ...
    if recipient_class.is_reserved_protocol_location:
        return (
...
@pytest.mark.parametrize("recipient_class", CanonicalZUSDRecipientClass)
...
@pytest.mark.parametrize(
    ...
    tuple(
        recipient_class
        for recipient_class in CanonicalZUSDRecipientClass
        if recipient_class.is_reserved_protocol_location
    ),
)
def test_every_reserved_protocol_location_role_rejects_generic_canonical_transfer(
    recipient_class: CanonicalZUSDRecipientClass,
) -> None:
    decision = evaluate_generic_token_admission(
        GenericTokenAdmissionCommand(
...
def test_every_policy_rejection_is_an_identity_transition(supply: int) -> None:
    ...
    rejected_commands = (
        ...
        GenericTokenAdmissionCommand(
            ...
            TokenAssetClass.CANONICAL_ZUSD,
            TokenWriterRole.GENERIC_TOKEN_WRITER,
            CanonicalZUSDRecipientClass.HOST_FEE_POOL_LEDGER,
        ),
        GenericTokenAdmissionCommand(
...
def test_reserved_protocol_location_registry_is_immutable_sorted_and_exact() -> None:
    stability_pool = ReservedCanonicalZUSDPrincipal(
        "0x01", CanonicalZUSDRecipientClass.STABILITY_POOL_ESCROW
    )
    bridge = ReservedCanonicalZUSDPrincipal(
        "0x02", CanonicalZUSDRecipientClass.BRIDGE_ESCROW
    )
    registry = CanonicalZUSDProtocolAddressRegistry((stability_pool, bridge))
    assert registry.classify("0x01") is CanonicalZUSDRecipientClass.STABILITY_POOL_ESCROW
    assert registry.classify("0x02") is CanonicalZUSDRecipientClass.BRIDGE_ESCROW
    assert registry.classify("0x03") is CanonicalZUSDRecipientClass.ORDINARY_ACCOUNT

    with pytest.raises(ValueError, match="strictly sorted"):
        CanonicalZUSDProtocolAddressRegistry((bridge, stability_pool))
    with pytest.raises(ValueError, match="strictly sorted"):
        CanonicalZUSDProtocolAddressRegistry((stability_pool, stability_pool))
    with pytest.raises(ValueError, match="ordinary accounts"):
        ReservedCanonicalZUSDPrincipal(
            "0x03", CanonicalZUSDRecipientClass.ORDINARY_ACCOUNT
        )
...
def test_inputs_and_outputs_are_strictly_typed_and_immutable() -> None:
    ...
    with pytest.raises(TypeError, match="recipient_class"):
        GenericTokenAdmissionCommand(  # type: ignore[arg-type]
            GenericTokenAction.TRANSFER,
            TokenAssetClass.CANONICAL_ZUSD,
            TokenWriterRole.GENERIC_TOKEN_WRITER,
