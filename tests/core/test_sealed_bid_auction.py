...
import src.core.sealed_bid_auction as sealed_bid
...
def test_commit_receipt_rejects_malformed_numeric_fields() -> None:
    commitment = sealed_bid_reveal_hash(quantity=4, limit_price=105, nonce="n1")
    receipt = make_sealed_bid_commit_receipt(
        batch_id="b1",
        bidder_id="alice",
    ...
    receipt["body"] = dict(receipt["body"], commit_epoch=object())
...
def test_reveal_match_surfaces_hash_implementation_fault(monkeypatch) -> None:
    def fail_hash(*_args, **_kwargs):
        raise RuntimeError("sealed-bid hash bug")

    monkeypatch.setattr(sealed_bid, "sealed_bid_reveal_hash", fail_hash)

    with pytest.raises(RuntimeError, match="sealed-bid hash bug"):
        reveal_matches_commitment(commitment="0xabc", quantity=4, limit_price=105, nonce="n1")
