"""Smoke tests for the generated reference model of the 3-party transfer clearinghouse perp kernel."""
...
import perp_epoch_clearinghouse_3p_transfer_v0_1_ref as ref  # type: ignore  # noqa: E402
...
def test_settlement_can_transfer_distressed_position_to_idle_account():
    s = ref.init_state()
    ...
    # Fund all accounts (quote-e8 collateral).
    ...
    s, _ = _apply(s, "deposit_collateral_c", amount_e8=20_000_000_000, auth_ok=True)
    assert s.net_deposited_e8 == 40_000_000_000
    assert s.collateral_e8_a + s.collateral_e8_b + s.collateral_e8_c + s.fee_pool_e8 == s.net_deposited_e8

    # Open a matched AB position pair (C must be idle).
    s, _ = _apply(s, "set_position_pair_ab", new_position_base_a=1000, auth_ok=True)
    assert s.position_base_a == 1000
    assert s.position_base_b == -s.position_base_a
    assert s.position_base_c == 0

    # Move the clearing price up by +5%: the short side drops below maintenance.
    # The distressed position is transferred to the idle account (C), and a liquidation penalty is charged.
    s, _ = _apply(s, "advance_epoch", delta=1)
    s, _ = _apply(s, "publish_clearing_price", price_e8=105_000_000)
    s, eff = _apply(s, "settle_epoch")

    assert eff["liquidated"] is True
    assert s.index_price_e8 == 105_000_000

    # Position transfer: B was distressed; its position is moved to idle C.
    assert s.position_base_a == 1000
    assert s.position_base_b == 0
    assert s.position_base_c == -1000
    assert s.entry_price_e8_a == s.index_price_e8
    assert s.entry_price_e8_b == 0
    assert s.entry_price_e8_c == s.index_price_e8
    ...
    assert s.collateral_e8_c == 20_000_000_000
