...
from src.core.fcis_traced_reads_v5 import (
...
def _replay_reject(
    ...
    result, observed = replay_exact_route_observed_v1(intent, binding, snapshot_pool_map(pools))
...
def test_exact_in_and_exact_out_bindings_derive_and_replay() -> None:
    ...
    exact_in_intent = _admitted(_exact_in_fields(pools, (4_000, 6_000)))
    exact_in = _derive_ok(exact_in_intent)
    assert exact_in.kind is RouteKindV1.EXACT_IN
    assert exact_in.total_amount_in == 10_000
    assert len(exact_in.legs) == 2
    replay, observed = replay_exact_route_observed_v1(exact_in_intent, exact_in, committed)
    ...
    assert observed == tuple(sorted(pool.pool_id for pool in pools))
    assert route_binding_pins_exact_snapshot_v1(exact_in_intent, exact_in, committed) is True

    exact_out_intent = _admitted(
        _exact_out_fields(pools, (3_000, 5_000)), IntentKind.ROUTE_EXACT_OUT
    )
    exact_out = _derive_ok(exact_out_intent)
    assert exact_out.kind is RouteKindV1.EXACT_OUT
    replay_out = replay_exact_route_v1(exact_out_intent, exact_out, committed)
    assert type(replay_out) is RouteReplayOkV1
    assert replay_out.total_amount_out == 8_000
    assert route_binding_pins_exact_snapshot_v1(exact_out_intent, exact_out, committed) is True


def test_repeated_pool_ids_across_legs_share_threaded_reserves() -> None:
    pool = _pool(30)
    fields = _exact_in_fields((pool, pool), (4_000, 6_000))
    intent = _admitted(fields)
    binding = _derive_ok(intent)
    ...
    assert observed == (pool.pool_id,)


def test_replay_rejection_precedence_and_observed_prefixes() -> None:
    pool_a, pool_b = _pools()
    intent = _admitted(_exact_in_fields((pool_a, pool_b), (4_000, 6_000)))
    binding = _derive_ok(intent)
    ...
    result, observed = _replay_reject(intent, binding, missing)
    ...
    result, observed = _replay_reject(intent, binding, frozen_map)
    ...
    result, observed = _replay_reject(intent, binding, {**pools_by_id, pool_b.pool_id: drifted})
...
def test_invalid_orientation_and_quote_mismatch_reject() -> None:
    ...
    wrong_binding = _derive_ok(wrong_intent)
    result, observed = _replay_reject(wrong_intent, wrong_binding, {wrong_pool.pool_id: wrong_pool})
    assert result.code is RouteReplayRejectCodeV1.INVALID_PARAMS
    assert observed == (wrong_pool.pool_id,)
    ...
    mismatch_intent = _admitted(mismatch_fields)
    mismatch_binding = _derive_ok(mismatch_intent)
    result, observed = _replay_reject(mismatch_intent, mismatch_binding, {pool.pool_id: pool})
...
def test_reversed_leg_order_changes_binding_and_replay_when_semantic() -> None:
    pool = _pool(30)
    fields = _exact_in_fields((pool, pool), (4_000, 6_000))
    forward_intent = _admitted(fields)
    forward = _derive_ok(forward_intent)

    reversed_fields = dict(fields)
    reversed_fields["route_legs"] = [fields["route_legs"][1], fields["route_legs"][0]]  # type: ignore[index]
    reversed_intent = _admitted(reversed_fields)
    reversed_binding = _derive_ok(reversed_intent)

    assert forward != reversed_binding
    committed = snapshot_pool_map({pool.pool_id: pool})
    assert type(replay_exact_route_v1(forward_intent, forward, committed)) is RouteReplayOkV1
    replay, observed = replay_exact_route_observed_v1(reversed_intent, reversed_binding, committed)
...
def test_hostile_inputs_at_exact_apis() -> None:
    ...
    with pytest.raises(TypeError, match="exact OwnedIntentV1"):
        replay_exact_route_observed_v1(object(), binding, committed)  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="exact RouteBindingV1"):
        replay_exact_route_observed_v1(intent, object(), committed)  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="exact committed pool map"):
        replay_exact_route_observed_v1(
            intent,
            binding,
            {pool.pool_id: pool for pool in pools},  # type: ignore[arg-type]
        )
...
def test_object_setattr_corruption_returns_closed_rejection_with_no_pool_read() -> None:
    pool_a, pool_b = _pools()
    fields = _exact_in_fields((pool_a, pool_b), (4_000, 6_000))
    committed = snapshot_pool_map({pool_a.pool_id: pool_a, pool_b.pool_id: pool_b})

    def fresh() -> tuple[OwnedIntentV1, RouteBindingV1]:
        intent = _admitted(fields)
        return intent, _derive_ok(intent)
    ...
    for field, value in corruption_cases:
        intent, binding = fresh()
        object.__setattr__(binding, field, value)
        result, observed = replay_exact_route_observed_v1(intent, binding, committed)
        ...
        pins, pin_reads = route_binding_pins_exact_snapshot_observed_v1(intent, binding, committed)
    ...
    object.__setattr__(
        binding.pool_fingerprints,
        "_index",
        dict(binding.pool_fingerprints.entries),
    )
    result, observed = replay_exact_route_observed_v1(intent, binding, committed)
    assert type(result) is RouteReplayRejectV1
    assert result.code is RouteReplayRejectCodeV1.BINDING_INVALID
...
def test_derivation_and_replay_are_deterministic_across_repeated_calls() -> None:
    ...
    first_replay, first_observed = replay_exact_route_observed_v1(intent, first.binding, committed)
    second_replay, second_observed = replay_exact_route_observed_v1(
        intent, first.binding, committed
    )
...
def test_non_observed_wrappers_are_projections_of_observed_variants() -> None:
    ...
    observed_replay, _reads = replay_exact_route_observed_v1(intent, binding, committed)
    assert replay_exact_route_v1(intent, binding, committed) == observed_replay
    observed_pins, _pin_reads = route_binding_pins_exact_snapshot_observed_v1(
        intent, binding, committed
    )
    assert route_binding_pins_exact_snapshot_v1(intent, binding, committed) is observed_pins


def test_exact_traced_wrappers_preserve_command_binding_and_canonical_reads() -> None:
    pool = _pool(30)
    intent = _admitted(_exact_in_fields((pool, pool), (4_000, 6_000)))
    binding = _derive_ok(intent)
    committed = snapshot_pool_map({pool.pool_id: pool})

    replay, replay_trace = replay_exact_route_traced_v5(
        intent=intent,
        binding=binding,
        pools=committed,
    ...
    assert type(replay) is RouteReplayOkV1
    assert replay_trace.pool_ids == (pool.pool_id,)

    pins, pin_trace = route_binding_pins_exact_snapshot_traced_v5(
        intent=intent,
        binding=binding,
        pools=committed,
        trace=FCISStateReadTraceV5(),
    ...
    assert pins is True
    assert pin_trace.pool_ids == (pool.pool_id,)

    other_intent = _admitted(_exact_in_fields((pool,), (5_000,)))
    rejected, rejected_trace = replay_exact_route_traced_v5(
        intent=other_intent,
        binding=binding,
        pools=committed,
        trace=FCISStateReadTraceV5(),
    )
    assert type(rejected) is RouteReplayRejectV1
    assert rejected.code is RouteReplayRejectCodeV1.BINDING_INVALID
    assert rejected_trace == FCISStateReadTraceV5()
