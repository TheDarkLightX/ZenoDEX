...
from src.core.quartic_blend_amm import swap_exact_in_quartic_blend, swap_exact_out_quartic_blend


def _k(*, x: int, y: int, c_num: int, c_den: int) -> int:
    quad_scaled = c_den * (x * x + y * y) + c_num * x * y
    return x * y * quad_scaled
            ...
            for dx in range(1, 18):
                ...
                if out <= 0:
                    with pytest.raises(ValueError):
                        swap_exact_in_quartic_blend(x, y, dx, c_num=c_num, c_den=c_den)
                    continue
                out2, (x2, y2) = swap_exact_in_quartic_blend(x, y, dx, c_num=c_num, c_den=c_den)
                assert out2 == out
    ...
    for x in range(1, 14):
        for y in range(2, 14):
            ...
            for dy in range(1, y):
                ...
                dx2, (x2, y2) = swap_exact_out_quartic_blend(x, y, dy, c_num=c_num, c_den=c_den)
                assert dx2 == dx
                assert (x2, y2) == (x + dx, y - dy)
                assert _k(x=x2, y=y2, c_num=c_num, c_den=c_den) >= k0

                out_check, _ = swap_exact_in_quartic_blend(x, y, dx, c_num=c_num, c_den=c_den)
                assert out_check >= dy

                if dx > 1:
                    try:
                        out_prev, _ = swap_exact_in_quartic_blend(x, y, dx - 1, c_num=c_num, c_den=c_den)
                        assert out_prev < dy
                    except ValueError:
                        pass
        ...
        for y in range(2, 12):
            ...
            for dx_gross in range(1, 28):
                try:
                    ...
                    with pytest.raises(ValueError):
                        swap_exact_in_quartic_blend(x, y, dx_gross, c_num=c_num, c_den=c_den, fee_bps=fee_bps)
                    continue
                ...
                out2, (x2, y2) = swap_exact_in_quartic_blend(
    ...
    for x in range(2, 11):
        for y in range(3, 11):
            ...
            for dy in range(1, min(6, y - 1)):
                ...
                dx2, (x2, y2) = swap_exact_out_quartic_blend(
                ...
                out_check, _ = swap_exact_in_quartic_blend(
                ...
                if dx2 > 1:
                    try:
                        out_prev, _ = swap_exact_in_quartic_blend(
