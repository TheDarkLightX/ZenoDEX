...
def test_strong_validator_has_no_strippable_runtime_asserts() -> None:
    tree = ast.parse(Path(strong_validator.__file__).read_text(encoding="utf-8"))
...
def test_strong_validator_rejects_exact_in_field_kernel_and_apply_failures(monkeypatch) -> None:
    ...
    assert ok is False
    assert err == "strong validator crashed: RuntimeError: helper bug"
    monkeypatch.undo()
