...
def test_apply_funding_rate_cap_boundary_exact_vs_outside():
    ...
    r_pos_outside = step(s, ActionParams(action=Action.APPLY_FUNDING, new_rate_bps=101, auth_ok=True))
    assert not r_pos_outside.accepted
    assert r_pos_outside.rejection == "guard"

    r_neg_outside = step(s, ActionParams(action=Action.APPLY_FUNDING, new_rate_bps=-101, auth_ok=True))
...
def test_apply_insurance_claim_amount_boundary_exact_balance_vs_plus_one():
    s = replace(initial_state(), insurance_balance=1_000, initial_insurance=1_000, fee_income=0, claims_paid=0)
    ...
    r_eq = step(s, ActionParams(action=Action.APPLY_INSURANCE_CLAIM, claim_amount=1_000, auth_ok=True))
    assert r_eq.accepted
    assert r_eq.state is not None
    assert r_eq.state.insurance_balance == 0

    r_plus_one = step(s, ActionParams(action=Action.APPLY_INSURANCE_CLAIM, claim_amount=1_001, auth_ok=True))
...
def test_settle_epoch_oracle_move_boundary_exact_vs_one_tick_past():
    ...
    r_exact = step(replace(s, clearing_price_e8=105_000_000), ActionParams(action=Action.SETTLE_EPOCH))
    ...
    r_past = step(replace(s, clearing_price_e8=105_000_001), ActionParams(action=Action.SETTLE_EPOCH))
