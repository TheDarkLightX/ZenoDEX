...
def test_limits_and_input_validation_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="must be an int"):
        solve_two_pool_cpmm_subset_dp(pool0, pool1, [True])
    with pytest.raises(ValueError, match="fee_bps must be an int"):
        solve_two_pool_cpmm_subset_dp(TwoPoolCPMM(10, 10, True), pool1, [1])
