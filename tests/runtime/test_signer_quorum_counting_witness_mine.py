...
*counting* logic — active-signer filtering, (signer_id,key_id) dedup, weight
...
uses `hypothesis` to search for a counting witness for the disaster class:
    ...
    QUORUM FORGERY:  admit  ⟹  the accepted set is DISTINCT, ACTIVE registry
    signers whose registry weights sum to accepted_weight, and
...
# A signer slice: (active?, weight in 1..5). Identity i is fixed by position.
_signers = st.lists(
...
def test_invariant_catches_forged_certificate():
    ...
    with pytest.raises(AssertionError, match="non-active/unregistered"):
        _assert_no_forgery(forged_phantom_signer, threshold=5, active_ids=active_ids, weight_by_id=weight_by_id)


@settings(max_examples=2000, suppress_health_check=[HealthCheck.function_scoped_fixture])
