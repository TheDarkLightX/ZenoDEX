...
def test_credit_does_not_swallow_unexpected_canonicalizer_fault(monkeypatch):
    def broken_canonicalizer(*_args, **_kwargs):
        raise RuntimeError("canonicalizer implementation fault")

    monkeypatch.setattr("src.core.balance_kernel.canonical_hex_fixed_allow_0x", broken_canonicalizer)
