...
from src.core import replay_guard as replay_guard_module
...
def test_internal_sender_canonicalizer_fault_is_not_masked(monkeypatch):
    def broken_canonicalizer(*_args, **_kwargs):
        raise RuntimeError("internal sender canonicalizer fault")

    monkeypatch.setattr(replay_guard_module, "canonical_hex_fixed_allow_0x", broken_canonicalizer)

    with pytest.raises(RuntimeError, match="internal sender canonicalizer fault"):
