...
from tools.risc0_build_host_qualification_v1 import (
    ...
    IMMUTABLE_BOOTSTRAP_CLOSURE_RULE_V1,
    IMMUTABLE_BOOTSTRAP_MINIMUM_SUBJECTS_V1,
    IMMUTABLE_BOOTSTRAP_PREREQUISITE_V1,
    LEGACY_LOCK_PATH_V1,
    MINIMUM_AVAILABLE_MEMORY_BYTES_V1,
    MINIMUM_TMP_FREE_BYTES_V1,
    PLAN_PATH_V1,
    ...
    sha256_prefixed_v1,
    with_artifact_payload_digest_v1,
)
...
def _forged_qualified_projection(artifact: dict[str, object]) -> dict[str, object]:
    forged = decode_json_object_v1(canonical_json_bytes_v1(artifact), "forged-artifact.json")
    result = _json_object(forged["result"])
    result["blocked"] = False
    result["qualification_eligible"] = True
    result["status"] = "QUALIFIED"
    return with_artifact_payload_digest_v1(forged)
...
def test_given_exact_p_c_e_when_replayed_then_the_blocked_artifact_is_source_bound(tmp_path: Path) -> None:
    ...
    _expect(
        bootstrap["status"] == IMMUTABLE_BOOTSTRAP_PREREQUISITE_V1,
        "the checker must name its immutable executable-closure prerequisite",
    )
    _expect(
        tuple(_json_array(bootstrap["minimum_subjects"]))
        == IMMUTABLE_BOOTSTRAP_MINIMUM_SUBJECTS_V1,
        "bootstrap nonclaim must enumerate its minimum externally pinned subjects",
    )
    _expect(
        bootstrap["closure_rule"] == IMMUTABLE_BOOTSTRAP_CLOSURE_RULE_V1,
        "bootstrap nonclaim must require the external launcher to bind transitive execution",
    )
    _expect(
        bootstrap["self_bootstrap"] == "NOT_CLAIMED",
        "the Python checker must not claim to authenticate its own import closure",
    )
    _expect(
        bootstrap["process_isolation"]
        == "EXTERNAL_SANDBOX_REQUIRED_FOR_DESCENDANTS_ESCAPING_THE_GIT_PROCESS_GROUP",
        "the checker must not claim complete containment from process-group cleanup",
    )
    ...
    _expect(Path(cast(str, evidence["git_executable_path"])).is_absolute(), "Git path must be absolute")


def test_ripr_given_repo_local_fsmonitor_when_checker_runs_then_helper_is_never_executed(tmp_path: Path) -> None:
    # Arrange exact E plus an executable repository-local fsmonitor helper.
    ...
    helper = tmp_path / "hostile-fsmonitor.sh"
    helper.write_text(
        "#!/bin/sh\n"
    ...
    # Act through exact-E replay, whose closed Git command set excludes status.
    report = checker.check_risc0_build_host_qualification_v1(
        root=repo,
        expected_parent=base_commit,
    )

    # Assert command-line config overrides the hostile local config.
    _expect(report["artifact_valid"] is True, "exact E must remain replayable")
    _expect(not marker.exists(), "repository-local fsmonitor code must never execute")


def test_ripr_given_replacement_ref_for_e_when_checked_then_original_e_and_c_are_consumed(tmp_path: Path) -> None:
    # Arrange original E and a replacement commit carrying a self-declared QUALIFIED artifact.
    repo, base_commit, implementation_commit = _bootstrap_two_commit_subject(tmp_path)
    outcome = builder.build_artifact_for_head_v1(repo, expected_parent=base_commit)
    _expect(outcome.replay_ready is True, "C must produce the original blocked projection")
    original_raw = canonical_json_bytes_v1(outcome.artifact)
    evidence_commit = _create_artifact_only_e(repo, base_commit)
    forged = _forged_qualified_projection(outcome.artifact)
    _write(repo, ARTIFACT_PATH_V1, canonical_json_bytes_v1(forged))
    _git(repo, "add", ARTIFACT_PATH_V1)
    forged_tree = _git(repo, "write-tree")
    forged_e = _git(repo, "commit-tree", forged_tree, "-p", implementation_commit, "-m", "forged replacement E")
    _git(repo, "reset", "--hard", evidence_commit)
    _git(repo, "replace", evidence_commit, forged_e)

    # Act with replacement refs present in repository metadata.
    report = checker.check_risc0_build_host_qualification_v1(
        root=repo,
        expected_parent=base_commit,
    )

    # Assert Git replacement semantics are disabled for every object query.
    _expect(report["artifact_valid"] is True, "replacement refs must not alter exact E replay")
    _expect(report["status"] == "BLOCKED_TOOLCHAIN_VERSION_MISMATCH", "original blocked status must survive")
    evidence = _json_object(report["exact_evidence"])
    _expect(evidence["evidence_commit"] == evidence_commit, "report must retain original E identity")
    _expect(evidence["implementation_commit"] == implementation_commit, "report must retain original C identity")
    _expect(
        evidence["artifact_sha256"] == sha256_prefixed_v1(original_raw),
        "report must consume original E bytes rather than replacement bytes",
    )


def test_ripr_given_git_graft_when_c_raw_parent_differs_then_subject_rejects(
    tmp_path: Path,
) -> None:
    # Arrange a fake C whose raw parent is Q while a graft advertises P.
    repo, base_commit, implementation_commit = _bootstrap_two_commit_subject(tmp_path)
    base_tree = _git(repo, "rev-parse", f"{base_commit}^{{tree}}")
    implementation_tree = _git(repo, "rev-parse", f"{implementation_commit}^{{tree}}")
    unrelated_parent = _git(repo, "commit-tree", base_tree, "-m", "unrelated raw parent")
    fake_c = _git(
        repo,
        "commit-tree",
        implementation_tree,
        ...
        unrelated_parent,
        "-m",
        "fake implementation subject",
    )
    (repo / ".git" / "info" / "grafts").write_text(
        f"{fake_c} {base_commit}\n",
        encoding="ascii",
    )
    store = builder.GitObjectStoreV1.open_v1(repo)

    # Act
    with pytest.raises(builder.QualificationInputErrorV1) as raised:
        builder.verify_implementation_subject_v1(
            store,
            fake_c,
            expected_parent=base_commit,
        )

    # Assert raw commit headers, rather than graft traversal, own ancestry.
    _expect(raised.value.code == "IMPLEMENTATION_PARENT", "forged graft ancestry must reject")
    _expect(
        store.commit_parents_v1(fake_c) == (unrelated_parent,),
        "raw parent parser must ignore .git/info/grafts",
    )


def test_ripr_given_direct_e_with_forged_qualified_projection_when_checked_then_projection_rejects(
    tmp_path: Path,
) -> None:
    # Arrange direct E with a digest-valid artifact that self-declares QUALIFIED.
    repo, base_commit, _implementation_commit = _bootstrap_two_commit_subject(tmp_path)
    outcome = builder.build_artifact_for_head_v1(repo, expected_parent=base_commit)
    _expect(outcome.replay_ready is True, "C must produce the reference projection")
    forged = _forged_qualified_projection(outcome.artifact)
    builder.write_artifact_atomically_v1(repo, canonical_json_bytes_v1(forged))
    _git(repo, "add", ARTIFACT_PATH_V1)
    _git(repo, "commit", "-qm", "E forged qualification projection")
    ...
    # Assert the checker derives the projection from C and retains no authority.
    _expect(report["artifact_valid"] is False, "self-declared qualification must reject")
    _expect(report["status"] == "REJECTED_ARTIFACT_BINDING", "forged projection needs a typed reject")
    _expect(
        _json_object(_json_array(report["findings"])[0])["code"] == "ARTIFACT_PROJECTION_DRIFT",
        "the forged status must propagate to exact projection mismatch",
    )
    _expect(
        _json_object(report["claim_scope"])["production_authority"] == "NONE",
        "forged evidence must retain no production authority",
    )
...
def test_ripr_given_unavailable_resource_observation_when_projected_then_insufficient_replays() -> None:
    snapshot = _toolchain_passing_snapshot()
    unavailable = ResourceObservationV1(
        tmpdir_matches_required=True,
        free_tmp_bytes=None,
        available_memory_bytes=MINIMUM_AVAILABLE_MEMORY_BYTES_V1,
    )
...
@pytest.mark.parametrize(
    ("free_tmp_bytes", "available_memory_bytes", "expected_status"),
    (
        (
            MINIMUM_TMP_FREE_BYTES_V1,
            MINIMUM_AVAILABLE_MEMORY_BYTES_V1,
            "BLOCKED_BUILD_EVIDENCE_REQUIRED",
        ),
        (
            MINIMUM_TMP_FREE_BYTES_V1 + 1,
            MINIMUM_AVAILABLE_MEMORY_BYTES_V1 + 1,
            "BLOCKED_BUILD_EVIDENCE_REQUIRED",
        ),
        (
            MINIMUM_TMP_FREE_BYTES_V1,
    ...
    ids=("exact-minima", "minima-plus-one", "memory-minus-one", "tmp-minus-one"),
)
def test_ripr_given_resource_boundaries_when_projected_then_exact_status_replays(
    free_tmp_bytes: int,
    ...
    artifact = _replay_resource_projection(snapshot, observation)
    ...
    _expect(
        _json_object(artifact["result"])["status"] == expected_status,
        "resource boundary must use its exact replayed status",
    )


def test_ripr_given_timeout_after_descendant_spawn_when_runner_stops_then_process_group_is_killed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Arrange a parent that spawns a delayed marker-writing descendant.
    ...
    monkeypatch.setattr(builder, "GIT_TIMEOUT_SECONDS_V1", 0.2)

    # Act at the timeout boundary.
    with pytest.raises(builder.QualificationInputErrorV1) as raised:
        builder._run_bounded_process_v1(
            (sys.executable, "-c", parent_code, str(marker)),
            max_stdout_bytes=1024,
        )

    # Assert the timeout is typed and the descendant never survives to write.
    _expect(raised.value.code == "GIT_TIMEOUT", "timeout must retain its stable reject code")
    time.sleep(1.0)
    _expect(not marker.exists(), "timeout must kill the complete child process group")


def test_given_exact_blocked_e_when_checker_cli_runs_then_exit_is_nonzero_and_json_is_fail_closed(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Arrange exact E whose artifact remains intentionally blocked.
    repo, base_commit, _implementation_commit = _bootstrap_two_commit_subject(tmp_path)
    _create_artifact_only_e(repo, base_commit)

    # Act through the CLI adapter with the test fixture's pinned P.
    exit_code = checker.main(["--root", str(repo)], expected_parent=base_commit)
    output = capsys.readouterr().out.encode("utf-8")
    report = decode_json_object_v1(output, "checker-cli.json")

    # Assert artifact validity never becomes process success or qualification.
    _expect(exit_code == 1, "blocked exact E must produce a nonzero checker exit")
    _expect(report["artifact_valid"] is True, "CLI must preserve exact-E artifact validity")
    _expect(report["ok"] is False, "CLI JSON must remain fail closed")
    _expect(report["qualification_eligible"] is False, "CLI must not promote host qualification")
